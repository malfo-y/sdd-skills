...
| `feature-draft` | spec patch 초안과 구현 계획 초안을 한 번에 만든다 | spec 수정과 구현 계획의 반복 작업을 줄인다 | `.claude/agents/feature-draft-agent.md`<br>`.claude/skills/feature-draft/SKILL.md` | wrapper -> agent 패턴(대화 태생 입력은 wrapper가 forwarding, Mode B). `Strategic Code Map`은 context hint로만 읽고 `Touchpoints`와 `Target Files`는 현재 코드로 재확인한다 |
...
| Component | Purpose | Why | Primary Source | Notes |
|-----------|---------|-----|----------------|-------|
| `implementation-plan` | 대규모 구현을 phase/task 단위로 나눈다 | 한 세션에 모든 구현을 몰아넣을 때 생기는 품질 저하를 줄인다 | `.claude/agents/implementation-plan-agent.md`<br>`.claude/skills/implementation-plan/SKILL.md` | `feature-draft` 후속 확장 단계. phase metadata가 autopilot의 execution gate source가 된다 |
...
|---------|------------------|--------|
| Claude wrapper/agent split | 다수의 execution-heavy skill은 thin wrapper가 사용자 entrypoint를 지키고 agent가 전체 계약을 단일 소스로 보유한다. fan-out이 필요한 `implementation`만 orchestrator(skill)+leaf(agent)로, 나머지 non-fan-out skill은 wrapper+single-source agent로 둔다 | `.claude/skills/`, `.claude/agents/` |
