...
|-----------|---------|-----|----------------|-------|
| `sdd-autopilot` | reasoning 기반으로 SDD 파이프라인을 조합하고 end-to-end 실행한다 | 대규모 작업에서 수동 handoff와 단계 누락을 줄인다 | `.claude/skills/sdd-autopilot/SKILL.md`<br>`.codex/skills/sdd-autopilot/SKILL.md` | 풀 스킬. generated orchestrator는 planning producer output을 `plan-review` gate 통과 후 downstream 소비하고, `implementation_agent` / `sdd-skills:implementation-agent` step을 task-level dispatch controller로 해석한다. non-trivial planning은 `feature-draft`를 기본 entry로 사용하고, multi-phase plan이면 `implementation-plan`의 phase `Checkpoint` 필드로 결정되는 `per-group` gate와 adaptive `final integration review`(1 group이면 마지막 group gate가 겸함, 2+ groups이면 별도 1회)를 집행한다 |
| `spec-create` | 초기 global spec과 workspace guidance를 부트스트랩한다 | 스펙 부재 상태에서 workflow 시작점을 만들고, thin global 기본 구조를 고정한다 | `.claude/skills/spec-create/SKILL.md` | 워크플로우 시작점. 기본값은 `_sdd/spec/main.md` 단일 파일이며, 코드베이스가 있으면 primary navigation axis를 하나 고른다. 짧은 `Strategic Code Map`은 appendix로, 긴 map은 supporting surface로 둔다 |
...
| `spec-update-todo` | 구현 전 planned persistent truth를 global spec에 반영한다 | spec-code drift를 사전에 줄인다 | `.claude/agents/spec-update-todo-agent.md`<br>`.claude/skills/spec-update-todo/SKILL.md` | wrapper -> agent 패턴. planned touchpoint를 통째로 복사하지 않고, 장기 navigation value가 있는 code map 후보만 보수적으로 다룬다 |
| `spec-update-done` | 구현 evidence를 검토해 검증된 지속 정보만 global spec에 올린다 | 임시 실행 메모와 검증된 truth를 분리한다 | `.claude/agents/spec-update-done-agent.md`<br>`.claude/skills/spec-update-done/SKILL.md` | delta status 분류 기반 sync. lowercase canonical artifact를 우선 읽고 legacy path를 fallback으로 허용한다. verified persistent `Strategic Code Map` 변화만 승격한다 |
...
| `implementation` | 구현 계획을 따라 코드를 작성하고 검증한다 | execute와 verify를 분리하지 않는 delivery step이 필요하다 | `.claude/skills/implementation/SKILL.md`<br>`.claude/agents/implementation-agent.md` | orchestrator(skill) + leaf(agent). skill이 task-set 확보·dependency 기반 그룹 파생·leaf fan-out·통합/회귀·report를 소유하고, `implementation-agent` leaf는 단일 task TDD만 수행한다(sub-agent spawn 없음). Step 6 Phase Review는 인라인 self-review가 아니라 외부 `implementation-review-agent` review→fix→re-review loop이며(fix=`implementation-agent` finding 순차 재dispatch, leaf는 fix mode 별도 계약 없이 task로 처리), loop scope는 phase 단위 1 gate다(autopilot global/per-group 미차용). AC-first와 재검증 loop가 핵심 |
...
| Spec sync map promotion | `.claude/agents/spec-update-done-agent.md`<br>`.codex/agents/spec-update-done-agent.toml` | temporary touchpoint 중 어떤 항목이 persistent navigation hint로 승격될 수 있는지 확인한다 |
