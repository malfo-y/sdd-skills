...
|-----------|---------|-----|----------------|-------|
| `sdd-autopilot` | reasoning 기반으로 SDD 파이프라인을 조합하고 end-to-end 실행한다 | 대규모 작업에서 수동 handoff와 단계 누락을 줄인다 | `.claude/skills/sdd-autopilot/SKILL.md`<br>`.codex/skills/sdd-autopilot/SKILL.md` | 풀 스킬. 기본 레인은 Step L lite fast-path(`feature-draft-lite` → `plan-review` Tier 2-lite → `implementation-lite` → `implementation-review` → `spec-sync` 메인 루프 체인, orchestrator·pipeline artifact 없음, 게이트당 fix 1회·re-review 없음)다. 규모 초과는 full 전환이 아니라 분할 규칙으로 처리하며 분할 판정 canonical은 lite 표면이 소유한다(autopilot은 신호 소비만). 아래 orchestrator 경로는 사용자 명시 full 요청 시에만 실행된다(🚧 Planned: full 레인 삭제 — 다음 슬라이스). generated orchestrator는 planning producer output을 `plan-review` gate 통과 후 downstream 소비하고, 구현 step을 1급 Step kind `implementation-dispatch-controller`로 선언한다(subagent_type 오버로드 아님). 이 controller는 wave별 3단계(`test-author-agent` 병렬 → orchestrator 소유 RED 게이트 → `implementation-agent` 병렬, cross-wave 중첩 없음)로 task-level leaf를 fan out한다. non-trivial planning은 `feature-draft`를 기본 entry로 사용하고, multi-phase plan이면 `implementation-plan`의 phase `Checkpoint` 필드로 결정되는 `per-group` gate와 adaptive `final integration review`(1 group이면 마지막 group gate가 겸함, 2+ groups이면 별도 1회)를 집행한다 |
...
| `feature-draft` | spec patch 초안과 구현 계획 초안을 한 번에 만든다 | spec 수정과 구현 계획의 반복 작업을 줄인다 | `.claude/skills/feature-draft/SKILL.md`<br>`.claude/agents/feature-draft-agent.md` | orchestrator(skill) + producer(agent). skill이 `feature-draft-agent` 생성 dispatch → `plan-review-agent` review→fix→re-review loop를 소유하고, agent는 draft producer 단일 소스(생성·fix mode)다. 대화에서 태어나는 입력은 orchestrator가 digest로 정리해 생성·fix 라운드 모두에 forwarding한다. `Strategic Code Map`은 context hint로만 읽고 `Touchpoints`와 `Target Files`는 현재 코드로 재확인한다 |
| `feature-draft-lite` | 단일 컨텍스트로 감당되는 변경의 기본 planning 경로 — coverage-index/validation ceremony 없이 task + Target Files + AC 중심 lite draft를 만든다 | 소규모 변경에서 full draft ceremony 비용을 없애고 lite 레인 진입점을 제공한다 | `.claude/skills/feature-draft-lite/SKILL.md`<br>`.codex/skills/feature-draft-lite/SKILL.md` | 메인 루프 직접 작성(신규 agent 없음). 분할 규칙의 canonical 소유자: coverage 눈검산 불가·단일 컨텍스트 초과면 full 승격이 아니라 롤링 분할(Part 1 마커에 분할 feature 목록 → `spec-sync` planned todo 고정, Part 2는 첫 feature task만)로 해소한다. census형 sweep은 분할 대상이 아니라 마지막 read-only 검증 task(변형 표기 전수 grep census AC)로 흡수한다. `> Lite 적격:` 리터럴이 plan-review Tier 2-lite 자가 식별 앵커다 |
| `spec-sync` | 구현 전 planned persistent truth를 사전 반영하거나 구현 후 evidence를 검토해 검증된 지속 정보만 global spec에 올린다 (evidence에 맞춰 적응) | spec-code drift를 사전·사후로 줄이고, 임시 실행 메모와 검증된 truth를 분리한다 | `.claude/agents/spec-sync-agent.md`<br>`.claude/skills/spec-sync/SKILL.md` | wrapper -> agent 패턴. planned touchpoint를 통째로 복사하지 않고 장기 navigation value가 있는 code map 후보만 보수적으로 다룬다. 구현 후에는 delta status 분류 기반 sync로 lowercase canonical artifact를 우선 읽고 verified persistent `Strategic Code Map` 변화만 승격한다 |
| `spec-review` | 스펙 품질과 코드-스펙 drift를 read-only로 진단한다 | 수정 없이 현재 상태를 객관적으로 점검하고, global/temporary rubric을 섞어 오탐하는 것을 줄인다 | `.claude/agents/spec-review-agent.md`<br>`.claude/skills/spec-review/SKILL.md` | wrapper -> agent 패턴. rubric separation, evidence strictness, code map freshness를 기준으로 본다 |
...
| Component | Purpose | Why | Primary Source | Notes |
|-----------|---------|-----|----------------|-------|
| `implementation-plan` | 대규모 구현을 phase/task 단위로 나눈다 | 한 세션에 모든 구현을 몰아넣을 때 생기는 품질 저하를 줄인다 | `.claude/skills/implementation-plan/SKILL.md`<br>`.claude/agents/implementation-plan-agent.md` | orchestrator(skill) + producer(agent). skill이 `implementation-plan-agent` 생성 dispatch → `plan-review-agent` review→fix→re-review loop를 소유하고, agent는 plan producer 단일 소스(생성·fix mode)다. 입력이 파일/경로에서 태어나므로 digest forwarding은 없다. `feature-draft` 후속 확장 단계. phase metadata가 autopilot의 execution gate source가 된다 |
| `plan-review` | 구현 전 계획을 findings-first로 감사한다 | 과잉 설계, 불필요한 새 파일, 약한 검증 기준이 구현으로 전파되기 전에 드러낸다 | `.claude/agents/plan-review-agent.md`<br>`.claude/skills/plan-review/SKILL.md`<br>`.codex/agents/plan-review-agent.toml`<br>`.codex/skills/plan-review/SKILL.md` | wrapper -> agent. review-only gate이며 직접 호출되는 optional plan quality check에서는 Critical/High finding을 implementation blocker로 표시한다. producer review-fix gate에서 소비될 때는 producer loop의 critical/high/medium 종료 조건을 따른다 |
| `implementation` | 구현 계획을 따라 코드를 작성하고 검증한다 | execute와 verify를 분리하지 않는 delivery step이 필요하다 | `.claude/skills/implementation/SKILL.md`<br>`.claude/agents/implementation-agent.md`<br>`.claude/agents/test-author-agent.md` | orchestrator(skill) + 2 leaf(agent). skill이 task-set 확보·dependency 기반 wave 파생·2-stage fan-out·RED/GREEN 게이트·통합/회귀·report를 소유한다(v3.4.0). 각 wave는 Stage A(`test-author-agent` 병렬, 테스트만) → orchestrator 소유 RED 게이트(실패 증거 캡처 + falsifiability 점검) → Stage B(`implementation-agent` 병렬, GREEN→REFACTOR) → GREEN 게이트로 진행하며, wave 간은 순차(cross-wave 중첩 없음)다. impl leaf는 RED를 자체 수행하지 않고 고정 테스트를 수정하지 않으며 이의는 `CONTRACT_MISMATCH`로만 제기한다. Step 6 Phase Review는 외부 correctness ∥ simplicity 2-reviewer review→fix→re-review loop이며(correctness finding=test-first fix, simplicity/refactor=직접 fix), loop scope는 phase 단위 1 gate다. AC-first와 falsifiable RED 게이트가 핵심 |
| `implementation-lite` | lite draft(또는 inline task 목록)를 메인 루프가 직접 RED→GREEN test-first로 구현한다 | 소규모 구현에서 leaf dispatch 오버헤드 없이 test-first 규율을 유지한다 | `.claude/skills/implementation-lite/SKILL.md`<br>`.codex/skills/implementation-lite/SKILL.md` | 작성자 불변식: 코드·테스트는 메인 루프가 직접 작성하고 read-only 보조 agent만 허용한다. 중단·분할 규칙의 canonical 소유자: 단일 세션 초과면 완료 가능한 범위까지 마감하고 잔여를 분할 feature로 draft Part 1 마커 반영 + `spec-sync` planned todo 고정, 계약 오류 선언이 같은 task에서 반복되면 구현 중단 + draft 복귀(full 전환 아님)다 |
| `test-author-agent` | 고정 실패(RED) 테스트만 작성하는 leaf | test 작성과 구현을 별도 렌즈로 분리해 test-first를 orchestrator가 집행 가능한 외부 산출물로 만든다 | `.claude/agents/test-author-agent.md`<br>`.codex/agents/test-author-agent.toml` | leaf only(skill 없음 — `implementation` orchestrator / autopilot dispatch controller가 호출). 입력=task AC + Validation Plan `V*` + Contract/Invariant Delta + 환경/테스트(orchestrator 전달). AC당 관찰 동작 검증 테스트 1개를 `V*`에 1:1 작성하고 테스트 경로는 기존 관습 탐색으로 자체 추론하며, 가정한 인터페이스 계약을 impl-agent에 전달한다. 구현 파일 생성 금지(테스트만), nesting 1단계. 프레임워크 부재 자산은 grep/구조 점검을 RED artifact로 쓴다(분기 canonical surface는 `implementation` SKILL RED 게이트) |
| `implementation-review` | 구현 결과를 계획/AC 기준으로 다시 검증한다 | 누락과 품질 이탈을 조기에 드러낸다 | `.claude/agents/implementation-review-agent.md`<br>`.claude/skills/implementation-review/SKILL.md` | wrapper -> agent. fresh verification 중시. 대화에서 태어나는 입력은 wrapper가 digest로 정리해 forwarding한다 |
| `pr-review` | PR 코드 품질과 spec 준수 여부를 함께 판정한다 | 코드 리뷰와 spec 기반 검증을 한 surface로 묶는다 | `.claude/skills/pr-review/SKILL.md`<br>`.claude/agents/pr-review-agent.md`<br>`.codex/skills/pr-review/SKILL.md`<br>`.codex/agents/pr-review-agent.toml` | orchestrator(skill) + 직교 2-렌즈 병렬 dispatch. correctness(`pr-review-agent`) ∥ simplicity(`simplicity-review-agent`) 두 read-only leaf가 각자 리포트를 쓰고, skill은 verdict 합성 + 통합 리포트만 소유한다. correctness가 inline이 아닌 agent라 `--model` override가 두 렌즈에 균일 적용. findings-first, spec 존재 시 spec-based 추가 검증 |
...
| Autopilot generated orchestrator contract | `.claude/skills/sdd-autopilot/references/orchestrator-contract.md`<br>`.codex/skills/sdd-autopilot/references/orchestrator-contract.md` | producer review gates, implementation dispatch controller, canonical agent invocation names, review-fix severity boundaries를 확인한다 |
| Autopilot planning graph reference | `.claude/skills/sdd-autopilot/references/sdd-reasoning-reference.md`<br>`.codex/skills/sdd-autopilot/references/sdd-reasoning-reference.md` | `feature-draft` precedence, `plan-review` producer gate placement, multi-phase execution unit selection을 확인한다 |
| Claude reusable execution | `.claude/agents/` | wrapper-backed skill의 single-source agent 본문을 찾는 위치다 |
| Implementation orchestrator/leaf | `.claude/skills/implementation/SKILL.md`<br>`.claude/agents/test-author-agent.md`<br>`.claude/agents/implementation-agent.md` | 2-stage fan-out과 orchestrator 소유 RED 게이트가 skill에, 테스트 작성이 `test-author-agent` leaf에, GREEN→REFACTOR가 `implementation-agent` leaf에 어떻게 나뉘는지 확인한다. RED 게이트 서술이 falsifiable test-first 불변식 + graceful-degradation 분기의 canonical surface다 |
