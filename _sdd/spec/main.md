...
- Codex multi-agent dispatch는 Desktop/CLI 같은 surface 이름이 아니라 **활성 tool schema만으로 lifecycle contract를 선택**한다. mailbox schema는 invocation별 parent-tree 고유 `task_name`·`fork_turns: "none"`·target 없는 mailbox wait를 사용하고 완료 agent를 닫지 않는다. target wait와 `close_agent`가 함께 노출된 legacy schema에서만 target/close contract를 유지한다. schema가 불완전하거나 모호하면 mandatory dispatch는 fail closed, optional helper는 inline fallback하며, 없는 lifecycle tool을 검색하거나 두 contract를 섞지 않는다. `agent_type` 같은 role selector는 지원될 때만 추가하는 선택 필드이며, 없으면 framed `message` 계약으로 범용 sub-agent 역할을 부여한다(부재만으로 blocker가 아님). 사용자가 요청한 model·reasoning override가 미지원이면 dispatch를 막는다
- review나 validation이 포함된 workflow는 review-only로 닫지 않고 fix 또는 명시적 잔여 이슈 보고로 마무리한다
  ...
  - **feature planning의 producer↔reviewer 계약은 producer 정본 + verifier pointer로 유지한다**: `feature-draft`가 질문 발동·output 구조·AC evidence·minimum-code의 canonical producer contract를 소유하고, verifier(`plan-review`)는 상세를 복제하지 않는다 — AC 평가방법은 `Verification Weakness`에서 검사하고, draft가 지목한 외부 사실(Target Files 경로·content anchor·`[C]` 생성 사유의 전제·명시 인용 spec 절)의 실재 대조를 마친 뒤에만 근거 부족을 판정하며, 그 뒤에도 근거가 부족하면 읽기를 확장하지 않고 그 smell의 finding을 만들지 않는다. recommendation은 인용 evidence를 해소하는 가장 작은 plan change만 허용한다. `Hidden Decision` 검사는 Open Questions 항목별 결정·확인 필요 여부와 남은 숨은 가정 2개 술어를 rubric 행 하나가 단독 소유한다
  ...
  - **리뷰 읽기 다이어트** — 리뷰 벽시계는 병렬화 완결 이후 pole 작성자의 "읽고 확인할 양"이 결정하므로, 품질 선별 타협 2종을 적용한다. 재제안 금지: **사실 주장을 별도 구조에 선고정해 reviewer 대조 범위를 좁히는 축**(구 Claim Manifest — 도입 당일 철회)은 실측을 뒤집는 새 근거 없이는 재제안 대상이 아니다
    ...
    - **ledger 통과 증거 다이어트**(현 소유자: 각 SKILL의 correctness 절): 문제 있는 verdict(NOT MET·UNTESTED·PARTIAL·FAIL)만 증거 행으로 낸다. 통과(MET)의 형태는 스킬별로 다르다 — `pr-review` AC 검증 ledger는 `MET: #1–#N` 꼴 축약 한 줄로 접고, `implementation-review` Verification ledger는 AC당 증거 포인터 한 줄(`AC1 MET — file:line` 또는 실행 명령 1개)로 낸다. 어느 쪽이든 판정-측 "증거 없는 MET 금지"는 보존하고 증거 본문 전사만 다이어트한다; implementation-review가 포인터 형인 이유는 접기가 판정 생략으로 소비되는 것을 반환에서 검증 가능하게 막기 위해서다(행동 바닥 — [decision_log.md](./decision_log.md) 2026-08-26)
- 구현의 test-first는 `implementation` 스킬이 메인 루프에서 직접 집행하는 실행 규율이다: task별 3-way triage — (a) test: 실패하는 테스트 작성 가능, (b) structural-check: 프레임워크 부재 자산이라도 grep·diff·exit code로 실질 구조를 판정 가능, (c) test-free: 동어반복 check뿐인 non-falsifiable task(분류 근거 1줄 기록 필수, "간단한 구현이라서"는 (c) 자격이 아니며 경계 애매하면 (b)로 보수 분류) — 구현 변경이 있는 (a)/(b)는 RED 실패 관찰 전 구현 금지 → 최소 구현 GREEN → 커버리지 델타 → 마감 시 AC→증거 테이블(증거 없는 AC는 "충족" 금지). producer가 명시한 read-only 검증 task는 fresh 검사 결과로 닫으며 RED를 만들기 위해 정상 구현을 깨지 않는다. 실패는 관련 구현 task로 돌리고, 검증 기준은 호출자가 지정한 draft 또는 inline AC를 우선한다. triage와 규칙의 canonical surface는 `implementation` SKILL이다
...
- 번들 내 leaf dispatch는 런타임 내장 agent type(Claude `general-purpose`, Codex `explorer`)을 사용한다. Claude 전용 `second-opinion`은 독립 Codex 분석을 위한 외부 `codex:codex-rescue` adapter 예외다. 사용 가능한 외부 adapter와 read-only 경계를 확인하고, 미가용이면 미완료로 보고하며 같은 모델의 자체 답변으로 대체하지 않는다. 이 예외는 custom agent를 번들에 추가하거나 중첩 spawn을 허용하지 않는다
- 모든 SDD 파이프라인 스킬의 SKILL.md는 실행 종료 전 자체 검증하는 `## Acceptance Criteria` 섹션을 갖는다 — 선택한 실행 경로에 적용되는 AC를 종료 전에 점검한다. 보완 가능한 산출물 누락은 수정하고, 비대상·이관·외부 blocker·이미 발생한 절차 위반은 해당 경로의 한계와 미충족을 보고한다. 과거 준수를 소급하거나 실패를 성공으로 표시하지 않으며 기존 재시도 상한을 넘지 않는다(`## Goal`은 권장 표면). AC는 그 스킬의 완료 신호 섹션(Required Output·반환·마감·보고 등)을 참조로 결속하고 상세를 재서술하지 않으며, 작성 체크리스트 canonical은 [docs/SKILL_AUTHORING_NORMS.md](../../docs/SKILL_AUTHORING_NORMS.md)다
- goal setup은 목표·권한·검증 기준·하네스 완성을 고정하고 대화 순서는 상황에 맡긴다. 이미 확정한 정보를 재수집하거나 최소 가설 수를 채우지 않는다. PR 리뷰의 깊이는 파일 수보다 변경 위험·관련 AC로 정하고, 검사 결과는 실행 범위와 PASS/FAIL/UNTESTED로 보고한다.
- implementation-review의 변경 집합은 호출자 scope·구현 시작점에 속하는 커밋·staged·unstaged·untracked 변경을 함께 포함한다. 최초 dirty에 있던 변경과 무관한 작업을 구분하되 같은 파일의 이번 hunk는 포함하며, 범위가 불확실하면 한계를 보고한다. 구현 ledger의 필수 resume 정보와 발생한 예외를 구분하고, 채팅 증거는 사용자·활성 goal의 소비 요구를 따른다.
- `sdd-autopilot`은 existing `goal-init(preset=sdd)`를 호출하는 setup-only thin entrypoint다. 사용자 목표와 관련 context를 runtime별 canonical 방식(Claude plugin-prefixed invocation / Codex active installed skill catalog)으로 전달하고, `goal-init`이 만든 자족적 조건 문자열·runtime 실행법·4파일 경로를 relay한 뒤 사용자 activation 경계에서 종료한다
  ...
  - SDD preset은 generic `goal-init`의 결과 중심 판단 기준, condition self-check, `goal.md`·`experiments.md`·`journal.md`·`report.md` 4파일 계약을 그대로 재사용하고 `goal.md`의 `Loop Protocol` payload만 바꾼다. `goal.md`는 조건 문자열(outcome 수준 DONE WHEN + 위조 어려운 anchor + 표준 레시피 참조 문구 + drift 가드·위임 CONSTRAINT) / 검증 레시피(브리틀 검증 디테일 — goal 재설정 없이 수정 가능) / 자율 수행 위임(루프 중 행동에 대한 사용자의 durable authorization — 수준 `unattended`|`attended`, 사전 승인·항상 확인 목록, 질문 대체 규칙; 하류 스킬·런타임 규범의 "실행 전 확인"은 사전 승인 범위에서 이 섹션으로 충족되며 `git` 스킬 CONFIRM이 이를 인정한다) / Loop Protocol(HOW)의 4분할 계약이다. 자율 수준은 setup의 Decision Criteria에 따라 확정한다 — 이미 정한 수준·범위는 재사용하고, 수준이 미정이면 원문의 "알아서/무인" 신호로 `unattended`, 수준·신호 모두 없으면 1회 질문. 루프 중 확인 요청은 사용자가 답할 수 없어 무진척과 같다는 것이 근거다
...
| Codex multi-agent runtime adapter | active tool schema로 mailbox 또는 legacy target/close contract 중 정확히 하나를 선택한다. `agent_type` 같은 role selector는 지원될 때만 추가하고, 없으면 framed `message` 계약을 주입한 범용 sub-agent를 사용한다. legacy target/close는 target wait와 close lifecycle이 함께 노출될 때만 사용한다 | runtime 이름을 capability의 대리값으로 쓰지 않아 schema 세대 차이를 안전하게 흡수하고, 선택적 role selector의 부재를 lifecycle blocker로 오인하지 않으면서 존재하지 않는 lifecycle 호출·혼합 contract·지원되지 않는 사용자 요청 override 전달은 차단한다 |
...
| document producer output interface | `spec-summary`와 `guide-create`는 작성 직전에 runtime-local rich reference의 fenced skeleton을 verbatim 복사하며, reference가 명시한 슬롯 치환·반복 행·조건부 생략 외에는 어떤 줄도 추가·삭제·재배열·요약하지 않는다. 사용자 지정 언어에 맞춘 표시 제목·label 번역은 계층·순서·필드 의미를 유지하는 범위에서 허용하며, output shape·rubric은 그 reference 한 곳만 소유한다. `spec-snapshot`은 별도 reference 없이 양 runtime의 공통 SKILL 본문이 source manifest·destination collision·metadata/body 보존 interface를 소유하고, source pre/post manifest exact match를 완료 hard gate로 둔다. 세 producer는 main loop가 skeleton 또는 file set을 직접 작성·검증하며 runtime helper lifecycle이나 완성 example로 같은 계약을 재소유하지 않는다 | 실제 산출물 interface를 한 곳에서 기계적으로 소비하면 설명·예시·runtime mirror 사이의 format drift를 줄이고, snapshot 원본 read-only와 재현 가능한 보존 경계를 검증할 수 있다 |
...
- 🚧 Planned / 미검증: 실제 Astra/Fable의 스킬 선택·대화·도구 사용 효과는 별도 실행 관측이 필요하다. 이번 변경은 저장소 소스에 한정하며 설치된 plugin cache·외부 adapter는 갱신하지 않았다.
