...
- **sdd-autopilot 메타스킬**: 규모별 적응형 파이프라인을 자동 생성하고 end-to-end 자율 실행
- **듀얼 플랫폼**: Claude Code (`.claude/skills/`) + Codex (`.codex/skills/`) 동시 지원
- **Plugin 배포**: Claude Code Plugin Marketplace를 통한 원클릭 설치
- **규모별 워크플로우**: 대규모(6단계) / 중규모(3단계) / 소규모(1단계) 분리, sdd-autopilot으로 자동 판단 및 실행 가능
...
- sdd-autopilot이 규모별 파이프라인을 생성하고 end-to-end 자율 실행 완료
...
SDD Skills의 중심 설계 원리는 **"SKILL.md = 실행 가능한 프롬프트"**이다. v3.0부터 **스킬 + 에이전트 이중 아키텍처(dual architecture)**를 채택하여, 파이프라인 필수 스킬 8개는 에이전트 정의(`.claude/agents/*.md`)로 전체 로직을 분리하고, 기존 스킬은 에이전트에 위임하는 래퍼(wrapper)로 전환하였다. v3.1에서 8개 에이전트의 `AskUserQuestion`을 모두 제거하고 **Autonomous Decision-Making 패턴**으로 대체하여, sdd-autopilot 파이프라인 내에서 완전 non-interactive 실행이 가능해졌다. 이를 통해:

1. **사용자는 기존처럼 `/스킬명`으로 개별 호출** 가능 (래퍼 스킬)
2. **sdd-autopilot이나 다른 에이전트가 서브에이전트로 호출** 가능 (에이전트 정의)
...
# 실행 흐름 B: sdd-autopilot 자동 호출 (에이전트 직접)
...
**2-Phase Orchestration 패턴**: `sdd-autopilot` 메타스킬에서 사용한다. 사용자 인터랙션을 전반부에 집중하고, 후반부는 완전 자율 실행한다.
...
[Skill Dispatch] ─── skill.json의 description으로 매칭
    ...
    │
    └── [sdd-autopilot] ─── 오케스트레이터 생성 후 에이전트 파이프라인 실행
...
| `_sdd/pipeline/orchestrator_<topic>_<ts>.md` | sdd-autopilot | 생성된 오케스트레이터 |
| `_sdd/pipeline/log_<topic>_<ts>.md` | sdd-autopilot | 파이프라인 실행 로그 |
...
| 카테고리 | 스킬 | 실행 형태 | 역할 |
|----------|------|-----------|------|
| **메타** | sdd-autopilot | 풀 스킬 | 적응형 오케스트레이터 생성 + end-to-end 자율 파이프라인 실행 |
...
| 에이전트 | tools | 호출 경로 |
|----------|-------|-----------|
| feature-draft | Read, Write, Edit, Glob, Grep, Agent | 래퍼 스킬 또는 sdd-autopilot |
| implementation-plan | Read, Write, Edit, Glob, Grep, Agent | 래퍼 스킬 또는 sdd-autopilot |
| implementation | Read, Write, Edit, Glob, Grep, Bash, Agent | 래퍼 스킬 또는 sdd-autopilot |
| implementation-review | Read, Glob, Grep, Agent | 래퍼 스킬 또는 sdd-autopilot |
| ralph-loop-init | Read, Write, Edit, Glob, Grep, Bash | 래퍼 스킬 또는 sdd-autopilot |
| spec-review | Read, Glob, Grep, Agent | 래퍼 스킬 또는 sdd-autopilot |
| spec-update-done | Read, Write, Edit, Glob, Grep, Bash | 래퍼 스킬 또는 sdd-autopilot |
| spec-update-todo | Read, Write, Edit, Glob, Grep | 래퍼 스킬 또는 sdd-autopilot |
...
| **Why** | 대규모 기능 구현 시 6-7개 스킬을 수동 호출하면 맥락 유실과 단계 누락 위험이 있다. sdd-autopilot이 전체 파이프라인을 자동화하여 사용자는 요구사항만 전달하면 된다. |
| **Input** | 사용자의 기능 요청 (자연어) |
| **Output** | 구현 완료된 코드 + 동기화된 스펙 + `_sdd/pipeline/orchestrator_<topic>.md` + `_sdd/pipeline/log_<topic>.md` |
| **Source** | `.claude/skills/sdd-autopilot/SKILL.md` |
...
│       ├── sdd-autopilot/       # [신규] 적응형 오케스트레이터 메타스킬
│       ├── discussion/
...
└── _sdd/
    ...
    └── pipeline/                # [신규] sdd-autopilot 오케스트레이터 + 실행 로그
```
...
| 에이전트 수 | 9개 | 0개 |
| AskUserQuestion | 풀 스킬에서만 사용 (에이전트는 non-interactive) | 미지원 (request_user_input으로 변환) |
| Agent tool | 지원 (서브에이전트 호출) | 제한적 지원 (multi_tool_use.parallel) |
| SKILL.md 차이 | 래퍼 또는 풀 | 풀 SKILL.md만 (래퍼 패턴 미적용) |
| sdd-autopilot | 지원 | 미지원 (Agent 도구 제한) |
...
- `git`, `sdd-upgrade`, `sdd-autopilot` 스킬은 Claude Code 전용 (Codex 미배포)
