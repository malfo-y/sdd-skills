...
- review나 validation이 포함된 workflow는 review-only로 닫지 않고 fix 또는 명시적 잔여 이슈 보고로 마무리한다
  ...
  - `implementation-review`도 같은 두 렌즈(correctness=`implementation-review-agent` ∥ simplicity=`simplicity-review-agent`)를 병렬 dispatch하는 review-only orchestrator이며, correctness는 기준 draft의 Part 2 task가 2개 이상이면 **task별 shard로 분할 dispatch한다(1+N)** — shard k의 digest는 공통 digest에 Task k의 AC·Target Files 범위 한정을 더한 것이고, 전부 한 메시지에서 병렬 dispatch한다. simplicity는 항상 통짜 1회다(중복 탐지 렌즈는 두 지점을 같이 봐야 해서 task 분할이 렌즈 본질을 자른다). task가 1개이거나 draft 없이 대화 digest 기반이면 분할하지 않는다(1+1). relay는 correctness AC ledger의 shard 연접(모든 task의 AC가 정확히 한 shard 소속) + 모든 반환(N+1) 합산 severity이고, shard 간 중복 finding은 dedup 없이 전부 relay한다(dedup은 fix 주체인 호출자 소관). 분할 상한은 두지 않는다 — draft 분할 규칙이 task 수를 소수로 유지하고 simplicity가 자연 바닥이다(YAGNI)
