...
- goal setup은 목표·권한·검증 기준·하네스 완성을 고정하고 대화 순서는 상황에 맡긴다. 이미 확정한 정보를 재수집하거나 최소 가설 수를 채우지 않는다.
- implementation-review의 변경 집합은 호출자 scope·구현 시작점에 속하는 커밋·staged·unstaged·untracked 변경을 함께 포함한다. 최초 dirty에 있던 변경과 무관한 작업을 구분하되 같은 파일의 이번 hunk는 포함하며, 범위가 불확실하면 한계를 보고한다.
