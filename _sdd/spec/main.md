...
- sdd-autopilot이 규모별 파이프라인을 생성하고 end-to-end 자율 실행 완료 (재개/부분 실행 지원)
...
| 2-Phase Orchestration | Phase 1에서 충분히 논의 후 Phase 2는 중단 없이 자율 실행. 파이프라인 효율 극대화. Step 0에서 재개 지원 | 매 단계 사용자 확인 (거부: 중간 중단이 효율 저하) |
...
[_sdd/ 아티팩트 생성/수정]
    ...
    ├── _sdd/pr/             (PR 리뷰/패치)
    └── _sdd/pipeline/       (파이프라인 실행 로그)
    │
    ▼
[.claude/skills/orchestrator_<topic>/]  (생성된 오케스트레이터 스킬)
```
...
| `.claude/skills/orchestrator_<topic>/SKILL.md` | sdd-autopilot | 생성된 오케스트레이터 (스킬로 재사용/재개 가능) |
...
| **Output** | 구현 완료된 코드 + 동기화된 스펙 + `.claude/skills/orchestrator_<topic>/SKILL.md` + `_sdd/pipeline/log_<topic>.md` |
...
└── _sdd/
    ...
    └── pipeline/                # [신규] sdd-autopilot 파이프라인 실행 로그 (오케스트레이터는 .claude/skills/에 저장)
```
