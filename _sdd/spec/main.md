...
- **20개 스킬 + 18개 에이전트**: 스킬은 사용자 인터페이스(`/스킬명`), 에이전트는 플랫폼별로 spawn 가능한 자동화 파이프라인 실행 단위 (Claude 9 + Codex 9). 모든 agent와 full skill은 **AC-First 구조**(Acceptance Criteria + 자체 검증 + Final Check)로 통일
