...
- `sdd-autopilot`은 existing `goal-init(preset=sdd)`를 호출하는 setup-only thin entrypoint다. 사용자 목표와 관련 context를 runtime별 canonical 방식(Claude plugin-prefixed invocation / Codex active installed skill catalog)으로 전달하고, `goal-init`이 만든 자족적 조건 문자열·runtime 실행법·4파일 경로를 relay한 뒤 사용자 activation 경계에서 종료한다
  ...
  - SDD preset은 generic `goal-init`의 Goal Intake → Divergence → Condition Crafting → Harness Setup → Handoff 5단계, condition self-check, `goal.md`·`experiments.md`·`journal.md`·`report.md` 4파일 계약을 그대로 재사용하고 `goal.md`의 `Loop Protocol` payload만 바꾼다. `goal.md`는 조건 문자열(outcome 수준 DONE WHEN + 위조 어려운 anchor + 표준 레시피 참조 문구 + drift 가드·위임 CONSTRAINT) / 검증 레시피(브리틀 검증 디테일 — goal 재설정 없이 수정 가능) / 자율 수행 위임(루프 중 행동에 대한 사용자의 durable authorization — 수준 `unattended`|`attended`, 사전 승인·항상 확인 목록, 질문 대체 규칙; 하류 스킬·런타임 규범의 "실행 전 확인"은 사전 승인 범위에서 이 섹션으로 충족되며 `git` 스킬 CONFIRM이 이를 인정한다) / Loop Protocol(HOW)의 4분할 계약이다. 자율 수준은 Goal Intake에서 확정한다 — 원문의 "알아서/무인" 신호면 되묻지 않고 `unattended`, 없으면 1회 질문. 루프 중 확인 요청은 사용자가 답할 수 없어 무진척과 같다는 것이 근거다
...
- 2026-09-15 리뷰의 73개 항목을 20개 스킬 소스와 직접 reference에 반영했다. 공통 종료·검증-only task·표시 언어·외부 adapter 경계를 정렬했고, Ralph 실행 템플릿의 실패 턴 복원·lock 이후 reset은 격리 회귀로 검증했다. 변경별 근거는 [처리 내역](../../docs/reviews/2026-09-15-skill-instructions/implementation-dispositions.md)에 둔다.
