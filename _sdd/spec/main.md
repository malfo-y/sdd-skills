...
- **19개 스킬**: 스펙 생성부터 PR 리뷰까지 소프트웨어 개발 생명주기 전체를 커버 (코어 16 + 유틸리티 3)
...
- 모든 19개 스킬이 Claude Code에서 `/스킬명`으로 정상 호출 및 실행
...
사용자 ──→ Claude Code / Codex
              ...
              └── SDD Workflow Engine
                    ...
                    └── 유틸리티 스킬 (Git / SDD Upgrade / Spec Snapshot)
```
...
| **보조** | discussion | 구조화된 의사결정 토론 (Claude Code + Codex 지원) |
| | ralph-loop-init | ML 자동 트레이닝 디버그 루프 생성 |
| **유틸리티** | git | 스마트 Git 워크플로우 자동화 (커밋, 브랜치, 리베이스) |
| | sdd-upgrade | SDD 스킬 플러그인 GitHub 업데이트 |
| | spec-snapshot | 스펙 번역 스냅샷 생성 |
...
│       └── (위와 유사 구조, discussion 포함 / git·sdd-upgrade 제외)
│
...
- `git`, `sdd-upgrade` 스킬은 Claude Code 전용 (Codex 미배포)
