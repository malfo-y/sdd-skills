...
- **20개 스킬 + 9개 에이전트**: 스킬은 사용자 인터페이스(`/스킬명`), 에이전트는 서브에이전트로 호출 가능한 자동화 파이프라인 실행 단위 (코어 16 + 유틸리티 3 + 메타스킬 1)
- **autopilot 메타스킬**: 규모별 적응형 파이프라인을 자동 생성하고 end-to-end 자율 실행
- **듀얼 플랫폼**: Claude Code (`.claude/skills/`) + Codex (`.codex/skills/`) 동시 지원
- **Plugin 배포**: Claude Code Plugin Marketplace를 통한 원클릭 설치
- **규모별 워크플로우**: 대규모(6단계) / 중규모(3단계) / 소규모(1단계) 분리, autopilot으로 자동 판단 및 실행 가능
...
- 모든 20개 스킬이 Claude Code에서 `/스킬명`으로 정상 호출 및 실행
- 8개 래퍼 스킬이 대응하는 에이전트에 올바르게 위임
- 9개 에이전트가 서브에이전트로 호출 시 정상 동작
- autopilot이 규모별 파이프라인을 생성하고 end-to-end 자율 실행 완료
- Codex에서 17개 스킬(discussion 포함, git·sdd-upgrade 제외)이 정상 동작
...
SDD Skills의 중심 설계 원리는 **"SKILL.md = 실행 가능한 프롬프트"**이다. v3.0부터 **스킬 + 에이전트 이중 아키텍처(dual architecture)**를 채택하여, 파이프라인 필수 스킬 8개는 에이전트 정의(`.claude/agents/*.md`)로 전체 로직을 분리하고, 기존 스킬은 에이전트에 위임하는 래퍼(wrapper)로 전환하였다. 이를 통해:

1. **사용자는 기존처럼 `/스킬명`으로 개별 호출** 가능 (래퍼 스킬)
2. **autopilot이나 다른 에이전트가 서브에이전트로 호출** 가능 (에이전트 정의)
3. **스킬의 full content는 에이전트 정의에 한 곳에서 관리** (단일 책임)
...
[사용자: /feature-draft]
    → 래퍼 SKILL.md 로드
    → Agent(subagent_type="feature-draft", prompt="...")
    → 에이전트가 Step 1~7 실행
    → 출력: _sdd/drafts/feature_draft_<name>.md

# 실행 흐름 B: autopilot 자동 호출 (에이전트 직접)
[autopilot Phase 2]
    → Agent(subagent_type="feature-draft", prompt="...")
    → 에이전트가 Step 1~7 실행
    → 출력: _sdd/drafts/feature_draft_<name>.md
...
**풀 스킬** (에이전트 전환되지 않은 스킬)은 기존 마크다운 구조를 따른다 [`.claude/skills/*/SKILL.md`]:
...
**에이전트 정의** (`.claude/agents/<name>.md`): 풀 스킬의 전체 로직을 에이전트 frontmatter와 함께 포함한다:
...
# <Agent Title>
(풀 스킬의 전체 로직: Workflow Position, Hard Rules, Process, etc.)
```

**래퍼 스킬** (`.claude/skills/<name>/SKILL.md`): 에이전트로 전환된 스킬의 래퍼:
...
# <Skill Title> (Wrapper)
이 스킬은 `<agent-name>` 서브에이전트에 작업을 위임합니다.
...
## Execution
Agent(subagent_type="<agent-name>", prompt="[사용자의 원래 요청 전문]")
```
...
**Agent Wrapper 패턴**: 스킬을 에이전트로 전환할 때 적용한다. 스킬의 전체 로직은 `.claude/agents/<name>.md` 에이전트 정의로 이동하고, 기존 `.claude/skills/<name>/SKILL.md`는 에이전트에 위임하는 최소한의 래퍼로 전환한다. 이를 통해 사용자의 `/스킬명` 호출 인터페이스를 유지하면서 서브에이전트 호출도 가능하게 한다. `write-phased`에서 최초 검증된 패턴이며, v3.0에서 8개 파이프라인 필수 스킬에 확대 적용하였다.

**2-Phase Orchestration 패턴**: `autopilot` 메타스킬에서 사용한다. 사용자 인터랙션을 전반부에 집중하고, 후반부는 완전 자율 실행한다.
- Phase 1 (Interactive): 사용자와 인라인 discussion → 코드베이스 탐색 → 규모 판단 → 오케스트레이터 생성
- Phase 1.5 (Checkpoint): 생성된 오케스트레이터를 사용자에게 제시 → 확인/수정 → 실행 승인
- Phase 2 (Autonomous): 승인된 오케스트레이터가 에이전트 파이프라인을 자율 실행 → 마일스톤 텍스트 출력 + `_sdd/pipeline/` 로그 기록
...
| Agent Wrapper 패턴 | 스킬 인터페이스 유지 + 서브에이전트 호출 가능. write-phased에서 검증됨 | 스킬 완전 제거 (거부: 사용자 `/스킬명` 호출 불가) |
| 2-Phase Orchestration | Phase 1에서 충분히 논의 후 Phase 2는 중단 없이 자율 실행. 파이프라인 효율 극대화 | 매 단계 사용자 확인 (거부: 중간 중단이 효율 저하) |
...
사용자 ──→ Claude Code / Codex
              ...
              │     │     ├── 풀 스킬 (11개) ─── SKILL.md에 전체 로직 포함
              │     │     ├── 래퍼 스킬 (8개) ─── Agent()로 에이전트에 위임
              │     │     └── 메타스킬 (1개) ─── autopilot (오케스트레이터 생성)
              │     └── .codex/skills/   (Codex 스킬 17개, 풀 SKILL.md 유지)
              │
...
├── references/       # 에이전트가 Read로 참조
│   └── *.md
└── examples/         # 에이전트가 Read로 참조
    └── *.md
...
[Skill Dispatch] ─── skill.json의 description으로 매칭
    ...
    │
    ├── [래퍼 스킬] ─── Agent()로 에이전트에 위임
    │     └── [에이전트 실행] ─── .claude/agents/<name>.md 로드
    │           ├── references/*.md Read로 참조
    │           └── 서브에이전트 호출 가능 (write-phased 등)
    │
    └── [autopilot] ─── 오케스트레이터 생성 후 에이전트 파이프라인 실행
          ├── Phase 1: 사용자 인터랙션 (인라인)
          ├── Phase 1.5: 오케스트레이터 확인
          └── Phase 2: 에이전트 자율 파이프라인 실행
    │
    ▼
[_sdd/ 아티팩트 생성/수정]
    ├── _sdd/spec/           (스펙 문서)
    ...
    ├── _sdd/pr/             (PR 리뷰/패치)
    └── _sdd/pipeline/       (오케스트레이터 + 파이프라인 실행 로그)
```
...
| `_sdd/pipeline/orchestrator_<topic>_<ts>.md` | autopilot | 생성된 오케스트레이터 |
| `_sdd/pipeline/log_<topic>_<ts>.md` | autopilot | 파이프라인 실행 로그 |
...
| 카테고리 | 스킬 | 실행 형태 | 역할 |
|----------|------|-----------|------|
| **메타** | autopilot | 풀 스킬 | 적응형 오케스트레이터 생성 + end-to-end 자율 파이프라인 실행 |
...
| | ralph-loop-init | 래퍼 → 에이전트 | ML 자동 트레이닝 디버그 루프 생성 |
| **유틸리티** | git | 풀 스킬 | 스마트 Git 워크플로우 자동화 (커밋, 브랜치, 리베이스) |
| | sdd-upgrade | 풀 스킬 | SDD 스킬 플러그인 GitHub 업데이트 |
| | spec-snapshot | 풀 스킬 | 스펙 번역 스냅샷 생성 |
| | write-phased | 에이전트 전용 | 2-phase 문서/코드 작성 서브에이전트 |

**에이전트 목록** (`.claude/agents/`):

| 에이전트 | tools | 호출 경로 |
|----------|-------|-----------|
| feature-draft | Read, Write, Edit, Glob, Grep, Agent | 래퍼 스킬 또는 autopilot |
| implementation-plan | Read, Write, Edit, Glob, Grep, Agent | 래퍼 스킬 또는 autopilot |
| implementation | Read, Write, Edit, Glob, Grep, Bash, Agent | 래퍼 스킬 또는 autopilot |
| implementation-review | Read, Glob, Grep, Agent | 래퍼 스킬 또는 autopilot |
| ralph-loop-init | Read, Write, Edit, Glob, Grep, Bash | 래퍼 스킬 또는 autopilot |
| spec-review | Read, Glob, Grep, Agent | 래퍼 스킬 또는 autopilot |
| spec-update-done | Read, Write, Edit, Glob, Grep, Bash | 래퍼 스킬 또는 autopilot |
| spec-update-todo | Read, Write, Edit, Glob, Grep | 래퍼 스킬 또는 autopilot |
...
| Aspect | Description |
|--------|-------------|
| **Purpose** | 사용자 요청을 분석하여 규모별 적응형 오케스트레이터를 생성하고, 에이전트 파이프라인을 end-to-end 자율 실행 |
| **Why** | 대규모 기능 구현 시 6-7개 스킬을 수동 호출하면 맥락 유실과 단계 누락 위험이 있다. autopilot이 전체 파이프라인을 자동화하여 사용자는 요구사항만 전달하면 된다. |
| **Input** | 사용자의 기능 요청 (자연어) |
| **Output** | 구현 완료된 코드 + 동기화된 스펙 + `_sdd/pipeline/orchestrator_<topic>.md` + `_sdd/pipeline/log_<topic>.md` |
| **Source** | `.claude/skills/autopilot/SKILL.md` |
|            | `.claude/skills/autopilot/references/pipeline-templates.md`: 규모별 파이프라인 템플릿 |
|            | `.claude/skills/autopilot/references/scale-assessment.md`: 규모 판단 가이드라인 |
|            | `.claude/skills/autopilot/examples/sample-orchestrator.md`: 오케스트레이터 예시 |
| **Process** | Phase 1 (Interactive): 요청 분석 → 인라인 discussion → 코드베이스 탐색 → 규모 판단 → Pre-flight Check → Phase 1.5 (Checkpoint): 오케스트레이터 생성 + 사용자 확인 → Phase 2 (Autonomous): 에이전트 파이프라인 자율 실행 + 마일스톤 로그 |
...
| **Source** | `.claude/agents/feature-draft.md` (에이전트 정의, 전체 로직) |
|            | `.claude/skills/feature-draft/SKILL.md` (래퍼, v1.1.0) |
...
| **Source** | `.claude/agents/spec-update-todo.md` (에이전트 정의, 전체 로직) |
|            | `.claude/skills/spec-update-todo/SKILL.md` (래퍼, v1.1.0) |
...
| **Source** | `.claude/agents/spec-update-done.md` (에이전트 정의, 전체 로직) |
|            | `.claude/skills/spec-update-done/SKILL.md` (래퍼, v1.1.0) |
...
| **Source** | `.claude/agents/spec-review.md` (에이전트 정의, 전체 로직) |
|            | `.claude/skills/spec-review/SKILL.md` (래퍼, v1.1.0) |
...
| **Source** | `.claude/agents/implementation-plan.md` (에이전트 정의, 전체 로직) |
|            | `.claude/skills/implementation-plan/SKILL.md` (래퍼, v1.1.0) |
| **실행 형태** | 래퍼 → 에이전트 (Agent Wrapper 패턴) |
...
| **Source** | `.claude/agents/implementation.md` (에이전트 정의, 전체 로직) |
|            | `.claude/skills/implementation/SKILL.md` (래퍼, v1.1.0) |
| **실행 형태** | 래퍼 → 에이전트 (Agent Wrapper 패턴) |
...
| **Source** | `.claude/agents/implementation-review.md` (에이전트 정의, 전체 로직) |
|            | `.claude/skills/implementation-review/SKILL.md` (래퍼, v2.1.0) |
| **실행 형태** | 래퍼 → 에이전트 (Agent Wrapper 패턴) |
...
| **제한** | 듀얼 플랫폼 지원. Claude Code는 `AskUserQuestion`, Codex는 `request_user_input` 기반 반복 토론 사용 |
...
|            | `.claude/skills/ralph-loop-init/SKILL.md` (래퍼, v1.1.0) |
| **실행 형태** | 래퍼 → 에이전트 (Agent Wrapper 패턴) |
...
| Aspect | Description |
|--------|-------------|
| **Purpose** | SDD 스킬 플러그인을 GitHub에서 최신 버전으로 업데이트 |
| **Why** | 스킬 업데이트를 수동으로 pull/merge하면 번거롭다. 한 줄 명령으로 최신 스킬을 가져오고 변경 내역을 보고한다. |
| **Input** | 현재 설치된 스킬 상태 |
| **Output** | 업데이트 결과 보고 (before/after 커밋 해시, 변경 목록) |
| **Source** | `.claude/skills/sdd-upgrade/SKILL.md` |
| **제한** | Claude Code 전용 (Codex 미지원) |
...
### Scenario 2b: 대규모 기능 추가 (autopilot 자동 실행)

**Action:**
```bash
/autopilot "인증 시스템 구현 — JWT 기반, 로그인/로그아웃/토큰 갱신"
```

**Expected Result:**
- Phase 1: autopilot이 인라인 discussion으로 요구사항을 구체화하고, 코드베이스를 탐색하여 규모를 판단
...
- `_sdd/pipeline/orchestrator_auth_system_<ts>.md` — 생성된 오케스트레이터
- `_sdd/pipeline/log_auth_system_<ts>.md` — 파이프라인 실행 로그 (각 에이전트 시작/완료, 결정사항, 에러)
- 구현 완료 + 스펙 동기화 완료
...
│   └── AUTOPILOT_GUIDE.md       # autopilot 메타스킬 사용 가이드
│
├── .claude/
│   ├── agents/                  # 에이전트 정의 (9개)
│   │   ├── feature-draft.md
...
│       ├── autopilot/           # [신규] 적응형 오케스트레이터 메타스킬
│       ├── discussion/
│       ├── feature-draft/       # [래퍼] → agents/feature-draft.md
│       ├── git/
│       ├── guide-create/
│       ├── implementation/      # [래퍼] → agents/implementation.md
│       ├── implementation-plan/ # [래퍼] → agents/implementation-plan.md
│       ├── implementation-review/ # [래퍼] → agents/implementation-review.md
│       ├── pr-review/
│       ├── pr-spec-patch/
│       ├── ralph-loop-init/     # [래퍼] → agents/ralph-loop-init.md
│       ├── sdd-upgrade/
│       ├── spec-create/
│       ├── spec-review/         # [래퍼] → agents/spec-review.md
│       ├── spec-rewrite/
│       ├── spec-snapshot/
│       ├── spec-summary/
│       ├── spec-update-done/    # [래퍼] → agents/spec-update-done.md
│       ├── spec-update-todo/    # [래퍼] → agents/spec-update-todo.md
│       ├── spec-upgrade/
│       └── write-phased/
│
├── .codex/
│   └── skills/                  # Codex 스킬 (17개, git·sdd-upgrade 제외)
│       └── (풀 SKILL.md 유지, 에이전트 전환 없음)
│
...
└── _sdd/
    ...
    └── pipeline/                # [신규] autopilot 오케스트레이터 + 실행 로그
```
...
| 에이전트 경로 | `.claude/agents/` | 미지원 |
| 설치 방법 | Plugin Marketplace | LobeHub / 수동 복사 |
| 스킬 수 | 20개 (래퍼 8 + 풀 11 + 메타 1) | 17개 (전부 풀 SKILL.md) |
| 에이전트 수 | 9개 | 0개 |
| AskUserQuestion | 지원 | 미지원 (request_user_input으로 변환) |
| Agent tool | 지원 (서브에이전트 호출) | 제한적 지원 (multi_tool_use.parallel) |
| SKILL.md 차이 | 래퍼 또는 풀 | 풀 SKILL.md만 (래퍼 패턴 미적용) |
| autopilot | 지원 | 미지원 (Agent 도구 제한) |

**Codex 제한 사항:**
- `AskUserQuestion` 도구 미지원 → SKILL.md에서 `request_user_input`으로 변환
- `Agent` 도구 제한 → 병렬 Agent 실행 시 `multi_tool_use.parallel` 사용. 래퍼 패턴 불가
- `discussion` 스킬은 Codex 전용 변형에서 `request_user_input` 기반 반복 루프로 지원
- `git`, `sdd-upgrade`, `autopilot` 스킬은 Claude Code 전용 (Codex 미배포)
- Codex 스킬은 에이전트 전환 없이 기존 풀 SKILL.md 유지 (별도 동기화 후속 작업 필요)
...
|------|-------|---------------|
| `.claude/skills/autopilot/SKILL.md` | autopilot | Core Design, Component Details, Workflow |
...
| `.claude/skills/sdd-upgrade/SKILL.md` | sdd-upgrade | Component Details |
...
- **신규**: autopilot 적응형 오케스트레이터 메타스킬 추가 (`.claude/skills/autopilot/`)
...
- **신규**: `docs/AUTOPILOT_GUIDE.md` 사용 가이드 추가
- **신규**: `_sdd/env.md`에 Autopilot Resources 섹션 추가
- **신규 디자인 패턴**: Agent Wrapper 패턴, 2-Phase Orchestration 패턴 추가
- 스킬 수: 19 → 20 (autopilot 추가), 에이전트 수: 1 → 9 (8개 파이프라인 에이전트 + write-phased)
