...
- review나 validation이 포함된 workflow는 review-only로 닫지 않고 fix 또는 명시적 잔여 이슈 보고로 마무리한다
  ...
  - `plan-review` 게이트도 같은 리포트-분할 메커니즘을 쓰되 분할 축이 다르다 — task 축은 불성립하고(plan-review finding은 섹션 교차·repo 대조형 — 실측 finding 9건 중 task 내부로 닫힌 것 0건) correctness∥simplicity 직교 렌즈도 아니라, **같은 6-smell rubric을 실측 ∥ 판단 2-렌즈로 나눈 병렬 분해**다. orchestrator SKILL이 동일 `plan-review-agent`를 렌즈 한정 digest로 한 메시지 2회 병렬 dispatch하고 병합 relay한다(Blocker Status는 하나라도 BLOCKED면 BLOCKED, findings 합산, smell 판정 합집합 — 각 smell이 정확히 한 렌즈에 소유돼 중복 없음, 규모 판정 검사 결과는 판단 렌즈 반환). 실측 렌즈 = Step 3 도구 계단 + `Verification Weakness` + draft 사실 주장의 repo 대조(판단 렌즈 소유 smell의 사실 전제 — 기존 파일의 수정 수용 가능성, 기존 로직/중복의 실재 여부 — 포함), 판단 렌즈 = 나머지 5 smell + 규모 판정 검사 + Step 4 — Step 3 계단을 밟지 않고 draft 내부 근거로만 판정하며 사실 전제에 UNKNOWN을 내지 않는다(repo 반증은 실측 렌즈 반환에서 온다). agent 자체 검증의 규모 판정·Step 4 항목은 소유 렌즈(판단)에만 적용된다. 렌즈 미지정 dispatch는 전체 6 smell 그대로다(후방 호환). 새 agent 없음(동일 agent 2회 dispatch — 등록 census 리스크 회피). 렌즈 소유 정의의 단일 소스는 agent의 `호출자 렌즈 한정` 절이다
