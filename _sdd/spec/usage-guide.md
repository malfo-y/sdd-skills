...
- `_sdd/discussion/<YYYY-MM-DD>_discussion_<slug>.md` — 토론 배경·결정과 근거·미결·다음 행동을 정리한다. 질문 순서와 라운드 수는 고정하지 않으며, 외부 데이터 대기는 미결로 기록한 뒤 사용자의 선택에 따라 종료할 수 있다. 상세 라운드 로그는 요청 또는 감사 요구가 있을 때만 포함한다.
