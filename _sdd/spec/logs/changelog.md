...
- **`plan-review` `Principle Link` 반환 필드 폐지 · `Step 5` dangling 참조 정정**: rubric 표의 `Principle Link` 열이 `bc26e5c`에서 사라진 뒤 반환 필드 이름만 남아 reviewer가 값 목록 없이 원칙명을 지어내던 상태를 닫았다. finding 블록은 `[Smell] 제목` + Evidence·Affected Plan Surface·Recommended Plan Change·Implementation Blocker 여부다. 자체 검증 AC2·AC3의 `Step 5` 참조는 2-렌즈 철회로 밀린 실제 Return 위치 `Step 4`로 정정했다(claude·codex 짝). 2026-08-11 결정 #2의 "필드는 유지하되" 판단만 supersede하며 하네스 §0 ↔ reviewer 어휘 결합 해제는 보존한다.
- **검증 evidence**: 하네스 5벌 §3 문단 동일(AGENTS.md ↔ 템플릿 diff는 placeholder 적응 delta 4종 — `<repo-name>`·`<test command>`·브랜치/커밋 규칙·섹션 번호 — 만 남고, 템플릿 4벌은 바이트 동일 `744321c371f6`). live 표면 census 잔존 0 — `경량 경로`·`light path`·`적용 범위 게이트`·`Principle Link`(변형형 `Principle-Link`·`principle_link`·소문자 포함)·agent 짝 `Step 5`. Step 헤딩 실제 범위 1~4로 AC 참조 일치. 소비 스킬 SKILL.md는 §3 본문을 인용하지 않아 변경 불요(grep 확인), §번호 불변이라 `§0~§N` 리터럴 무영향. `git diff --check` 무출력.
...
- **검증 evidence**: live 표면 census 잔존 0(`Propagation Surface`·`Discovery evidence`·`Required surfaces`·`propagation 표`·`UNKNOWN`, 날짜 표기 이력 제외). 미러 짝 대조 — `feature-draft` byte-identical, `plan-review` 본문 동일(적응 delta = TOML 헤더·Codex Agent Boundary·Source Pointer·닫는 `'''`), `template-compact` 본문 동일(기존 `$guide-create` 적응만). 사용자 편집 줄의 trailing whitespace 제거로 `git diff --check` 무출력.

#### v4.12.0 (2026-08-12)

- **하네스 §3에 SDD 적용 범위 게이트 신설 — "스펙에 영향 없으면 SDD 대상 아님" · 애매하면 비대상 (post-implementation sync)**: 하네스 §3 첫 줄 `이 repo는 SDD를 따른다. 단계 순서:`를 적용 범위 문단으로 교체했다 — SDD는 **스펙에 영향을 주는 변경**(끝났을 때 `spec-sync`가 반영할 것이 있는 작업)에 적용되고, 스펙이 그대로인 작업(읽기 전용 조사·질문 답변, 커밋되지 않는 일회성 산출물, 규약대로의 기계적 기록)은 대상이 아니며 바로 수행한다. **대상 여부가 애매하면 비대상**이 기본값이다(회색지대에서 체인을 끌어오는 결정은 사용자의 명시 호출 몫이며, 하네스 문면에는 모델이 실행할 규칙만 남겼다 — 사용자가 무엇을 하는지 서술하는 절은 모델에게 지시가 없어 삭제). 관측된 문제: 하네스에 "SDD 비대상" 개념 자체가 없어 유일한 탈출구가 경량 경로뿐이었고, 그 조건 ②(신규 파일 없음)와 "애매하면 풀 체인" 보수 기본값이 겹쳐 **파일 하나 생성 같은 잡일까지 풀 체인으로 라우팅**됐다(Codex 실측). 경량 경로 3조건·보수 기본값은 손대지 않았다 — 적용 범위 게이트는 대상 판정 축, 경량 경로는 깊이 선택 축으로 축이 다르다. 다만 경량 경로의 **항상-풀-체인 목록은 generic화**했다: `새 계약·agent 반환 형식·dispatch 구조 변경·신규 skill/agent 추가`는 스킬/에이전트 제작 repo에서만 해석되는 어휘인 데다 3조건의 재진술(각각 ①·①·①·② 위반)이라 삭제하고, 조건으로 환원되지 않는 한 갈래 — `rename·전파류는 census가 변형형을 놓치기 쉬워 ③을 충족한 듯 보여도 항상 풀 체인` — 만 남겼다(이 repo의 census 실패 실측이 근거이며 rename은 어느 repo에나 있다). 같은 이유로 조건 ③의 예시도 중립화했다 — `(미러·섹션 리터럴·등록 목록)` → `(예: 미러본·참조 리터럴·등록처)`. 전파 표면 5곳(`AGENTS.md` + `{.claude,.codex}/skills/{spec-create,spec-upgrade}/references/agents-harness-template.md`)에 verbatim 동일 적용.
- **검증 evidence**: 5벌 신규 문장 각 1회·해시 동일(`93058487a503`), 구 문장 `이 repo는 SDD를 따른다` live 표면 잔존 0, `git diff --check` 무출력. 소비 스킬 SKILL.md 4벌은 §3 본문을 인용하지 않고 "단계 순서"만 참조해 변경 불요(grep 확인). 경량 경로 채택 건이며 판정 근거는 work log에 기록.
...
- **검증 evidence**: claude·codex agent 짝 + wrapper 2벌 4파일 전파(commit bc26e5c). 18 AC 전부 MET(fresh grep/diff/tomllib), 흡수 무손실은 구 Step 4 4항목·구 `DRY Risk` 3술어·`New File Justification` 2술어가 새 셀에서 전수 식별됨으로 확인. implementation-review gate 1 correctness `H1 L2`(H1 = codex intro 미러 drift, 동기화 후 구 문면 양쪽 0건) / simplicity `M4 L3` → fix 1회 후 재검증 GREEN, 임계값 미달로 gate 2 미호출. census live 표면 0건, producer `> 규모 판정:` 8건 생존.
