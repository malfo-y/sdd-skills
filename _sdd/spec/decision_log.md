...
사용자 실측: 하네스 §3 첫 줄이 `이 repo는 SDD를 따른다`뿐이라, Codex가 파일 하나 만드는 잡일까지 풀 체인으로 수행했다. 문서를 읽으면 그 동작이 오히려 정확하다 — 하네스에 **"SDD 비대상"이라는 상태가 없어** 탈출구가 경량 경로 하나뿐인데, 그 조건 ②는 "신규 파일이 없다"라 파일 생성은 자동 탈락하고, 말미의 "판정이 애매하면 풀 체인"이 남은 회색지대를 전부 무거운 쪽으로 민다.

### Decision

1. **판정 기준은 "스펙에 영향을 주는가"** — 즉 끝났을 때 `spec-sync`가 반영할 것이 있는 작업만 SDD 대상이다. 체인의 종착역을 판정자로 삼아 기준을 실행 가능하게 만들었다. 기각한 대안 둘: (a) "메인 기능 구현에만 적용" — '메인 기능'이 판정 불가 술어라 모델마다 갈리고 결국 안전한 쪽(풀 체인)으로 도망간다, (b) 적용 대상을 경로 목록(`skills/`·`marketplace.json` 등)으로 열거 — 이 repo 전용이라 소비 repo로 나가는 템플릿에 담을 수 없다.
2. **애매하면 비대상**(보수 기본값 반전, 이 축 한정): 대상 여부가 불명확하면 그냥 수행한다. 회색지대에서 체인을 끌어오는 결정권은 사용자에게 있고(`discussion`·`feature-draft` 명시 호출), 그 사실을 서술한 절은 **하네스 문면에서 뺐다** — 사용자가 무엇을 하는지는 모델에게 실행할 지시가 없어 규칙으로 기능하지 않는다. 하네스에는 모델이 실행할 문장만 남긴다.
3. **경량 경로 3조건·"애매하면 풀 체인"은 불변**: 두 규칙은 축이 다르다(대상 판정 vs 대상 확정 후의 깊이). 조건 ②를 "신규 파일이 새 진입점·등록 대상을 만들지 않는다"로 완화하는 안을 검토했으나 **철회**했다 — '진입점·등록 대상'이 다시 판정 불가 술어이고(1과 같은 결함), 스펙 영향 없는 신규 파일은 게이트 1에서 이미 빠지므로 ②에 도달한 신규 파일은 풀 체인이 맞다. 고칠 곳이 두 군데에서 한 군데로 줄었다.

4. **경량 경로의 항상-풀-체인 목록 generic화** (같은 세션 후속 지적): 하네스는 소비 repo로 나가는 템플릿인데 그 목록(`agent 반환 형식`·`dispatch 구조`·`신규 skill/agent`)은 스킬/에이전트 제작 repo에서만 해석된다. 게다가 대부분 3조건의 재진술이라(새 계약·반환 형식·dispatch = ① 위반, 신규 skill/agent = ② 위반) canonical 목록 복제 금지 규범에도 걸린다. 조건으로 환원되지 않는 한 갈래만 남겼다 — `rename·전파류는 census가 변형형을 놓치기 쉬워 ③을 충족한 듯 보여도 항상 풀 체인`(③은 "전수 열거했다"는 자기 판정에 의존하는데 rename census가 변형형을 놓치는 게 이 repo 실측이고, rename은 어느 repo에나 있다). 조건 ③의 예시 괄호도 같은 기준으로 중립화했다(`미러·섹션 리터럴·등록 목록` → `예: 미러본·참조 리터럴·등록처`) — 판정 술어가 아니라 예시라 문면만 바뀌고 적격 판정은 불변이다.

### Rationale / Evidence

- 순증 1문단, 삭제 1문장 + 경량 경로 1문장 축약. 전파 표면 5곳 verbatim 동일(§3 문단 해시 `fa2dcfe11fd6`, 경량 경로 문단 해시 `ff4f920eef5f`), 구 문장 live 잔존 0.
- 이 규칙 자체는 하네스 계약 변경이라 원칙상 풀 체인 대상이나, 설계가 대화에서 확정됐고(=discussion 상당) 5곳 1문장 교체라 경량 경로로 처리했다 — 판정 근거는 work log에 기록.

### Changes

- `AGENTS.md`, `{.claude,.codex}/skills/{spec-create,spec-upgrade}/references/agents-harness-template.md` — §3 첫 문단 교체(5벌 동일)
- `_sdd/spec/main.md` — §2 Guardrails에 적용 범위 게이트 불릿 추가(경량 경로 불릿 앞), v4.11.0 → v4.12.0
...
## 2026-08-17 - `ralph-loop-init` 검증 구조화: verify.sh 분리 + run.sh DONE 게이트 (components-only, post-implementation)

- **배경**: 무인 skip-permissions 루프에서 루프 LLM이 (a) PROMPT.md targeted edit로 Success Criteria를 약화하거나 (b) 검증 없이 `phase: DONE`을 선언하는 자가 성공 경로가 프롬프트 순종에만 막혀 있었다. plan-review 직접 실행 전환(v4.20)에서 확립한 "통제는 순종이 아니라 구조로" 원칙을 ralph 생성물에 적용했다.
- **결정**: ① Step 2 hard gate의 검증 명령을 init이 고정 `ralph/verify.sh`(exit code 판정, 루프 LLM 편집 금지)로 굳혀 판정 단일 소스로 삼는다. ② run.sh DONE 게이트 — final_report.md `Final status:` 부재 시 reject, PASS 주장 시 verify.sh exit 0 추가 요구, reject 시 `ADJUST_PHASE`(시작 시 VALID_PHASES 멤버십 검증) 복귀 + notes 기록; 에스컬레이션 DONE(STUCK 등)은 verify 없이 수용하되 final_report 의무. ③ PROMPT.md에 Success Criteria·Escalation·verify.sh 편집 금지 구역, 장기 실행 in-run 조기 종료 가드, `_sdd/` 수정 금지 추가. 생성 파일 6→7.
- **근거 evidence**: 게이트 4분기 기능 테스트 6/6(양 미러, 템플릿 추출 실행) + 변이 확인(verify 무력화 시 해당 케이스만 FAIL), anchor check 20/20, 미러 parity 비적응 차이 0. draft `_processed_2026-08-17_feature_draft_ralph_verify_hardening.md`, gate 1: correctness Low 1·simplicity Med 1 → fix 반영(set_state_note helper·ADJUST_PHASE 멤버십 가드).
- **범위 밖(불변)**: 기존 소비 repo의 생성된 ralph/ 마이그레이션, phase 구성·discovery 로직, main.md 몸통(버전 불변 — components.md row만 갱신).

## 2026-08-18 - custom agent 전면 폐지: simplicity 계약의 프롬프트 주입 전환 (v4.21.0 → v4.22.0, post-implementation)

- **배경**: Codex 플러그인과 Agent Plugins 1.0 개방 표준(2026-08-06, Codex 0.147.0 지원)의 포터블 번들 범위는 skills+MCP이고 custom agent(TOML/md)는 클라이언트별 수동 배치만 가능하다 — 마지막 잔존 custom agent `simplicity-review-agent`가 codex 번들 배포의 유일한 비번들 표면이었다.
...
## 2026-08-26 - 리뷰 correctness에 행동 바닥(evidence floor) 도입 (v4.27.0 → v4.28.0, post-implementation)

- **배경**: opus-4.8급 모델이 `plan-review`·`implementation-review`에서 correctness를 거의 수행하지 않고 넘기는 관측. 원인 진단: (a) `plan-review`의 "근거 부족→읽기 미확장·finding 없음" 규칙이 아무 대조 없이 CLEAR를 내는 최저 비용 경로를 보상, (b) `implementation-review`의 `MET: AC1–AC5` 접기가 "증거 없는 MET 금지"를 반환에서 검증 불가하게 만들어 판정 생략이 드러나지 않음. 공통: 출력 억제 규칙("줄이는 것은 출력이지 점검 범위가 아니다")이 약한 모델에겐 순종 요구일 뿐 기제가 아님 — 2026-08-15 "읽기 통제는 순종 아닌 구조로"와 같은 교훈. 강한 모델은 바닥 위에서 판단하지만 약한 모델은 바닥을 천장으로 쓴다.
- **결정**: ① `plan-review`: draft가 지목한 외부 사실(Target Files 경로·content anchor·`[C]` 사유 전제·명시 인용 spec 절)은 판정 전 전부 실재 대조, 대조 없는 근거 부족 판정 금지, AC2가 대조 호출 흔적을 판정. draft 내부 판정 smell(Task Boundary Drift·Hidden Decision)에는 도구 흔적을 요구하지 않는다(가짜 호출 유발 방지 — plan gate M1로 정정). ② `implementation-review`: MET은 AC당 증거 포인터 한 줄(`AC1 MET — file:line` 또는 실행 명령 1개), 본문 전사 없음. ③ 채택 안 함: 스크래치 파일 증거 원장(Codex 비호환), correctness를 별도 AC로 승격(자기 체크 항목 추가는 같은 순종 레버), `pr-review` 접기 변경(인간 보조 리뷰, 문제 미관측 — 후속 후보).
- **evidence**: structural check 12/12(구 리터럴 잔존 0·미러 정규화 hunk 3/14), 게이트 결과는 changelog v4.28.0. 효과 계측은 플러그인 갱신 후 별도 세션 필요(work_log 모델별 finding 밀도 비교로 추적).

## 2026-08-26 - goal 하네스에 자율 수행 위임(Autonomy Grant) 섹션 도입 (v4.28.0 → v4.29.0, post-implementation)

- **배경**: native `/goal` 루프 중 에이전트가 commit·push·BC 태스크 제출 앞에서 사용자 확인을 요청하며 턴을 끝낸다. 사용자는 루프 중 답할 수 없으므로 이는 무진척이다. 확인 요청의 출처는 goal-init 문면이 아니라 하류였다 — `git` 스킬 CONFIRM 승인 대기(AC3·AC4 "사용자 확인 없이 push/commit 금지"), 런타임 규범("되돌리기 어려운 행동은 확인"), bc-mcp의 quota 안내.
- **결정**: ① `goal.md`에 `자율 수행 위임` 섹션 — 루프 중 행동에 대한 durable authorization. 수준 `unattended`|`attended`, 사전 승인 목록(기본: 브랜치·commit·feature push·PR 생성 / 테스트·빌드·의존성 / repo 안 파일 / spec-sync / 레시피 동등·강화 변경 / BC 제출·인스턴스 생성 with 리소스 상한), 항상 확인 목록(수준 무관: main 직접 push·force-push·history rewrite / PR merge / 원격·공유 자원 삭제 / 상한 초과 / 판정 약화 / 시크릿 / 외부 발신), 질문 대체 규칙(범위 안 수행+journal / 범위 밖 우회 먼저 / 우회 불가 STUCK). 규칙은 섹션 고정 문장이 단독 소유(payload 복제 안 함 — gate 1 simplicity). ② CONSTRAINTS 표준 문구에 "사전 승인 범위 행동에 확인 요청하며 턴을 끝내지 않는다" — 평가자가 transcript로 판정 가능. ③ goal-init Goal Intake가 수준 확정: 원문 "알아서/자율/무인/확인 없이" 신호면 되묻지 않고 unattended, 없으면 AskUserQuestion 1회. ④ `git` 스킬은 위임을 인정 — AC3·AC4·Hard Rule·Phase 4를 in-place로 "(또는 Phase 3 goal 위임)" 수정(원격 재해석 문장 대신 — gate 2 simplicity). 계획 표시는 불변. ⑤ 채택 안 함: AGENTS.md spec-sync 확인 문구 수정(SDD payload가 직접 실행), settings.json permission allowlist(하네스 밖), bc-mcp 지시문(외부).
- **evidence**: structural check 18/18, 미러 hunk 기준선(템플릿 2·SKILL 6·예제 10·autopilot 3 — `rtk proxy diff`), 게이트 결과는 changelog v4.29.0. 효과 계측은 플러그인 갱신 후 실제 `/goal` 루프에서 확인 요청 턴 수로.

## 2026-09-02 - simplicity dispatch를 사용자 명시 요청으로 선언 (v4.29.0 → v4.30.0)
