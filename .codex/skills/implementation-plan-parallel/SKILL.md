...
description: Use this skill when the user asks to "create a parallel implementation plan", "plan parallel implementation", "parallel roadmap", "병렬 구현 계획", or wants a structured implementation plan with task-level Target Files for conflict-aware parallel execution.
---
...
Use this skill when planning is needed separately from `feature-draft-parallel`.

Keep this file concise and rely on `references/` for detailed patterns.

## Workflow Position

Legacy-compatible planning path:

```
spec -> implementation-plan-parallel (this) -> implementation-parallel -> spec-update-done
```

If the user also needs requirement clarification + spec patch drafting in one run, prefer `feature-draft-parallel`.
...
- Upstream: `spec-create`, `spec-draft`, `feature-draft-parallel` (optional)
- Downstream: `implementation-parallel` (preferred), `implementation` (sequential fallback)
