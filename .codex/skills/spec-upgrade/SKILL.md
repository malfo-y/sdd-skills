...
## Codex Fan-out Upgrade Contract

멀티파일 whitepaper 업그레이드는 다음 순서를 따른다.

- 대표 index/canonical spec을 먼저 확정한다
- 공통 서사 섹션(§1, §2, §3, §5, §8)은 대표 파일에 우선 반영한다
- 컴포넌트별 §4/§6/§7 보강은 `spawn_agent(agent_type="write_phased")`로 병렬 위임한다
- 부모는 `wait_agent(...)`로 결과를 수집하고 cross-file citation/link를 정리한다
...
출력 문서 작성 시 `$write-phased` 전략을 적용한다. Step 4.3의 Whitepaper 구조 변환 규칙(§1-§8)과 Step 1 Gap 분석 결과, Step 2 코드 분석 결과, 기존 스펙 내용을 기반으로 작성하며, skeleton→fill 2-페이즈 전략은 `$write-phased` 내부에서 처리된다.

### Step 4: Backup & Upgrade (백업 및 업그레이드 실행)

**Tools**: `apply_patch`, `functions.exec_command`, `spawn_agent`, `wait_agent`
...
Codex-native fan-out:

```text
1. canonical top-level spec을 먼저 업그레이드한다.
2. 하위 파일별 보강 범위를 확정한다.
3. 하위 파일마다 write_phased agent를 병렬 spawn한다.
4. wait_agent로 모두 수집한다.
5. Appendix: Code Reference Index와 상대 링크를 부모가 최종 정리한다.
```
