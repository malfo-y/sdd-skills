...
- `spec_update_done`은 implementation-scoped review-fix와 required inline validation이 끝난 뒤 최종 단계로만 배치한다.
...
- multi-phase plan이므로 `Review-Fix Loop.scope = per-phase`를 사용하고 각 phase `implementation` 직후 immediate gate를 닫는다.
- 마지막 phase gate가 닫힌 뒤 `final integration review`를 수행하고, 그 이후에만 `spec_update_done`을 실행한다.
