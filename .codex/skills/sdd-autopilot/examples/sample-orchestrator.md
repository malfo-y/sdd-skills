...
**Exit Gate**:
이 step은 단독 완료가 아니다. 같은 범위의 global `Review-Fix Loop`를 즉시 실행해 `critical/high/medium = 0`을 만족하고, required inline validation을 확인한 뒤에만 다음 step으로 진행한다.
...
**Exit Gate**:
각 phase의 `implementation` 직후 같은 phase 범위의 `Review-Fix Loop`와 phase validation을 즉시 닫는다. 현재 phase gate가 닫히기 전에는 다음 phase나 Step 5로 진행할 수 없다.
...
- `Phase Source`: 위 Step 4에 선언된 `Phase Source`를 따른다 (`_sdd/implementation/2026-04-10_implementation_plan_payment_system.md`)
- `phase exit criteria`:
