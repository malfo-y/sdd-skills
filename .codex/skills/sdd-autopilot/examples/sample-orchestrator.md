...
- multi-phase plan이므로 `Review-Fix Loop.scope = per-phase`를 사용하고 각 phase `implementation` 직후 immediate gate를 닫은 뒤, 마지막에 `final integration review` 후 `spec_update_done`까지 연결한다.

## Execution Profiles

> 선택 섹션이다. step 또는 review-fix loop가 어떤 실행 강도를 기본값으로 쓰는지 명시할 때 사용한다.
> multi-phase path에서는 phase gate와 final integration review 해석을 고정해야 하므로, 기본 policy만 따르더라도 이 섹션을 명시하는 편을 기본값으로 본다.
...
- `implementation`: `impl_default` -> `gpt-5.5 / high`
- `implementation_review`: `review_default` -> `gpt-5.5 / high`
- `final_integration_review`: `review_integration_strict` -> `gpt-5.5 / xhigh`
- `spec_update_done`: `spec_sync_default` -> `gpt-5.5 / medium`
...
각 phase 구현 직후에는 같은 phase 범위의 `implementation_review` gate를 즉시 닫아야 하며, gate가 열려 있으면 다음 phase로 진행하지 마세요.
...
- `execution_sequence_per_phase`: `implementation -> implementation_review -> implementation (if needed) -> implementation_review -> phase validation`
- autopilot은 각 phase 구현 직후 현재 phase 범위로 `implementation_review` agent를 즉시 호출한다.
...
- `review_profile`: `review_default` -> `gpt-5.5 / high`
- `fix_profile`: `impl_default` -> `gpt-5.5 / high`
- `implementation_review` 결과에 `critical/high/medium`이 하나라도 있으면 autopilot은 그 finding만 입력으로 묶어 같은 phase 범위의 `implementation` agent를 다시 호출한다.
- fix 후 autopilot은 같은 phase scope로 `implementation_review` agent를 재호출한다.
- 현재 phase gate가 닫히기 전에는 다음 phase 또는 Step 5로 진행하지 않는다.

**Final integration review (must finish before Step 5)**:
- `timing`: 마지막 phase의 per-phase review-fix gate와 phase validation이 모두 닫힌 직후 1회 실행
- 마지막 phase도 예외 없이 자신의 per-phase gate를 먼저 닫은 뒤에만 이 단계로 넘어간다.
- `implementation_review`를 1회 더 실행해 cross-phase regressions를 점검한다.
...
- 모든 phase gate, 마지막 `final integration review`, required inline test가 모두 닫힌 뒤에만 Step 5로 진행한다.
...
- 이 오케스트레이터의 authoritative 순서와 프롬프트 계약은 Step 4의 `Per-phase review-fix gate`와 `Final integration review`에 인라인으로 고정한다.
- 별도 후처리 loop는 없고, 각 phase gate와 마지막 `final integration review`가 닫히기 전에는 `spec_update_done`으로 진행할 수 없다.
...
- `retries`: feature_draft/spec update는 1회, implementation_plan은 1회, 각 phase review-fix gate는 phase당 최대 3회
