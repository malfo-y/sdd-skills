...
1. **Spec-first when available, code-first fallback when absent**: global spec이 있으면 먼저 읽고, 없으면 autopilot은 `_sdd/`를 부트스트랩한 뒤 code-first로 진행하되 파이프라인 안에 `spec-create` 또는 spec sync 시점을 명시한다.
