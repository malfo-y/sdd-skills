...
write-phased는 별도 helper agent가 아니라 caller가 같은 흐름에서 skeleton 작성 + fill + finalize를 수행하도록 돕는 inline writing contract다.
...
| write-phased | inline 2-phase writing contract | caller가 같은 흐름에서 skeleton 작성 후 내용을 채움 |
| git | 커밋/브랜치 자동화 | |
...
보조 에이전트: `explorer` (코드베이스 탐색), `default` (범용 리서치/순차 fill), `worker` (독립 섹션 병렬 fill)
