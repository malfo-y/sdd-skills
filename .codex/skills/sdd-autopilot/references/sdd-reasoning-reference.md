...
write_phased는 다른 스킬의 하위 도구로 중첩 호출.
...
| write_phased | 장문 문서/코드 작성 | 다른 에이전트의 하위 도구로 중첩 호출 |
| git | 커밋/브랜치 자동화 | |
...
보조 에이전트: `explorer` (코드베이스 탐색), `write_phased` (장문 출력 시 중첩 호출), `general-purpose` (범용 리서치)
