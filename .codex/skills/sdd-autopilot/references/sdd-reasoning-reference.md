...
```text
[Default lane: lite fast-path — orchestrator 없음, 메인 컨텍스트 로컬 체인 (autopilot Step L)]
feature-draft-lite
...
#### feature-draft-lite
- Role: 단일 컨텍스트 변경의 경량 draft — Change Summary/Scope + task별 의도·AC·Target Files + Open Questions. 메인 컨텍스트가 직접 작성한다 (dispatch 없음).
...
#### implementation-lite
- Role: lite draft의 실행 짝 — 메인 컨텍스트가 코드·테스트를 직접 작성한다 (RED→GREEN test-first, AC→증거 테이블 마감; read-only 보조 agent 허용, 독립 task는 배칭 병렬 가능).
