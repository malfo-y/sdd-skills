...
- **임시 스펙** (`feature_draft`): 변경의 청사진. Git feature branch처럼 생성->검증->구현->병합->아카이브.
...
| git | 커밋/브랜치 자동화 | |
| pr-review | PR 코드 품질 검증 + from-branch spec 기반 추가 검증, verdict 부여 | |
...
보조 에이전트: `Explore` (코드베이스 탐색), `general-purpose` (범용 리서치)
...
```
feature-draft -> impl -> review-fix -> test -> spec-update-done
```
...
1. **리소스 준비됨**: 입력 데이터, API key, 환경변수, GPU 등 필요 리소스가 `_sdd/env.md` 또는 프로젝트 설정에서 확인됨
2. **E2E가 필요함**: 단위 테스트만으로 기능 검증이 불충분하고 컴포넌트 연동 확인이 필요
3. **실행 명령이 명확함**: `_sdd/env.md` 또는 스펙에서 테스트 명령 도출 가능
4. **자율 실행 가능**: 사용자 인터랙션(브라우저 수동 조작, 물리 디바이스 등) 불필요
...
| 반복 실행→분석→수정 사이클이 필요 / 장시간 실행 / 외부 환경 필요 | **ralph-loop-init** (타임아웃 제약 없음, background 실행) |
