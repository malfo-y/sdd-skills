...
- **글로벌 스펙** (`_sdd/spec/main.md`): 안정적 SoT. 직접 수정 금지. `spec_update_todo/done`으로만 변경.
- **임시 스펙** (`feature_draft`, `spec_patch_draft`): 변경의 청사진. Git feature branch처럼 생성->검증->구현->병합->아카이브.

**임시 스펙 생명주기:**
```
생성(`feature_draft`) -> 검증(사용자 확인) -> 구현(`implementation`) -> 병합(`spec_update_done`) -> 아카이브(prev/)
```
...
1. **Spec-first**: autopilot은 글로벌 스펙이 있을 때만 진행한다. 스펙이 없으면 오케스트레이터 생성을 중단하고 `/spec-create`를 안내한다.
2. **드리프트 방지**: 대규모 변경은 `spec_update_todo`로 사전 등록.
...
6. **스펙 직접 수정 금지**: spec 변경은 `spec_update_todo/done`으로만.
...
```
[precondition] _sdd/spec/main.md exists
...
guide-create, spec_review는 파이프라인 끝에 선택적 추가.
write_skeleton는 다른 스킬/에이전트가 skeleton을 먼저 만들 때 쓰는 하위 도구다. 반환이 `SKELETON_ONLY`이면 caller가 `default` 또는 `worker` agent로 남은 섹션을 채운다.
...
- **Reasoning note**: 대규모 구현에서 phase별 계획 필요 시. 중규모에서는 feature_draft의 Part 2가 대체.
...
| write_skeleton | 장문 문서/코드의 skeleton 생성 | caller가 후속 fill을 맡는 하위 도구 |
| git | 커밋/브랜치 자동화 | |
...
| 에이전트 | agent_type |
|---------|------------|
| feature_draft | `feature_draft` |
| implementation_plan | `implementation_plan` |
| implementation | `implementation` |
| implementation_review | `implementation_review` |
| spec_update_done | `spec_update_done` |
| spec_update_todo | `spec_update_todo` |
| spec_review | `spec_review` |
| ralph_loop_init | `ralph_loop_init` |

보조 에이전트: `explorer` (코드베이스 탐색), `write_skeleton` (장문 출력의 skeleton 생성), `default` (범용 리서치/순차 fill), `worker` (독립 섹션 병렬 fill)
...
```
feature_draft -> implementation -> review-fix -> test -> spec_update_done
```
- `feature_draft`가 스펙 패치 + 구현 계획을 통합 생성하므로 별도 `implementation_plan` 불필요
- test: E2E 테스트 포함 조건(§2.6) 충족 시 포함. 미충족 시 인라인 테스트 또는 생략(사유 명시)
...
```
feature_draft -> spec_update_todo -> implementation_plan -> implementation -> review-fix -> test -> spec_update_done
```
- `spec_update_todo`로 장기 구현 중 드리프트 방지
- `implementation_plan`으로 phase별 상세 계획
...
- **반복 검증/E2E/장시간 테스트**: `ralph_loop_init`을 테스트 단계에 배치 (ML 학습뿐 아니라 E2E·통합 검증에도 적극 고려)
...
| 프로세스 특성 | 전략 |
|--------------|------|
| 단순 단위 테스트/린트 + 1회 실행으로 결과 확정 (수분 이내) | **인라인 디버깅** |
| 반복 실행→분석→수정 사이클이 필요 / 장시간 실행 / 외부 환경 필요 | **ralph_loop_init** (타임아웃 제약 없음, background 실행) |
