...
모든 custom-agent step과 explorer/worker fan-out은 `spawn_agent -> wait_agent -> record -> close_agent -> verify/integrate` 순서를 따른다.
...
- `send_input`이 필요하면 닫기 전에 사용한다. 이미 닫은 agent에 이어서 일을 맡길 필요가 생기면 새 `spawn_agent`를 기본으로 한다.
...
- **fix step dispatch granularity**: `fix = implementation-agent` 재호출은 review finding을 fix-task로 보고 **finding 하나씩 순차로** `implementation-agent` leaf를 spawn하는 것이다(finding의 영향 파일 = 그 leaf의 Target Files). 각 fix leaf는 final status 수거와 기록 직후 `close_agent`로 닫는다. 별도 fix 분해 기계장치는 없으며, fix step에는 병렬을 도입하지 않는다(finding 수가 적고 상호작용 가능 → 순차 안전). 초기 구현 step의 병렬 dispatch 그룹(§2)과 달리 fix는 순차다.
