...
- 스펙 문서만 업데이트하는 경우 → `/spec_update_done` 또는 `/spec_update_todo` 직접 호출

**트리거 키워드**: "sdd-autopilot", "autopilot", "자동 구현", "end-to-end 구현", "전체 파이프라인", "자동으로 구현해줘", "처음부터 끝까지"

## Hard Rules

1. **Discussion은 인라인으로 실행**: Step 2의 사용자 대화는 서브에이전트로 위임하지 않고, autopilot 스킬 내에서 `request_user_input`으로 직접 수행한다.
2. **`_sdd/spec/` 직접 수정 금지**: 스펙 파일은 반드시 `spec_update_done` 또는 `spec_update_todo` 에이전트에 위임하여 수정한다.
3. **Phase 2에서 사용자 중단 없이 실행**: Phase 2(자율 실행) 진입 후에는 `request_user_input`을 호출하지 않는다. 마일스톤 텍스트 출력만 수행한다.
4. **오케스트레이터는 `.codex/skills/`에 저장**: 생성된 오케스트레이터 파일은 `.codex/skills/orchestrator_<topic>/SKILL.md`에 저장한다. 스킬로서 재사용 가능하며, 재개(resume) 시 파이프라인 정의로 활용된다.
...
9. **Review-Fix 사이클 필수**: 파이프라인에 review 단계(implementation_review 또는 모든 형태의 코드 리뷰)가 포함되면, 반드시 review → fix → re-review 사이클을 실행해야 한다. 리뷰만 하고 끝나는 것은 허용하지 않는다. 이 규칙은 전체 파이프라인, 부분 파이프라인, 중간부터 시작하는 파이프라인, 재개(resume) 모두에 적용된다. review가 포함된 파이프라인에서 `implementation_review`는 핵심 단계로 취급하며, 실패 시 건너뛸 수 없다.
10. **Execute → Verify 필수**: 모든 파이프라인 단계는 반드시 (1) 실행(Execute)과 (2) 검증(Verify) 두 페이즈를 거쳐야 한다. 에이전트를 호출한 것만으로 완료로 간주하지 않는다. 검증 페이즈에서 Exit Criteria를 만족하는지 확인하고, 만족하지 않으면 다음 단계로 넘어가지 않는다. 이 규칙은 생성 에이전트(ralph_loop_init 등)에도 동일하게 적용된다 — 설정을 생성한 후 실제로 실행하여 결과를 확인해야 한다.
11. **Spawned agent는 반드시 수집한다**: `spawn_agent(...)` 호출 후에는 `wait_agent(...)`로 결과를 수집하고, 출력 파일/핵심 결정사항을 로그에 남긴 뒤 다음 단계로 진행한다.

## Codex Agent Lifecycle Contract

autopilot에서 agent를 호출할 때는 다음 순서를 표준으로 사용한다.

1. `spawn_agent(...)`로 execution unit 생성
2. 반환된 `agent_id`를 추적
3. `wait_agent(ids=[...])`로 완료/실패를 수집
4. 산출물 파일 경로, severity, 핵심 결정사항만 추출
5. Verify 단계에서 Exit Criteria 검사
6. 필요 시 `send_input(...)` 또는 재-spawn으로 보완

## Process

### Step 0: Pipeline State Detection (파이프라인 상태 감지)

**Tools**: shell (`rg`, `sed`, `cat`)
...
```
rg --files _sdd/pipeline | rg '^_sdd/pipeline/log_.*\.md$'
```
...
- **테스트 전략 판단 기준**: 인라인 디버깅 vs ralph_loop_init을 프로세스 특성 힌트로 판단하는 기준
...
- 기존 산출물 스캔:
  - `_sdd/drafts/feature_draft_*.md` → feature_draft 완료 여부
  - `_sdd/implementation/IMPLEMENTATION_PLAN.md` → implementation_plan 완료 여부
...
**request_user_input 형식**:
```
request_user_input: "[구체적인 질문]"
옵션:
1. "[선택지 A]"
2. "[선택지 B]"
3. "충분합니다 -- 진행해주세요"
...
각 질문은 핵심 분기만 남겨 2-3개 선택지로 구성한다. 마지막 선택지는 항상 "충분합니다 -- 진행해주세요"로 두고, 사용자가 이를 선택하면 즉시 Step 3으로 진행한다.
...
**Tools**: `spawn_agent(agent_type="explorer")`, `wait_agent`, shell (`rg`, `sed`, `cat`)

코드베이스 구조와 관련 파일을 분석하여 규모 판단의 근거를 수집한다.

#### 3.1 explorer 에이전트 병렬 호출

```
structure_agent = spawn_agent(
  agent_type="explorer",
  message="다음 기능 구현을 위해 코드베이스 구조를 분석하세요: [기능 설명]
...
test_agent = spawn_agent(
  agent_type="explorer",
  message="다음 기능 구현을 위해 테스트/검증 관점을 분석하세요: [기능 설명]

  수집 대상:
  1. 기존 테스트 구조 (테스트 프레임워크, 패턴)
  2. 검증에 필요한 실행 경로
  3. 위험 구간과 회귀 가능성"
)
...
spec_agent = spawn_agent(
  agent_type="explorer",
  message="다음 기능 구현을 위해 `_sdd/spec/` 및 관련 문맥을 분석하세요: [기능 설명]

  수집 대상:
  1. _sdd/spec/ 문서 현황 (관련 스펙 섹션 식별)
  2. 기존 결정 로그/제약
  3. 스펙 변경 필요 여부"
)

wait_agent(ids=[structure_agent, test_agent, spec_agent], timeout_ms=1800000)
```
...
- shell `sed`/`cat`: 핵심 파일 내용 확인 (스펙 문서, 설정 파일)
...
IF 일부 explorer agent 실패 → 성공한 결과를 우선 사용하고, 실패한 축만 직접 탐색으로 보완 후 Step 4 진행
```
...
b) **변경 범위 판단**: 스펙 패치? 신규 섹션? → spec_update_todo 필요 여부
c) **계획 깊이**: 직접 구현? feature_draft? impl-plan까지?
d) **검증 수준**: 인라인 테스트? ralph loop? review 포함? 장시간 실행/반복 디버깅/환경 격리 필요성을 함께 판단
...
| ralph_loop_init | 장시간 실행 환경, 반복 디버깅 대상 명령, 결과 artifact 경로, 필요 시 격리 환경 |
...
**Decision Gate 4 → 5**:
```
orchestrator_created = 오케스트레이터 파일이 .codex/skills/orchestrator_<topic>/에 저장됨
...
b) **드리프트 방지**: 대규모 변경에 spec_update_todo 포함?
...
f) **스펙 직접 수정 금지**: spec 변경이 spec_update_todo/done으로만?
...
**Tools**: `request_user_input`, shell (`sed`, `cat`), `apply_patch`
...
| 예상 에이전트 수 | N개 |
| Review 최대 횟수 | 3회 |
| 테스트 전략 | [인라인 디버깅 / ralph_loop_init] |
| 오케스트레이터 경로 | .codex/skills/orchestrator_xxx/SKILL.md |
...
```
request_user_input: "위 파이프라인과 리소스를 확인해 주세요."
옵션:
1. "모두 준비됨 — 실행해주세요"
2. "env.md나 요구사항을 보완한 뒤 재확인"
3. "미확인 리소스는 mock/skip 전략으로 진행"
```

취소가 필요하면 free-form 입력으로 중단 의사를 받는다.

#### 6.3 수정 처리

사용자가 수정을 요청하면:
1. 수정 사항을 파악
2. 오케스트레이터 파일을 `apply_patch`로 수정
3. 수정된 요약을 다시 제시
4. 재확인 (최대 3회 반복)
...
user_cancelled = 사용자가 free-form 입력으로 중단 의사를 표시
...
**Tools**: `spawn_agent`, shell (`rg`, `sed`, `cat`, 테스트 커맨드), `apply_patch`

> **Phase 2 진입**: 이 단계부터 `request_user_input`을 호출하지 않는다. 마일스톤 텍스트 출력만 수행한다.
...
FOR EACH step IN pipeline_steps:
  ...
  2. 로그 기록: 시작 시간 기록 (`apply_patch`)
  3. 에이전트 호출:
     step_agent_id = spawn_agent(
       agent_type="<agent-name>",
       message="<오케스트레이터에 정의된 프롬프트>
...
| 에이전트 | Exit Criteria |
|---------|--------------|
| `feature_draft` | `_sdd/drafts/feature_draft_<topic>.md` 파일 존재 + 요구사항/제약조건 섹션이 비어있지 않음 |
| `implementation_plan` | `_sdd/implementation/IMPLEMENTATION_PLAN.md` 파일 존재 + 태스크가 1개 이상 정의됨 |
| `implementation` | 구현 대상 파일이 생성/수정됨 + 구문 에러 없음 (lint/parse 통과) |
| `implementation_review` | 리뷰 결과에 심각도 분류(critical/high/medium/low)가 포함됨 |
| `ralph_loop_init` | `ralph/` 디렉토리 존재 + `ralph/run.sh` 실행 가능 + `ralph/state.md` 존재 |
| ralph loop 실행 | `ralph/state.md`의 phase가 `DONE` + 최종 테스트 결과가 기록됨 |
| 인라인 테스트 | 테스트 명령 실행 완료 + 전체 테스트 통과 (또는 최대 재시도 후 결과 기록) |
| `spec_update_done` | `_sdd/spec/` 파일이 업데이트됨 + 구현 내용이 스펙에 반영됨 |
| `spec_update_todo` | `_sdd/spec/` 파일이 업데이트됨 + 계획된 기능이 스펙에 추가됨 |
| `spec_review` | 리뷰 결과가 텍스트로 반환됨 + 드리프트/품질 이슈가 분류됨 |
...
implementation 에이전트 완료 후 implementation_review 에이전트로 리뷰를 수행한다.
...
WHILE review_count < MAX_REVIEW:
  ...
  1. implementation_review 에이전트 호출:
     review_agent_id = spawn_agent(
       agent_type="implementation_review",
       message="구현 결과를 리뷰하세요.
     ...
     wait_agent(ids=[review_agent_id], timeout_ms=1800000)
  ...
  4. 수정 실행 (필수 -- 이슈 발견 시 건너뛸 수 없음):
     ...
     - implementation 에이전트 재호출:
       fix_agent_id = spawn_agent(
         agent_type="implementation",
         message="리뷰에서 발견된 critical/high 이슈를 수정하세요.
       ...
       wait_agent(ids=[fix_agent_id], timeout_ms=1800000)
...
1. 먼저 implementation_review를 실행한다
...
test_agent_id = spawn_agent(
  agent_type="implementation",
  message="구현된 코드의 테스트를 실행하고, 실패하는 테스트가 있으면 수정하세요.
...
wait_agent(ids=[test_agent_id], timeout_ms=1800000)
...
**ralph_loop_init** (장시간 실행/반복 디버깅):

> **주의**: ralph_loop_init은 설정 생성(Phase A-1)과 실제 실행+결과 확인(Phase A-2, Phase B)이 모두 완료되어야 한다. 설정만 만들고 넘어가는 것은 **Hard Rule #10 위반**이다.

```
## Phase A-1: Execute (설정 생성)
ralph_loop_init 에이전트로 설정 생성:
ralph_setup_agent_id = spawn_agent(
  agent_type="ralph_loop_init",
  message="다음 기능의 장시간 프로세스 자동 디버깅 루프를 설정하세요.

  기능 설명: <기능 설명>
  실행 명령: <장시간 실행 또는 반복 검증 명령>
  검증 기준: <성공 판단 기준 또는 검증 명령>
  관련 파일: <구현된 파일 목록>"
)
wait_agent(ids=[ralph_setup_agent_id], timeout_ms=1800000)
...
완료 알림 수신 후 Exit Criteria 확인:
  ...
  - 최종 실행/검증 결과가 기록되어 있는가
  - 디버깅 루프가 실제로 실행되었는가 (iteration 횟수 > 0)

IF phase == DONE AND 실행/검증 결과 기록됨:
  → 마일스톤: "[sdd-autopilot] ralph loop 완료 -- 결과: <요약>"
  → 다음 단계(spec_update_done) 진행
IF phase != DONE:
...
- `feature_draft` 에이전트 실패 → 파이프라인 중단 (계획 없이 구현 불가)
- `implementation_plan` 에이전트 실패 → 파이프라인 중단 (구현 계획 없이 구현 불가)
...
예시:
```
[sdd-autopilot] Step 1/6: feature_draft 시작...
[sdd-autopilot] Step 1/6: feature_draft 완료 -- _sdd/drafts/feature_draft_auth_system.md
[sdd-autopilot] Step 2/6: implementation_plan 시작...
[sdd-autopilot] Step 2/6: implementation_plan 완료 -- _sdd/implementation/IMPLEMENTATION_PLAN.md
...
[sdd-autopilot] Step 6/6: spec_update_done 시작...
[sdd-autopilot] Step 6/6: spec_update_done 완료 -- _sdd/spec/main.md 업데이트
```

#### 7.7 로그 파일 관리

각 에이전트 완료 후 로그 파일을 `apply_patch`로 업데이트한다.

로그 엔트리 추가 방식:
```
apply_patch: _sdd/pipeline/log_<topic>_<timestamp>.md
...
```
apply_patch: _sdd/pipeline/report_<topic>_<timestamp>.md
```
...
| 오케스트레이터 | .codex/skills/orchestrator_<topic>/SKILL.md |
