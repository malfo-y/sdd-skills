...
> this skill's functionality is included in **`feature-draft-sequential`** (Part 2: Implementation Plan).
> Use `feature-draft-sequential` when possible; it combines `spec-draft` + `spec-update-todo` + `implementation-plan-sequential` in one step.
