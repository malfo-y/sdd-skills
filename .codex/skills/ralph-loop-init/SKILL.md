...
# Optional completion marker checked when phase becomes DONE
COMPLETION_PROMISE=""
```
...
State Machine Reference 구조를 기반으로 프로젝트에 맞는 `PROMPT.md`를 생성한다.

필수 요소:
1. Anti-recursion warning: `IMPORTANT: Do NOT invoke any skills, modes, or slash commands. Do NOT delegate to other agents. You are inside a standalone automation loop, not an interactive session.`
...
6. Iteration Protocol: config 읽기 → state 읽기 → decisions.md 읽기 → last_exit_code 읽기 → 결과 읽기 → env.md 읽기 → 판단 → action.sh 작성 → state.md 업데이트 → decisions.md 추가 → 종료
7. `action.sh` Rules: `set -euo pipefail`, `source ralph/config.sh`, `mkdir -p ralph/results`, 필요 시 결과를 `ralph/results/`에 저장, `2>&1 | tee` 로깅
8. Known Errors section — `uname -s`로 플랫폼 감지, Darwin이면 timeout fallback 패턴 포함 + Step 1에서 발견한 프로젝트 고유 에러 추가
...
12. Self-correction protocol (`PROMPT.md` 자체 수정 규칙: 같은 원인 2회 이상 실패 시 targeted edit 허용)

### Step 6: Generate `ralph/run.sh`

아래 템플릿 구조를 기준으로 `ralph/run.sh`를 생성하고 `chmod +x ralph/run.sh`를 실행한다.
...
while :; do
  ...
  if grep -q "^phase: DONE" ralph/state.md 2>/dev/null; then
    if [ -n "${COMPLETION_PROMISE}" ]; then
      if grep -Fq "<completion_promise>${COMPLETION_PROMISE}</completion_promise>" "${LAST_MESSAGE_FILE}" 2>/dev/null || \
         grep -Fq "<completion_promise>${COMPLETION_PROMISE}</completion_promise>" "${JSONL_FILE}" 2>/dev/null; then
        echo "[ralph] State is DONE. Completion promise verified. Exiting loop."
      else
        echo "[ralph] WARNING: State is DONE but completion promise not found in Codex output."
...
> **Mirror Notice**: 이 스킬의 본문은 `.codex/agents/ralph-loop-init-agent.toml`의 `developer_instructions` 복사본이다.
