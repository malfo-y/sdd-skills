...
4. 결과 방향, target files, validation 전략을 실질적으로 바꾸는 핵심 ambiguity면 질문 1회를 추가한다. 그 외는 best-effort로 진행하고 저확신 항목은 `Risks / Open Questions`에 기록한다.
...
- `HIGH`: 구조만 정리하고 바로 진행
- `MEDIUM`: 누락된 planning metadata만 보완한다. 방향을 바꿀 핵심 ambiguity가 1개 있으면 질문 1회를 추가한다.
- `LOW`: 방향을 바꿀 핵심 ambiguity가 있으면 질문 1회 후 진행하고, 그 외는 best-effort assumptions로 진행한다. 불확실성은 `Risks / Open Questions`에 기록한다.
...
| 요청이 지나치게 모호함 | 방향을 바꿀 ambiguity면 질문 1회 후 진행, 아니면 best-effort draft를 만들고 missing axes를 `Risks / Open Questions`에 정리 |
