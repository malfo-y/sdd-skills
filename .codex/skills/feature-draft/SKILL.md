...
4. **Multiple features supported**: Multiple features can be included in one file. In Codex default mode, do not pause for confirmation; group related features into one draft by default and record grouping assumptions in `Open Questions` when needed.
...
ELSE → deterministic defaults (non-interactive):
  1. Record that `spec-create` is recommended
  2. Continue in Part 2 only mode (skip Part 1)
```
...
| 50-200 파일 | `rg`/`Glob` + 타겟 `Read` |
| > 200 파일 | `rg`/`Glob` 위주 |

### Step 3: Adaptive Completion

**Tools**: `Read`, `Glob`, `rg`, `Bash`

Apply different completion strategies based on input completeness level:

**HIGH (detailed input)**: Proceed directly. Only normalize wording and structure.

**MEDIUM (partial input)**: Fill in only missing planning metadata from local evidence:
- infer priority from user wording, implementation urgency, and surrounding context
- infer acceptance criteria from explicit behavior described by the user
- infer technical constraints from current spec/code patterns

**LOW (vague input)**: Use deterministic defaults:
1. infer requirement type from user wording and nearby code/spec context
2. consult `references/adaptive-questions.md` as an internal checklist for missing axes
3. continue with best-effort assumptions and record low-confidence items in `Open Questions`

**Multiple features handling**:
- If multiple related features are detected, include them in one draft by default
- If features are clearly independent, still keep one draft unless separation is necessary for correctness; note the recommended split in `Open Questions`
...
IF 모두 충족 → Step 4 진행
ELSE → 최대 2라운드 내부 보완 수행 (`references/adaptive-questions.md` 체크리스트 활용)
...
| Incomplete information | 내부 보완 후 진행, 저신뢰 항목은 `Open Questions`에 기록 |
| User interrupts | Save content gathered so far |
| Multiple features detected in input | 기본은 단일 draft로 묶고, 분리 권고가 필요하면 `Open Questions`에 기록 |
| Cannot determine Target Files | 코드베이스 기준으로 추정하고, 불가 시 `[TBD]`와 근거를 기록 |
