...
ELSE → request_user_input (Plan mode) / direct question (Default mode):
...
3. Use request_user_input (Plan mode) / direct question (Default mode) tool

**Multiple features check**:
- If multiple features are detected in user input:
  - Use request_user_input (Plan mode) / direct question (Default mode) to confirm: "It seems like multiple features are included. Would you like to include them all in one file, or separate them into individual files?"
...
IF 모두 충족 → Step 4 진행
ELSE → 미충족 항목에 대해 request_user_input (Plan mode) / direct question (Default mode) (최대 2라운드)
...
2. request_user_input (Plan mode) / direct question (Default mode): "Part 1 내용을 확인해 주세요."
   옵션:
...
**Tools**: `Write`, `Bash (mkdir/mv)`, `request_user_input (Plan mode) / direct question (Default mode)`, `Glob`

```
1. 요약 테이블 제시 (Part 1 항목 수 + Part 2 Phase/Tasks/병렬도)
2. request_user_input (Plan mode) / direct question (Default mode): "상세 확인하시겠습니까? (Part 1 / Part 2 / 전체)"
3. 요청된 섹션만 상세 출력
