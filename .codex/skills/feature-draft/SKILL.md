...
이 agent는 `_sdd/drafts/<YYYY-MM-DD>_feature_draft_<slug>.md` 한 파일에 다음 두 파트를 함께 생성한다.
...
- [ ] Part 2는 Self-Contained Authoring 원칙(Rule 1/2/3)을 따르며, Self-Containment Check (Pass 1 + Pass 2)를 수행하고 흔적(갭 위치와 보완 내용)을 기록한다.
- [ ] `Risks / Open Questions`의 각 항목이 Decision / Alternatives / Confidence / User confirmation needed 스키마를 따른다 (Hard Rule 4).
- [ ] Part 2의 어느 task도 요청되지 않은 추상화·옵션·설정 가능성·에러 처리를 포함하지 않는다 (Hard Rule 11).
...
4. 결과 방향을 바꿀 수 있는 ambiguity (target files, validation 전략 포함)는 best-effort로 결정하되 `Risks / Open Questions`에 (Decision taken / Alternatives considered / Confidence / User confirmation needed)를 기록한다. 사용자에게 inline 질문을 던지지 않으며, Confidence=LOW 또는 User confirmation needed=Yes인 항목은 Step 8에서 채팅으로 노출한다.
5. global spec이 thin core만 제공하거나 touchpoint가 code-obvious하지 않으면 Step 2 code exploration을 생략하지 않는다.
6. 여러 관련 기능이 보여도 기본적으로 하나의 temporary spec으로 묶고, 분리 vs. 통합 결정은 Rule 4 스키마에 따라 `Risks / Open Questions`의 Alternatives에 기록한다.
...
10. **Self-Contained Authoring (Part 2 대상)**: Part 2 (implementation plan) 산출 본문은 외부 문서나 작성 대화에 암묵 의존하지 않고 reader 단독으로 의도·근거·참조를 따라갈 수 있게 작성한다. 이 원칙은 아래 세 sub-rule로 구성된다.
    ...
    - **Part 1 ↔ Part 2 Carve-out**: 이 원칙은 Part 2에만 적용된다. Part 1 (temporary spec)은 `spec-update-todo`를 통해 canonical spec으로 머지되므로 적용 대상이 아니다. Part 2가 Part 1의 결정·계약을 참조할 때는 같은 파일 내 공존이므로 **ID + inline purpose** (예: "Contract C3 반영 — 세션 토큰 HMAC 검증")만으로 Rule 2 준수로 인정한다. Part 1 전체를 Part 2에 재진술할 필요는 없다.
11. **Minimum-Code Mandate (Part 2 대상)**: Part 2 task의 description과 acceptance criteria는 요청된 동작을 만드는 데 필요한 최소 코드만 명세한다.
...
- 작은 변경이면 `C1/I1/V1` 수준의 compact linkage로 충분하다. traceability는 유지하되 형식적인 row 증식을 만들지 않는다.
...
- 파일이 달라도 아래 패턴이면 의미적 충돌로 보고 같은 phase에 두거나 명시적 dependency를 둔다.
...
- 충돌 판단 확신이 낮으면 병렬보다 순차 계획이 우선이다.
...
1. `_sdd/spec/*.md`
2. lowercase canonical `_sdd/spec/decision_log.md` (있다면), legacy uppercase `_sdd/spec/DECISION_LOG.md` fallback
3. 관련 코드/테스트/설정 파일
...
- global spec이 thin core뿐이어도 `Touchpoints`와 `Target Files`를 코드 기준으로 재확인
...
- `HIGH`: 구조만 정리하고 바로 진행
- `MEDIUM`: 누락된 planning metadata만 보완하고 불확실성은 `Risks / Open Questions`에 Hard Rule 4 스키마로 기록한다.
- `LOW`: best-effort assumptions로 진행하고 불확실성은 `Risks / Open Questions`에 Hard Rule 4 스키마로 기록한다.
...
Part 2 작성 후 Hard Rule 11 self-check를 수행한다:
...
- Part 2에 Hard Rule 11 (Minimum-Code Mandate) 위반 표현이 없는가
- Part 2 Self-Containment Check (Pass 1 + Pass 2) 흔적이 구체적으로 기록됐는가
- thin global spec 상태에서도 code exploration evidence를 바탕으로 `Touchpoints`와 `Target Files`를 잡았는가
...
| 요청이 지나치게 모호함 | best-effort draft를 만들고 missing axes를 `Risks / Open Questions`에 Hard Rule 4 스키마로 정리 |
...
Part 2에 대해 아래 두 pass를 수행하고 흔적을 기록한다. 이 check는 Hard Rule 10 (Self-Contained Authoring)의 검증 절차이다.

**Pass 1 (Reference Enumeration)**: Part 2에 등장하는 모든 외부 파일 경로, spec 조항 ID, 결정 로그 참조를 추출해 목록화한다. 각 항목이 이 문서의 어떤 판단·변경과 연결되는지 inline 서술을 동반하는지 확인한다. 미동반 항목은 해당 위치로 돌아가 Rule 2에 따라 보완한다.

**Pass 2 (Fresh-Reader Readthrough)**: 작성 대화와 외부 파일 접근 없는 독자 관점에서 Part 2의 각 섹션을 순회한다. 각 섹션의 지시·결정·전제 문장에 대해 "이 문장을 따르려면 독자가 뭘 알아야 하는가?" 질문을 던져 답이 문서 내에 없으면 해당 결정·가정이 누락된 것이므로 Rule 1에 따라 surfacing한다.

**흔적 기록 (필수)**: Part 2 말미에 다음 체크리스트를 포함한다. 공허한 값("검토: N, 갭: 0" 식)은 금지 — 발견한 각 갭의 위치와 보완 내용을 구체적으로 적는다.
- 검토 섹션 수: N
