...
- 방향을 바꿀 ambiguity가 크면 질문 1회를 추가한다.
- 그 외에는 `spec-create` 권장 또는 low-confidence 진행 중 하나를 선택할 수 있다.
...
1. guide skeleton/섹션 헤더를 직접 작성
2. 같은 흐름에서 각 섹션 내용을 채움
3. TODO/placeholder를 제거하고 finalize
4. 의존 섹션은 `default`, 독립 시나리오/API 섹션은 `worker`로 채운다
5. helper agent를 썼다면 필요한 경우 먼저 `tool_search` query `spawn_agent wait_agent close_agent multi-agent sub-agent`로 multi-agent tools를 로드한 뒤, `wait_agent` final status 수거 후 결과를 반영하고 `close_agent({target: <agent_id>})`로 닫는다. helper dispatch는 실제 Codex schema인 `spawn_agent({agent_type: "worker", message: ...})`를 사용한다.
...
| citation 후보 부족 | 방향을 바꿀 ambiguity면 질문 1회 후, 아니면 best-effort references만 남기고 한계를 명시 |
| 문서가 너무 김 | 먼저 skeleton을 직접 저장한 뒤 같은 흐름에서 fill |
