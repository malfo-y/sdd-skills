...
2. `spawn_agent(agent_type="implementation-review-agent", prompt=<요청 + 경로 + 대화 맥락 digest>)`로 dispatch하고 `wait_agent`로 결과를 수거한 뒤 `close_agent(target=<agent_id>)`로 handle을 닫는다. 대상 경로가 불명확하면 agent가 Tier Selection·Input 우선순위로 자체 탐색하도록 위임한다.
