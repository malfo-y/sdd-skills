...
- [ ] Speculative Code 차원이 Step 5 Assessment에서 점검되고, Step 6 Findings에 분류 기준이 적용됐다.
- [ ] Recommendations 자체도 Min-Code 원칙을 따른다 — 사변적 권고 금지 (Hard Rule 10).
...
10. **Recommendations Min-Code**: Recommendations 자체도 Min-Code 원칙을 따른다. "future-proof / extensible / configurable" 같은 사변적 권고 금지. 권고는 발견된 실제 결함 또는 측정된 위험에 직접 대응해야 한다.
...
## 4. Recommendations
[Must / Should / Could — 모두 발견된 결함 또는 측정 위험에 직접 연결돼야 한다. 사변적 'future-proof' 권고 금지 (Hard Rule 10)]
...
> **Mirror Notice**: 이 스킬의 본문은 `.codex/agents/implementation-review-agent.toml`의 `developer_instructions` 복사본이다.
