...
If the user does not specify a model family, default to `gpt-5.3-codex` and pick reasoning effort by review complexity.
...
**Tools**: `rg`, `Glob`, `Read`, `Bash`, `Bash (test runner)`
...
1. Use `rg`/`Glob` to find the file/function
...
**Tools**: `Read`, `rg`, `Bash (test runner)`
