...
> Source: correctness 계약·severity·반환 형식은 `.codex/agents/implementation-review-agent.toml`이, simplicity 계약·4개 차원·falsifiable severity는 `.codex/agents/simplicity-review-agent.toml`이 각각 단일 소스로 보유한다 (orchestrator↔agent; 동일 본문 mirror 아님).
