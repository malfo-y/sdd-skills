...
이 스킬은 global spec 또는 temporary spec의 품질과 코드-스펙 정합성을 review-only로 감사하고 `_sdd/spec/logs/spec_review_report.md`를 생성한다. 메인 루프가 dispatch 없이 본문을 직접 수행한다.
...
리포트를 `_sdd/spec/logs/spec_review_report.md`에 저장한다. 리포트 구조는 아래 `## Output Format`을, 후속 스킬 연결은 아래 `## Integration`을 따른다.
