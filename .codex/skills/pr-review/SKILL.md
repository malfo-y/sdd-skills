...
두 reviewer는 sub-agent를 spawn하지 않는 leaf이고 **입력 코드·spec를 수정하지 않는다** — 각자 자기 리포트만 쓴다. correctness reviewer는 테스트 실행을 위해 `Bash`를, 두 reviewer 모두 자기 리포트 저장을 위해 `Write`를 갖지만, 그 write 대상은 각자의 리포트뿐이고 **서로 다른 경로**(`*_pr_correctness_*` vs `*_simplicity_review_*`)라 충돌이 없다. 따라서 한 번에 동시 spawn해도 안전하다.
...
`_sdd/pr/` 디렉토리가 없으면 생성. 세 리포트가 공유할 `slug`를 여기서 정한다 (소문자 snake_case). 이 slug를 두 agent와 통합 리포트에 모두 전달해 `_pr_correctness_<slug>` / `_simplicity_review_<slug>` / `_pr_review_<slug>`가 정렬되게 한다.
