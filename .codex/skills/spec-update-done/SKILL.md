...
이 스킬은 entrypoint wrapper다. 사용자의 spec-update-done 요청을 `spec-update-done-agent`에 위임하고 결과를 사용자에게 전달한다. 전체 sync 프로세스·drift 분류·Repo-wide Invariant Test·Change Report 형식은 agent가 단일 소스로 보유한다.
