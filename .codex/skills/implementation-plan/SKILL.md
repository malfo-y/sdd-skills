...
Phase 전략은 단순하고 설명 가능해야 한다. 복잡한 전략보다 실행 가능성이 우선이다.
...
문서가 길어지면 먼저 `write_skeleton`를 nested agent로 호출해 skeleton을 저장할 수 있다. 반환값이 `COMPLETE`면 그대로 사용하고, `SKELETON_ONLY`이면 `Sections Remaining` 목록을 보고 이 agent가 직접 남은 섹션을 채운다. 의존 섹션은 `default` agent로 순차 fill하고, 독립 섹션은 `worker` agent로 병렬 fill할 수 있다. plan의 필수 구조와 task 내용 결정은 이 agent가 책임진다.
