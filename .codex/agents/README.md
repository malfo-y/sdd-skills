...
장문 산출물이 핵심인 아래 agent는 필요 시 `write_skeleton`를 nested spawn한다. 반환이 `SKELETON_ONLY`이면 caller가 `default` 또는 `worker` agent로 남은 섹션을 채운다.
...
- `spawn_agent(agent_type="write_skeleton")` for skeleton generation, then caller-managed fill via `default` / `worker`
