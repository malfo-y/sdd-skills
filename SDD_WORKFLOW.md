...
4. [리뷰 프로세스](#4-리뷰-프로세스)
5. [장기 실행 디버깅 — Ralph Loop](#5-장기-실행-디버깅--ralph-loop)
6. [빠른 참조](#6-빠른-참조)
...
> 상세 워크플로우와 사용 예시는 [5. 장기 실행 디버깅 — Ralph Loop](#5-장기-실행-디버깅--ralph-loop) 참고.
...
## 5. 장기 실행 디버깅 — Ralph Loop

### Ralph Loop란?

Ralph Loop는 **한 턴의 디버깅에 시간이 오래 걸리는 작업**을 LLM이 자동으로 반복 진행하는 루프입니다. 사람이 매번 개입하지 않아도 LLM이 상태를 분석하고, 액션을 작성·실행하고, 결과를 보고 다음 단계를 판단합니다.
...
**핵심 원리**: `LLM이 분석 → action.sh 작성 → 스크립트 실행(오래 걸림) → 결과 저장 → LLM이 다시 분석` 사이클을 phase가 DONE이 될 때까지 반복합니다.
