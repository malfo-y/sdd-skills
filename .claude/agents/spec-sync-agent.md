...
- [ ] 처리한 input file은 `_processed_*`로 마킹했다.
- [ ] 단일 `Spec Sync Report`(status 컬럼 포함)를 작성했다.
...
각 delta 항목(Part 1 `Persistent Spec Implications`, Part 2 `Contract/Invariant Delta and Coverage`, 또는 정규화된 user input)을 실제 코드 + validation evidence 기준으로 아래 4분류 중 하나로 라우팅한다. 임시 실행 메모는 반영 대상이 아니며, global spec에 올리지 않을 항목도 명시적으로 제외/보류 판단한다.
...
Acceptance Criteria가 모두 만족되었나 1회 점검한다 (Step 7이 검증 패스다 — 추가 수정이 있었을 때만 재점검). 미충족 항목이 있으면 해당 단계로 돌아가 수정한다.
