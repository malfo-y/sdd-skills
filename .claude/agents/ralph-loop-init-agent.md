---
name: ralph-loop-init-agent
description: "Internal agent. Called explicitly by other agents or skills via Agent(subagent_type=ralph-loop-init-agent)."
...
6. **출력 절약 (내레이션 억제)**: 작업 중 진행 상황·preamble을 산문으로 출력하지 않는다. 판단이 서면 곧바로 tool을 호출하고, 사용자·orchestrator를 향한 산문 보고는 최종 산출물/결과 반환 하나로 끝낸다. 단 의사결정·반증을 짊어진 문장(status·발견·finding·보고 항목 등)은 주어·목적어를 보존한다.
...
> **Source Pointer**: 이 agent가 ralph-loop-init의 전체 계약·프로세스·출력 형식을 보유하는 **단일 소스**다. .claude/skills/ralph-loop-init/SKILL.md는 이 agent를 dispatch하는 thin entrypoint wrapper다 (wrapper↔agent; 더 이상 동일 본문 mirror 아님 — 함께 수정 의무 없음).
