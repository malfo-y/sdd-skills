...
- [ ] 실제 구현을 feature draft Part 1 `Persistent Spec Implications`와 Part 2 `Contract/Invariant Delta and Coverage` 및 validation evidence에 대조한다.
...
- [ ] temporary `Touchpoints`는 통째로 복사하지 않고, 구현으로 검증된 persistent `Strategic Code Map` 변화만 반영했다.
...
3. temporary spec의 `Touchpoints`, `Implementation Plan`, validation detail, transient risk log를 global spec 본문에 그대로 남기지 않는다.
...
10. temporary `Touchpoints` 중 장기적으로 반복 사용될 entrypoint, extension point, invariant hotspot, validation surface만 검증된 `Strategic Code Map` 후보로 본다. 나머지 target file / task-level touchpoint는 global spec에 복구하지 않는다.
...
- 현재 global spec
- feature draft Part 1 `Persistent Spec Implications`
- feature draft Part 2 `Contract/Invariant Delta and Coverage`
- 구현 관련 `_sdd/implementation/*`
- 실제 코드/테스트/설정

### Step 2: Compare Delta to Reality

Part 1 `Persistent Spec Implications`와 Part 2 `Contract/Invariant Delta and Coverage`의 각 delta 항목을 실제 구현 및 validation evidence 기준으로 아래 중 하나로 분류한다.
...
- Part 2 coverage 또는 validation evidence와 연결되지 않으면 `UNVERIFIED`로 남긴다.
...
- `Strategic Code Map`에 반영할 verified persistent navigation 변화가 있는지
- drift type
- 적용 action
...
- `Strategic Code Map`은 시작 힌트만 반영하고, temporary `Touchpoints`를 통째로 복사하지 않음
...
- code map이 있다면 현재 코드와 맞는 verified persistent navigation hint만 담고 있는가
...
> **Source Pointer**: 이 agent가 spec-update-done의 전체 계약·프로세스·출력 형식을 보유하는 **단일 소스**다. .claude/skills/spec-update-done/SKILL.md는 이 agent를 dispatch하는 thin entrypoint wrapper다 (wrapper↔agent; 더 이상 동일 본문 mirror 아님 — 함께 수정 의무 없음).
