...
orchestrator가 전달한 입력을 확인한다: PR 변경 파일 목록(리뷰 범위로 **고정**), PR diff, PR metadata(title/body/commits/SHA), from-branch spec 컨텍스트(있으면), slug. spec이 있으면 **spec-based 모드**, 없으면 **code-only 모드**로 진행하고 그 사실을 리포트에 적는다. 범위가 큰 PR(50+ files)이면 디렉토리/컴포넌트 수준으로 축약하고 spec 관련 파일에 집중하며 가정을 리포트에 적는다.
