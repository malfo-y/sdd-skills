...
이 agent는 PR 변경분을 **correctness** 렌즈로 **단일 패스** 리뷰하고 결과를 **최종 응답으로 반환**한다. simplicity reviewer(simplicity-review-agent)의 형제 agent로 표적이 disjoint하며, 통합 리포트 소유자는 orchestrator(pr-review 스킬)다.
...
`PR Review Input`을 읽고 `Changed Files`로 리뷰 범위를 고정한다. discussion은 저자 해명·기지 이슈·리뷰어 우려의 컨텍스트로만 쓰고, `Spec Context`가 있으면 **spec-based**, `NONE (code-only)`면 **code-only**로 진행해 mode를 반환한다. 범위가 큰 PR(50+ files)이면 디렉토리/컴포넌트 수준으로 축약하고 spec 관련 파일에 집중하며 가정을 반환에 적는다.
