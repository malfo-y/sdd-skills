...
plan surface(scope, Target Files, task boundaries, dependencies, parallelism, Part 2 coverage/validation linkage)와 AC·`[C]` 신규 파일·top-level `Risks/Mitigations and Open Questions`·decision markers(assumptions·alternatives·confidence·user-confirmation)를 추출한다. 추출은 리뷰 판단용이다 — §4에 전사하지 않는다.
