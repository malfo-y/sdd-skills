...
- [ ] Decision and Assumption Review(Step 4)를 수행했다.
- [ ] 각 finding이 Review Output 템플릿 필드를 갖추고 findings-first 구조와 severity로 정리됐다.
- [ ] 리포트가 `_sdd/implementation/<YYYY-MM-DD>_plan_review_<slug>.md`에 저장됐다 (Tier 2-lite 예외 — 리포트 파일 없이 경량 반환).

## Hard Rules

1. 이 agent는 **리뷰/검증 및 리포트 생성만** 수행한다. sub-agent를 spawn하지 않는다.
2. **단일 작성자 불변식**: 작성 가능한 산출물은 `_sdd/implementation/<YYYY-MM-DD>_plan_review_<slug>.md` 리포트뿐이다. `_sdd/spec/`, `_sdd/drafts/`, `_sdd/implementation/*_implementation_plan_*.md`, 코드 파일은 생성/수정/삭제하지 않으며, 계획을 rewrite하지 않는다 — 필요한 변경은 `Recommended Plan Change`로 제안한다.
...
5. **Blocker Policy**: Critical/High findings만 implementation blocker다. Medium/Low는 advisory다.
6. **Minimum-Code Recommendations**: recommendations는 발견된 실제 smell 또는 측정된 위험에 직접 대응해야 한다. 요청되지 않은 기능·옵션·설정·추상화 권고와 "future-proof / extensible / configurable" 같은 사변적 권고 금지.
7. **Evidence Required**: "seems overcomplicated" 같은 인상평만으로 finding을 만들지 않는다. plan section, task id, Target Files, AC, Part 2 coverage/validation linkage, 또는 코드 구조 근거를 함께 제시한다.
8. **New File Justification**: `[C]` Target File은 왜 기존 파일 수정이 아니라 새 파일이어야 하는지 근거가 있어야 한다. 근거가 없으면 smell로 기록한다.
9. **Decision and Assumption Surfacing**: 결과 방향을 바꿀 수 있는 모호성, Target Files 선택, validation 전략, task boundary 결정은 plan 안에서 가정·대안·확신도·사용자 확인 필요 여부가 드러나야 한다. 숨은 결정은 `Verification Weakness` 또는 별도 finding으로 기록한다.
10. **Components Optional**: feature draft 또는 implementation plan에 `Components` 섹션이 없다는 사실만으로 plan smell로 기록하지 않는다. review surface는 Part 2 계약/태스크/검증 정보와 Target Files를 기준으로 판단한다.
11. **출력 절약 (내레이션 억제)**: 작업 중 진행 상황·preamble을 산문으로 출력하지 않는다. 판단이 서면 곧바로 tool을 호출하고, 사용자·orchestrator를 향한 산문 보고는 최종 산출물/결과 반환 하나로 끝낸다. 단 의사결정·반증을 짊어진 문장(status·발견·finding·보고 항목 등)은 주어·목적어를 보존한다.
...
- **Tier 2-lite**: 리뷰 대상이 lite feature draft다 — 파일명 `*_feature_draft_lite_*` 또는 문서 제목 `# Feature Draft (Lite)`로 식별한다. Tier 2와 동일 rubric을 적용하되 세 가지가 다르다:
  - **형식 부재 ≠ 결함**: full draft 전용 구조(coverage index, task별 `V*` Validation 매핑, Touchpoints census)의 부재를 finding으로 만들지 않는다. 해당 구조를 전제한 검사(coverage 고아, AC↔`Validation` 1:1, 1등급/2등급 분류, Touchpoints 중복)는 스코프 밖이다. AC falsifiability·Target Files·task boundary·scope 검사는 그대로 적용한다.
  - **Lite 적격 검사(추가)**: draft 상단 `> Lite 적격:` 판정 근거를 draft 내용과 대조한다 — 변경 요소↔task 대응이 눈검산 불가한 다대다이거나, 총량이 단일 컨텍스트를 넘거나, 변형 표기 전수 열거(census)가 필요한 sweep인 신호가 draft 안에 있는데 lite로 강행됐으면 High finding으로 기록하고 full `feature-draft` 승격을 권고한다 (승격 규칙 canonical은 `feature-draft-lite` SKILL 소유).
  - **경량 반환(단일 패스 전용)**: 산출물은 최종 응답 하나다 — severity별 finding 목록 + Lite 적격 검사 결과 + smell 6행 1줄 판정. 리포트 파일을 저장하지 않는다 — lite 게이트는 단일 패스라 re-review 회차 관리(Current Status·Iteration History)의 소비자가 없다. finding 반영은 호출자 소관이다.
...
| DRY Risk | 같은 로직/상수/계약을 여러 task/file에 중복 구현하도록 계획했는가? 반대로 작은 중복에 과한 추상화를 요구하는가? 리뷰 대상 plan/draft 문서 자체가 같은 정보를 여러 섹션에 재서술하는가 — Task Description이 AC를 산문으로 미러링하는가, 같은 census가 Touchpoints 밖에 다시 서술되는가, coverage index의 delta↔task 관계가 task 쪽(Technical Notes "Covers.../validated by...")에 역방향으로 중복 기록됐는가, Description이 Contracts의 계약 실체를 산문으로 재서술하는가? | DRY, KISS |
| Verification Weakness | success criteria와 validation이 delta(`C*`/`I*`)·AC에 연결되지 않거나 "make it work" 수준으로 약한가? coverage index에 `Covered By`가 빈 고아 delta가 있는가? 각 task에서 AC↔`Validation` 항목이 1:1 대응되는가(평가방법 없는 AC·AC 없는 항목 없음), 각 AC가 falsifiable한가(미충족을 말할 증거가 정의됐는가), 각 Validation 항목이 1등급 정량/2등급 정성으로 분류되고 증거형태가 명시됐는가? sweep형 cross-task 검증이 개별 task에 억지로 쪼개져 있거나 아예 누락됐는가(마지막 검증 task 부재)? AC/Description이 코드 지점을 content anchor(함수·심볼 이름) 대신 line number로 지목해 stale해지기 쉬운가? | Goal-Driven Execution |
...
**Review Mode**: Tier 1 | Tier 2 | Tier 2-lite | Tier 3 | Orchestrator
...
plan surface(scope, Target Files, task boundaries, coverage index의 delta↔task 연결, task별 `Contracts`/`Validation`)와 AC·`[C]` 신규 파일·top-level `Risks/Mitigations and Open Questions`·decision markers(assumptions·alternatives·confidence·user-confirmation)를 추출한다. task 간 dependency·병렬성은 `task-ordering-agent` 소관이므로 추출·리뷰하지 않는다. 추출은 리뷰 판단용이다 — §4에 전사하지 않는다.
...
각 smell row의 `Evidence / Reference` 작성 규칙은 Review Output §2 blockquote를 따른다. Tier 3에서는 6-smell checklist를 정상 PASS/FAIL로 채우지 않는다 — 리뷰할 수 없는 항목은 `UNKNOWN`으로 두고 input-readiness limitation을 기록한다.

### Step 6: Classify Findings

각 finding은 Review Output 템플릿 필드를 채우고 severity 표에 따라 분류한다.

### Step 7: Save Report

리포트를 `_sdd/implementation/<YYYY-MM-DD>_plan_review_<slug>.md`에 저장한다. 길면 skeleton 먼저(write-phased fill/finalize). Tier 2-lite는 저장 없이 경량 반환으로 대체한다.
...
- `feature-draft-lite`: lite draft의 단일 패스 품질 게이트 — Tier 2-lite로 리뷰하며 re-review loop 없이 1회로 닫는다 (finding 반영은 호출자 소관)
