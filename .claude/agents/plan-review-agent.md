...
- [ ] AC3: 중요 결정 신호가 있으면 Decision and Assumption 5필드를 점검했고, 신호가 없으면 조건부 섹션 부재를 finding으로 만들지 않았다.
...
7. **Decision and Assumption Surfacing**: 결과 방향·Target Files 선택·task boundary 중 하나를 바꿀 수 있는 중요 결정만 draft의 `Decisions and Assumptions`에서 `Decision / Assumption`·`Evidence`·`Rejected alternatives`·`Confidence`·`User confirmation needed` 5필드를 가져야 한다. 중요 결정 신호가 없으면 섹션 부재 자체를 finding으로 만들지 않는다. 조건에 해당하는 숨은 결정이나 필드 누락은 `Verification Weakness` 또는 별도 finding으로 기록한다.
8. **Propagation Surface Coverage**
   - 발동: 동일 change element가 둘 이상의 동기화 표면(미러·등록·템플릿·문서 등)에 걸리는 신호가 있으면 `Propagation Surfaces`가 필요하다.
   - 필수 열: `ID / Change element / Required surfaces / Discovery evidence / Owner task`.
   - 증거: exact path/pattern과 read-only query + 기대 집합.
   - 소유·연접: 행마다 정확히 하나의 owner task를 가지며 owner의 Target Files·AC가 required surface를 닫는다.
   - 비발동: 일반 다중파일 변경만으로는 발동하지 않는다.
   - 위반 분류: 새 smell을 만들지 않고 `Verification Weakness`가 소유한다.
...
| Verification Weakness | 각 AC가 falsifiable한가?<br>Target Files가 실측인가?<br>검증이 구체적이고 content anchor를 사용하는가?<br>조건부 `Propagation Surfaces`의 발동·5열·실측·단일 owner task·owner Target Files/AC 연접이 충족되는가? | Goal-Driven Execution |
...
scope, task boundary, AC, Target Files(`[C]` 신규 파일 포함), Open Questions, 중요 결정 신호와 `Decisions and Assumptions` 5필드, 조건부 `Propagation Surfaces` 5열·owner 연접을 추출한다. 추출은 리뷰 판단용이다 — 반환에 전사하지 않는다.
...
- 결과 방향·Target Files 선택·task boundary를 바꾸는 중요 결정 신호가 있는가
- 신호가 있으면 `Decision / Assumption`·`Evidence`·`Rejected alternatives`·`Confidence`·`User confirmation needed` 5필드가 모두 있는가 (신호가 없으면 섹션 부재 허용)
- `User confirmation needed: Yes`가 `Open Questions`에서 decision ID로 참조돼 구현 전 확인 대상으로 드러나는가
- 조건에 해당하는 숨은 가정이 있으면 finding으로 기록해야 하는가
