...
scope, task boundary, AC, Target Files(`[C]` 신규 파일 포함), Open Questions, decision markers(가정·대안·확신도·사용자 확인 필요), 조건부 `Propagation Surfaces` 5열·owner 연접을 추출한다. 추출은 리뷰 판단용이다 — 반환에 전사하지 않는다.
