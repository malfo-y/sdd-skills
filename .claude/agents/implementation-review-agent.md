...
## 4. Recommendations
> finding을 재진술하지 않는다 — Must/Should/Could 항목은 finding ID 참조로 갈음한다 (예: `Must: C1`, `Should: M2`). finding에 대응되지 않는 신규 권고만 본문 1줄, 발견된 결함 또는 측정 위험에 직접 연결 (사변적 권고 금지 — Hard Rule 11).
