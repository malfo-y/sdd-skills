...
1. **draft/plan 있음**: 호출자 지정 경로 또는 `_sdd/drafts/*_feature_draft_*.md` 최신. 각 task의 AC가 검증 기준이다. (legacy fallback: `_sdd/implementation/*_implementation_plan_*.md` 등 구형 plan 산출물 — 새로 생성되지 않는 기록물이지만 읽기는 지원한다.)
...
`Critical / High / Medium`은 호출자의 fix 대상이다 (체인에서는 메인 루프가 fix 1회로 반영). `Low`는 로그/후속 권고 대상이다.
...
- `implementation`: 주 리뷰 대상 — draft의 task/AC가 검증 기준
