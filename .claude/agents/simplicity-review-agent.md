...
1. 이 agent는 **단순성 리뷰만** 수행한다. 제안은 반환에만 기록한다.
...
`PR Review Input`이 있으면 `Changed Files`와 `PR Diff`로 리뷰 범위를 고정하고 나머지 필드는 PR 맥락으로만 사용한다. 그 외 호출은 기존 우선순위(호출자 지정 경로/범위 → 변경된 코드 파일; legacy fallback으로 `_sdd/implementation/*_implementation_plan_*.md` 구형 plan 산출물)를 따른다. 범위 불확정 시 최신 변경 범위로 진행하고 가정을 반환 Assumptions에 적는다.
...
- `implementation-review` 스킬: `implementation-review-agent`와 병렬 dispatch되는 형제 리뷰
