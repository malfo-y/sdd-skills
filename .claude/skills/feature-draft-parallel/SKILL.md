...
description: This skill should be used when the user asks to "feature draft parallel", "parallel feature draft", "병렬 기능 초안", "parallel feature plan", or wants to combine requirements gathering, spec patch drafting, and implementation planning with Target Files for parallel execution support.
version: 1.0.0
---

# Feature Draft (Parallel) - Unified Spec Patch + Implementation Plan with Target Files

Collects requirements through conversation with the user, then outputs a spec patch draft and implementation plan as a **single file** — with **Target Files** fields on every task to enable parallel execution via `implementation-parallel`.

## Relationship to `feature-draft`

This skill extends `feature-draft` with one key addition: **Target Files** on each task in Part 2.

| Aspect | `feature-draft` | `feature-draft-parallel` (this) |
|--------|-----------------|--------------------------------|
...
| 3 | implementation-parallel | Execute the plan with parallel sub-agents |
| 4 | spec-update-done | Sync spec with actual code |

> The non-parallel workflow (`feature-draft` → `implementation`) remains available for simpler projects.
...
**This skill**: `feature-draft-parallel` (1 invocation, shared context)
...
**Key difference from `feature-draft`**: Every task includes a `**Target Files**` field.
...
- **Parallel**: Run `implementation-parallel` skill → use Part 2 as implementation plan
- **Sequential**: Run `implementation` skill → use Part 2 as implementation plan (Target Files ignored)
...
Non-parallel fallback:
  Output is fully compatible with `implementation` (Target Files ignored)
```
