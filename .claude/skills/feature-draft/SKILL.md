...
이 스킬은 entrypoint wrapper다. 사용자의 feature-draft 요청을 `sdd-skills:feature-draft-agent`에 위임하고 그 결과를 사용자에게 전달한다. temporary spec 7섹션·Part 2 작성 규칙·Hard Rules·출력 형식은 agent가 단일 소스로 보유한다.
...
2. `Agent(subagent_type="sdd-skills:feature-draft-agent", prompt=<요청 + 경로 + 대화 맥락 digest>)`로 dispatch한다.
...
> Source: 전체 계약·temporary spec 7섹션·작성 규칙은 `.claude/agents/feature-draft-agent.md`가 단일 소스로 보유한다 (wrapper↔agent; 더 이상 동일 본문 mirror 아님).
