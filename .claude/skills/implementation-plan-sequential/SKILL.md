---
name: implementation-plan-sequential
description: This skill should be used when the user asks to "create a sequential implementation plan", "implementation plan sequential", "legacy implementation plan", "순차 구현 계획", or explicitly wants a non-parallel implementation plan without Target Files.
version: 1.0.0
---
...
> In the simplified 4-step workflow (`spec → feature-draft-sequential → implementation-sequential → spec-update-done`),
> this skill's functionality is included in the **`feature-draft-sequential`** skill (Part 2: Implementation Plan).
> Consider using `feature-draft-sequential` instead, which combines `spec-draft` + `spec-update-todo` + `implementation-plan-sequential` into a single step.
