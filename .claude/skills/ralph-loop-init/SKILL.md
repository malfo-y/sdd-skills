...
이 스킬은 entrypoint wrapper다. 사용자의 ralph-loop-init 요청을 `sdd-skills:ralph-loop-init-agent`에 위임하고 그 결과를 사용자에게 전달한다. 전체 discovery 프로세스·상태 머신·파일 생성·CHECKS 검증·출력 형식은 agent가 단일 소스로 보유한다.
...
> Source: 전체 계약·상태 머신·run.sh 템플릿·출력 형식은 `.claude/agents/ralph-loop-init-agent.md`가 단일 소스로 보유한다 (wrapper↔agent; 더 이상 동일 본문 mirror 아님).
