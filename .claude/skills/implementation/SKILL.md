...
- [ ] AC3: wave별 2-stage로 leaf를 dispatch(claude는 `sdd-skills:` prefix 필수) — Stage A는 `sdd-skills:test-author-agent`에 입력(task AC + Validation Plan `V*` + Contract/Invariant Delta + 환경/테스트)을 전달하고, RED 게이트 통과 후 Stage B는 `sdd-skills:implementation-agent`에 입력(task 필드 + Target Files + 환경/테스트 + 선행 보장 + 고정 실패 테스트 + RED 증거)을 전달. 단 RED 게이트가 **(c) test-free**로 triage한 task는 Stage A를 스킵하고, 고정 실패 테스트/RED 증거 대신 triage 근거를 Stage B 입력으로 전달
...
For each phase:
  ...
  2. For each wave (순차 — 이전 wave의 GREEN 게이트 통과 후 다음 wave 시작):
     Triage (Stage A dispatch 직전 1회, RED 게이트 소유): wave 내 task를 (a) test / (b) structural-check / (c) test-free로 3-way triage
              → (c)는 Stage A 스킵 + RED artifact 없음, triage 근거만 기록해 Stage B로 직행
     Stage A: (a)/(b) task마다 sdd-skills:test-author-agent leaf를 동시 dispatch (테스트만)
              → 전원 완료 대기
...
**Stage A dispatch 직전 — 3-way triage (wave당 1회, RED 게이트 소유):** orchestrator는 wave 내 각 task를 세 갈래로 분류한다.
...
1. **review**: 한 메시지에서 두 reviewer를 **병렬 dispatch**한다 — `Agent(subagent_type="sdd-skills:implementation-review-agent")`(correctness)와 `Agent(subagent_type="sdd-skills:simplicity-review-agent")`(simplicity)에 각각 이 phase 범위의 변경 파일 전체 + 테스트 결과를 전달한다. phase에 (c) test-free task가 있으면 그 triage 근거도 dispatch 입력에 함께 전달한다(리뷰 불면제 — reviewer가 근거 타당성을 점검). 두 reviewer가 각자 severity별 finding을 반환한다.
