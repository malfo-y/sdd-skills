...
1. **회귀 1회**: 이번 변경 관련 표적 test/check + fast 회귀(무거운 test 제외)를 실행한다. **무거운 test가 포함된 전체 suite는 repo(`_sdd/env.md` 등)가 명시한 checkpoint이거나 사용자 확인을 받은 경우에만** 실행한다 — 확인 없이 강행하지 않고, 조용히 skip하지도 않는다(미실행분은 마감 요약에 보고). 무거운지 애매하면 사용자에게 묻는다. 이번 변경과 무관한 실패를 발견하면 사용자에게 보고한다 — 몰래 고치지 않는다.
