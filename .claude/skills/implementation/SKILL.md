...
이 스킬은 **orchestrator**다. 메인 루프에서 실행되어 fan-out이 가능하다. task-set을 확보하고(plan 파싱 또는 plan 없으면 경량 분해), dependency 기반으로 병렬 그룹을 파생해 **`implementation-agent` leaf를 task당 dispatch**하며, 통합·회귀·phase review·report를 소유한다. 각 task의 TDD(RED→GREEN→REFACTOR)는 leaf가 수행한다. 병렬화는 최적화 토글일 뿐 — 불가하면 동일 흐름으로 순차 실행한다.
...
- [ ] AC3: 그룹 내 task마다 `sdd-skills:implementation-agent` leaf를 dispatch(claude는 `sdd-skills:` prefix 필수)하고 leaf 입력 4종(task 필드 + Target Files + 환경/테스트 + 선행 보장)을 전달
...
복수 파일 존재 시 사용자에게 확인. plan에서 추출: Components, Phases, Tasks (Target Files·Dependencies 포함), Open Questions. Open Questions는 최선의 판단으로 해결하고, 판단 불가 항목은 리포트에 기록.
...
For each phase:
  ...
  2. For each group:
     a. 그룹 내 task마다 sdd-skills:implementation-agent leaf를 동시 dispatch
     b. 전원 완료 대기
...
병렬 불가(파일 충돌·dependency·Target Files 부재·no-plan 저확신)면 동일 leaf를 하나씩 순차 dispatch한다. 흐름은 동일하고 병렬성만 빠진다.
...
1. **review**: `Agent(subagent_type="sdd-skills:implementation-review-agent")`를 호출해 이 phase 범위의 변경 파일 전체 + 테스트 결과를 전달한다. reviewer가 severity별 finding을 반환한다.
2. **fix**: critical/high/medium finding이 있으면, finding을 **하나씩 순차** fix-task로 변환해 `Agent(subagent_type="sdd-skills:implementation-agent")` leaf를 재dispatch한다(finding 영향 파일 = 그 leaf의 Target Files). `implementation-agent`는 fix mode 별도 계약 없이 finding을 task로 받아 기존 TDD 계약으로 처리한다(I3 — leaf는 단일 task 실행자라 finding이 곧 task).
...
> **Role Pointer**: 이 스킬은 orchestrator다. 단일 task TDD를 수행하는 leaf는 `.claude/agents/implementation-agent.md`다. 더 이상 leaf와 동일 계약을 mirror하지 않는다 (orchestrator↔leaf 관계) — 변경 시 함께 수정할 동기화 의무 없음.
