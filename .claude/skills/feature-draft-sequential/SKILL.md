...
description: This skill should be used when the user asks to "feature draft sequential", "sequential feature draft", "legacy feature draft", "순차 기능 초안", "레거시 기능 계획", or explicitly wants a sequential feature-draft workflow without Target Files.
version: 1.0.0
---
...
This skill integrates the functionality of three skills: `spec-draft` + `spec-update-todo` + `implementation-plan-sequential`.
...
**Previous workflow**: `spec-draft` → `spec-update-todo` → `implementation-plan-sequential` (3 invocations, 3x tokens)
**This skill**: `feature-draft-sequential` (1 invocation, shared context)
...
- When you want to collapse the `spec-draft` → `spec-update-todo` → `implementation-plan-sequential` chain into one step
