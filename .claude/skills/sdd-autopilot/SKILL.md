...
4. **시작점/종료점 감지** (산출물 스캔):
   - 사용자 요청에서 시작/종료 힌트를 추출한다 ("구현부터", "리뷰까지만" 등)
   - `_sdd/` 하위 산출물을 스캔하여 기존 작업 결과를 확인한다:
     - `_sdd/drafts/feature_draft_*.md` → feature-draft 완료 여부
     - `_sdd/implementation/IMPLEMENTATION_PLAN.md` → implementation-plan 완료 여부
   - 기존 산출물과 현재 요청의 관련성을 판단한다:
     - 관련성 높음 → 해당 산출물을 활용하여 이후 스텝부터 파이프라인 구성
     - 관련성 불확실 → Step 2에서 사용자에게 "기존 산출물을 활용할까요?" 확인
     - 관련성 없음 → 무시하고 처음부터 시작
   - 종료점이 지정된 경우 → Step 5 오케스트레이터 생성 시 해당 스텝까지만 포함
   - **Review-Fix 사이클 필수 검증**: 파이프라인 범위에 review 단계가 포함되면, 반드시 implementation + review-fix loop이 함께 포함되어야 한다. "리뷰만" 요청하더라도 review → fix → re-review 사이클을 강제한다. 예: 사용자가 "리뷰만 해줘"라고 요청해도, review에서 이슈가 발견되면 fix → re-review까지 수행한다.

**Decision Gate 1 -> 2**:
```
request_parsed = 기능 설명이 추출됨
initial_direction = 초기 복잡도 예측 완료
pipeline_range = 시작점/종료점 감지 완료 (전체 or 부분)
...
규모에 맞는 맞춤형 오케스트레이터 파일을 생성하여 `.claude/skills/orchestrator_<topic>/`에 저장한다.
...
파이프라인 실행 중 각 에이전트 호출 전후에 Status 테이블을 `Edit`으로 업데이트한다:
...
.claude/skills/orchestrator_<topic>/SKILL.md → autopilot이 생성, Step 7에서 참조
