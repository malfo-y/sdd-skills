...
- **테스트 전략 판단 기준**: 인라인 디버깅 vs ralph-loop-init 자율 판단을 위한 힌트
...
d) **검증 수준**: 인라인 테스트? ralph-loop? (프로세스 특성 기반 자율 판단) review 포함?
...
| ralph-loop-init | 장시간 실행 환경, 반복 디버깅 프로세스 (ML, e2e, 빌드 등) |
...
Agent(
  ...
  prompt="다음 프로세스의 자동 디버깅 루프를 설정하세요.

  프로세스 설명: <프로세스 설명>
  실행 명령: <실행 명령>
  프로세스 특성: <예상 실행 시간, 필요 리소스 등>
  관련 파일: <구현된 파일 목록>"
)
