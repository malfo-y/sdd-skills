...
**Claude subagent_type**: `feature-draft-agent`
**Model**: `opus`
...
**Claude subagent_type**: `implementation-agent`
**Model**: `sonnet`
**입력 파일**:
...
**Leaf fan-out**: `implementation-agent`는 단일 task leaf다. autopilot은 이 step의 task를 dependency 기반 **병렬 dispatch 그룹**("dependency 없음 + Target Files disjoint → 병렬")으로 파생해 task당 `implementation-agent`를 dispatch한다(병렬 불가·저확신 시 순차). 이 병렬 dispatch 그룹은 아래 review-fix gate의 Checkpoint 리뷰 그룹과 다른 중첩 개념이다 (각각 "무엇을 동시에 dispatch하나" vs "언제 gate를 닫나"). progress/report는 autopilot이 canonical 경로로 소유한다.
...
- `agent_mapping`: `review = implementation-review-agent (model: opus)`, `fix = implementation-agent (model: sonnet)`, `re-review = implementation-review-agent (model: opus)`
...
- `implementation-review` 결과에 `critical/high/medium`이 하나라도 있으면 autopilot은 fix를 수행한다 — finding을 fix-task로 보고 **finding 하나씩 순차로** `implementation-agent` leaf를 dispatch한다(finding 영향 파일 = leaf Target Files). 초기 구현의 병렬 dispatch 그룹과 달리 fix는 순차다(별도 fix 분해 기계장치 없음).
...
**Claude subagent_type**: `spec-update-done-agent`
**Model**: `sonnet`
**입력 파일**:
- `_sdd/spec/main.md`
- `_sdd/drafts/2026-04-10_feature_draft_jwt_auth.md`
...
**Claude subagent_type**: `spec-update-todo-agent`
**Model**: `sonnet`
**입력 파일**:
- `_sdd/spec/main.md`
...
**Claude subagent_type**: `implementation-plan-agent`
**Model**: `opus`
**입력 파일**:
- `_sdd/drafts/2026-04-10_feature_draft_payment_system.md`
- `_sdd/spec/main.md`
...
**Leaf fan-out (phase 내부)**: `implementation-agent`는 단일 task leaf다. autopilot은 각 phase 안에서 task를 dependency 기반 **병렬 dispatch 그룹**("dependency 없음 + Target Files disjoint → 병렬")으로 파생해 task당 `implementation-agent`를 dispatch한다(병렬 불가·저확신 시 순차). 이 **병렬 dispatch 그룹(phase 내부)**은 아래 `group boundary`의 **Checkpoint 리뷰 그룹(phase 경계)**과 다른 중첩 개념이다 — 전자는 "무엇을 동시에 dispatch하나", 후자는 "언제 review-fix gate를 닫나". progress/report는 autopilot이 canonical 경로로 소유한다.
...
- `agent_mapping`:
  - `review = implementation-review-agent (model: opus)`
  - `fix = implementation-agent (model: sonnet)`
  - `re-review = implementation-review-agent (model: opus)`
...
- autopilot은 각 Checkpoint phase 직후 해당 group 전체 범위로 `implementation-review` subagent를 즉시 호출한다.
...
- `agent`: `implementation-review (model: opus)`
- `implementation-review`를 1회 더 실행해 cross-group regressions를 점검한다.
...
- `retries`: feature-draft/spec-update-*는 1회, implementation-plan은 1회, 각 group review-fix gate는 group당 최대 3회
