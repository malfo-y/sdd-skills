...
- `spec-update-done`은 implementation-scoped review-fix와 required inline validation이 끝난 뒤 최종 단계로만 배치한다.
...
- 마지막 phase gate가 닫힌 뒤 `final integration review`를 수행하고, 그 이후에만 `spec-update-done`을 실행한다.
