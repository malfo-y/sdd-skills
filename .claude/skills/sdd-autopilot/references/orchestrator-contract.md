...
- `Part 2`는 flat task-set(각 task 정의 + `Target Files`; sweep 검증 task는 `없음 (read-only 검증)` 허용)을 산출한다. dependency edge·phase 분할·실행 순서·Checkpoint는 포함하지 않는다(그것은 `task-ordering` 소관)
