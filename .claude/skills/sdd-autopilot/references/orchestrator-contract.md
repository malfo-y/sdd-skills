...
각 에이전트 step은 아래 4개 필드를 반드시 가진다.
...
아래는 오케스트레이터에 이름은 나타날 수 있지만, 새 에이전트를 뜻하지 않는다.
...
- `ralph-loop-init`은 설정 생성까지만이 아니라 실제 실행 검증까지 포함해야 한다.
...
> 자율 판단 노트: 방식 선택은 프로세스 특성(실행 시간, 반복 검증 필요성, 환경 격리 필요성)에 기반한다. 인라인은 수분 이내 완료되는 경우, ralph-loop-init은 장시간/반복 사이클이 필요한 경우 (타임아웃 제약 없음, background 실행).

필수 규칙:

- 테스트/검증 단계는 **건너뛸 수 없다**. 실행 불가 시 사유와 수동 검증 방법을 보고서에 명시해야 한다.
- inline 테스트: 실제 테스트 실행 여부와 통과 결과를 검증한다.
- `ralph-loop-init`: setup 검증과 run 결과 검증을 모두 가진다.
...
- `_sdd/drafts/feature_draft_<topic>.md` -> `implementation` 또는 `implementation-plan`
- `_sdd/implementation/implementation_plan.md` -> `implementation`
- `.claude/skills/orchestrator_<topic>/SKILL.md` -> autopilot 실행 입력
