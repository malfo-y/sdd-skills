...
| write-phased | 장문 문서/코드 작성 | 다른 에이전트의 하위 도구로 중첩 호출 |
| git | 커밋/브랜치 자동화 | |
...
| spec-update-done | `spec-update-done` |
| spec-update-todo | `spec-update-todo` |
| spec-review | `spec-review` |
| ralph-loop-init | `ralph-loop-init` |

보조 에이전트: `Explore` (코드베이스 탐색), `write-phased` (장문 출력 시 중첩 호출), `general-purpose` (범용 리서치)
...
- **ML/장시간 테스트**: ralph-loop-init을 테스트 단계에 배치
...
| 테스트 수십분+ / GPU/대용량 / 외부 CI 필요 | **ralph-loop-init** |

| 프로젝트 유형 | 기본 전략 |
|-------------|----------|
| 웹 백엔드, 프론트엔드, CLI | 인라인 디버깅 |
| ML/DL, 모바일(빌드), 인프라/DevOps | ralph-loop-init |
