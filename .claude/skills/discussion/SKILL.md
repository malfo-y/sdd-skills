...
2. **토론 종료 옵션 항상 포함**: Step 3의 매 질문에 "토론 종료 / 정리해줘" 옵션을 반드시 포함한다.
3. **AskUserQuestion 도구 필수**: Step 3 토론 루프에서 사용자에게 질문할 때 반드시 `AskUserQuestion` 도구를 사용한다.
4. **in-scope 미결 종료 차단**: 카테고리가 `out-of-scope`나 `deferred-deliberately`가 아닌 미결(이하 "in-scope `open_questions`")이 남은 상태로는 Gate 3→4 외 어떤 경로(수렴 신호, stagnation fallback 등)로도 토론을 종료하지 않는다.
...
매 라운드는 다음 흐름을 반복한다: 3.2 전략으로 질문을 하나 고르고, `AskUserQuestion`으로 제시하며(옵션 2-3개 + "토론 종료"), 응답을 분석해 내부 상태와 coverage를 갱신한다. 근거가 필요하면 리서치를 디스패치하고(3.3), 리서치·답변이 기존 결정의 전제와 모순되면 결정을 재방문하며(3.3.1), 수렴 신호가 감지되면 종료를 권유한다(3.5). 사용자가 "토론 종료"를 선택하거나 "정리해줘"를 입력하면 Gate 3→4로 이동한다.
...
- **언어**: 토론에서 사용된 언어로 작성
- **섹션 구성**: `references/summary-template.md` 코드블록을 따른다 (verbatim 복사가 전 섹션 포함을 보장하므로 별도 열거 없음)
