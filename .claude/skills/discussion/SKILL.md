...
| Any | Pre-draft | 방향/요구사항 불명확 시 논의 후 feature-draft-lite 연계 |
...
```
내부 상태 추적:
  key_points, decisions, open_questions, action_items, rejected_alternatives,
  conversation_log, initial_context
  round = 0
...
LOOP:
  ...
  5. 리서치/답변이 기존 결정의 전제와 모순되면 → 결정 재방문 (아래 3.3.1)
  6. 수렴 신호 감지 시 → 종료 권유 (아래 3.5)
...
2. 재방문은 **결정당 1회**로 제한한다 (같은 결정을 반복 재방문하며 표류하지 않는다).
...
- **언어**: 토론에서 사용된 언어로 작성
- **필수 포함**: 토론 배경 및 초기 콘텍스트, 핵심 논점, 결정 사항(근거 유형 표기 포함), 미결 질문, 실행 항목, 리서치 결과 요약, 토론 흐름
...
### 기각한 대안
<!-- 조건부: rejected_alternatives 1건 이상일 때만. 없으면 섹션 생략 -->
- [대안]: [기각 사유]
...
### 후속 핸드오프 (Handoff)
<!-- 조건부: 실행 항목이 후속 스킬·구현으로 이어질 때만. 없으면 섹션 생략 -->
...
토론 결과를 후속 스킬(spec-create, feature-draft-lite)에 활용하려면 사용자가 요약 파일을 참조하여 별도 실행한다. 후속 작업이 예정된 토론이면 요약의 "후속 핸드오프" 섹션(목표/변경 금지 제약/검증/중단 조건)이 후속 스킬의 입력 경계가 되고, 결정 테이블의 `미검증 가정` 표기는 후속 작업이 먼저 검증해야 할 전제 목록이 된다.
