...
7. **in-scope 미결 종료 차단**: 카테고리가 `out-of-scope`나 `deferred-deliberately`가 아닌 미결(이하 "in-scope `open_questions`")이 남은 상태로는 Gate 3→4 외 어떤 경로(수렴 신호, stagnation fallback 등)로도 토론을 종료하지 않는다.
...
4. **Scope atomicity 점검**: 토픽이 독립적 서브시스템 여러 개의 묶음인지 감지한다.
   - 감지 예: "플랫폼/시스템 전체를 한 번에 논의", 3개 이상의 독립 영역(예: 인증 + 결제 + 알림 + 분석) 나열
   - 감지 시: 서브프로젝트로 분해를 제안하고, 각 조각의 관계와 순서를 짧게 정리한 뒤, 사용자와 함께 첫 번째 서브프로젝트를 선택해 그것만 토론 대상으로 좁힌다.
   - 단일 단위로 토론 가능하면 이 단계를 건너뛴다.
...
분석 페이즈에서는 최소 1회 **비판적 질문**으로 약점이나 숨은 가정을 검증한다. 3.2.1의 Alternatives Initiation이 완료된 뒤 수행한다.
...
1. 모든 미결 질문에 대해 한 번의 `AskUserQuestion` (multiSelect 가능, 질문당 옵션 1세트)로 카테고리를 일괄 부여한다. 카테고리는 다음 4종 고정:
