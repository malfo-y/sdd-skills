...
토론 결과를 후속 스킬(spec-create, feature-draft, implementation-plan)에 활용하려면 사용자가 요약 파일을 참조하여 별도 실행한다. 후속 작업이 예정된 토론이면 요약의 "후속 핸드오프" 섹션(목표/변경 금지 제약/검증/중단 조건)이 후속 스킬의 입력 경계가 되고, 결정 테이블의 `미검증 가정` 표기는 후속 작업이 먼저 검증해야 할 전제 목록이 된다.
