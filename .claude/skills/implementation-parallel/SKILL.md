...
description: Use this skill when the user wants to execute an implementation plan with parallel sub-agents, or says "implement parallel", "parallel implementation", "병렬 구현", "병렬로 구현", "implement the plan in parallel". Requires a plan with Target Files for optimal parallel execution.
version: 1.0.0
---
...
This skill extends `implementation` with parallel execution capability:

| Aspect | `implementation` | `implementation-parallel` (this) |
|--------|-----------------|----------------------------------|
...
```
spec → feature-draft-parallel → implementation-parallel (this) → spec-update-done
```

| Step | Skill | Purpose |
|------|-------|---------|
| 1 | spec-create | Create the initial spec document |
| 2 | feature-draft-parallel | Draft feature spec patch + implementation plan (with Target Files) |
| **3** | **implementation-parallel** | Execute the plan with parallel sub-agents (TDD) |
| 4 | spec-update-done | Sync spec with actual code |

> Also compatible with `feature-draft` or `implementation-plan` output (falls back to sequential when Target Files missing).
...
1. **Locate the Implementation Plan**: Check for plan at:
   ...
   - `<project_root>/_sdd/drafts/feature_draft_<feature_name>.md` (produced by `feature-draft` or `feature-draft-parallel` skill; use Part 2: 구현 계획 as the implementation plan)
...
2. **Verify Plan Exists**: If no plan is found, suggest using the `implementation-plan-parallel` or `feature-draft-parallel` skill first.
...
This is the **key step** that differentiates this skill from `implementation`.
...
When tasks cannot be parallelized (conflicts or missing Target Files), execute them sequentially using the same TDD approach as the original `implementation` skill:
...
After completing all tasks in a phase, run a lightweight quality review (same as `implementation` skill).
...
### General (same as `implementation`)
...
- **implementation-plan-parallel**: Creates plans with Target Files that this skill consumes
- **feature-draft-parallel**: Also produces plans with Target Files (Part 2: 구현 계획)
- **implementation-plan / feature-draft**: Plans without Target Files → sequential fallback
...
1. Acquire implementation plan (from `feature-draft-parallel`, `implementation-plan-parallel`, or existing plan files)
