...
`implementation-parallel` 스킬의 병렬 실행 알고리즘, 충돌 감지 규칙, sub-agent 프로토콜을 정의합니다.
...
- `feature-draft-parallel` 또는 `implementation-plan-parallel`에서 Target Files를 잘 설계하면 `implementation-parallel`의 병렬화가 자연스럽게 최적화됨
