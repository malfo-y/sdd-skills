...
#### 서술형 동작 시나리오
> 사용자 관점에서 주요 흐름이 어떻게 동작하는지 서술한다.
> 다이어그램만으로 전달하기 어려운 맥락, 조건 분기, 에러 케이스를 자연어로 보충한다.

- **시나리오 1**: (예: 사용자가 주문을 생성하면 ...)
- **시나리오 2**: (예: 배치 작업이 매일 자정에 ...)
...
##### 동작 개요
> 이 컴포넌트가 어떻게 동작하는지 사용자/호출자 관점에서 서술한다.
> 코드 구현을 복사하지 말고, 핵심 동작 흐름과 조건을 2-3문단으로 요약한다.

##### 설계 의도
> 왜 이런 구조를 택했는지, 어떤 트레이드오프가 있는지 간결히 기록한다.
