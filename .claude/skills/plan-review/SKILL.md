...
- 서로 독립인 Read/Grep은 **한 메시지에 배칭**한다 — 앞 결과를 봐야 대상이 정해지는 호출만 다음 턴이다.
- `Grep`으로 좌표를 먼저 잡고 관련 구간만 선택적으로 `Read`한다 — 파일 전문 읽기는 판정에 전문이 필요할 때만.
...
- draft가 주장하는 외부 사실 — Target Files 경로, content anchor, `[C]` 생성 사유의 전제, 명시 인용한 spec 절 — 은 판정 전에 `Grep`/`Read`로 전부 실재를 대조한다. 대조 없이 내리는 근거 부족 판정은 없다. 대조를 마친 뒤에도 근거가 부족하면 읽기를 더 확장하지 않고 그 smell의 finding을 만들지 않는다.
