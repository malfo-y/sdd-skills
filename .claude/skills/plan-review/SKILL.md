...
이 스킬은 review-only orchestrator다. 사용자의 plan-review 요청을 `sdd-skills:plan-review-agent` **2회 병렬 dispatch**(실측 렌즈 ∥ 판단 렌즈)로 위임하고 두 **경량 반환**을 병합해 사용자에게 전달한다. 전체 리뷰 프로세스·6-smell rubric·severity·반환 형식·렌즈 정의는 agent가 단일 소스로 보유한다. 리뷰는 단일 패스이며(렌즈 2개는 한 패스의 병렬 분해이지 loop가 아니다) 리포트 파일을 만들지 않는다. agent는 read-only leaf(파일을 쓰지 않음)라 동시 dispatch가 안전하다.
