...
## SDD Quality Improvements

- **탐색 가능성**: Component Index와 Repository Map으로 기능 위치를 즉시 찾을 수 있게 됨
- **변경 지향성**: Change Recipes가 주요 컴포넌트에 포함되어 변경 시작점이 명확해짐
- **계약 가시화**: Cross-Cutting Invariants 섹션으로 깨지면 안 되는 규칙이 드러남
- **실제 경로 연결**: 모든 컴포넌트에 실제 파일 경로와 핵심 심볼이 명시됨
