...
현재 콘텍스트에서 먼저 변환 대상 skeleton/섹션 헤더를 기록한 뒤, 같은 흐름에서 Edit으로 내용을 채운다.
- 독립 섹션 2개+ → 병렬 Agent dispatch 가능
- 의존 섹션 → 순서대로 Edit
- 완료 후 TODO/Phase 마커 제거

Step 4.3 변환 규칙 + Gap/코드 분석 결과 + 기존 스펙 내용을 skeleton과 fill에 함께 반영한다.

- **단일 파일**: `[target path]`에 whitepaper skeleton을 직접 작성한 뒤 변환 규칙과 분석 결과를 채운다
