...
Agent(
  ...
  파일 경로: [target spec path - 기존 스펙 파일 경로]
...
> **먼저 `references/template-compact.md`를 Read로 읽는다.** 이 템플릿에 §1-§8 섹션 구조, Writing Rules(코드 발췌/인라인 citation/What-Why-How 트라이어드), 그리고 Modular Spec Guide가 정의되어 있다.
> **그다음 `references/template-full.md`를 Read로 읽는다.** 이 템플릿에 전체 섹션 구조와 코드 citation 형식의 상세 예시가 정의되어 있다.
