...
**Tools**: `Read`, `Agent` (sdd-skills:write-skeleton)

각 스펙 파일의 번역을 `sdd-skills:write-skeleton` 서브에이전트에 위임한다. 반환값이 SKELETON_ONLY이면 Sections Remaining 목록을 보고 Edit으로 채운다.
