...
- `simplicity-review-agent` → `_sdd/implementation/<YYYY-MM-DD>_simplicity_review_<slug>.md` + severity별 finding 요약. dispatch message에 **Medium+ finding당 위치(`file:line`)·현재 형태·제안된 더 단순한 형태, Low는 위치 포함 한 문장**으로 반환하도록 명시한다 (agent 리포트 §1이 이미 이 필드를 담는다).
