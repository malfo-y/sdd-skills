...
2. evidence 유무로 분기한다:
   ...
   - **구현 후(implemented sync)**: **선고정** — digest에 delta 목록·분류 근거·신규 spec **버전 번호**·결정 제목을 고정한다(버전이 대화에서 미상이면 `main.md` 헤더 버전만 targeted grep **1회** 허용). 그 뒤 **한 메시지에서 두 표면 묶음을 병렬 dispatch한다**:
