...
훅 설치는 `AGENTS.md` 하네스 생성/병합과 동일 조건이다. 하네스를 설치하면 Claude Code와 Codex 등록을 함께 설치하며 별도 opt-in으로 다루지 않는다.

1. 호출 중인 skill package의 `references/hook-installation.md`를 **Read**한다.
2. 그 reference의 `Trigger and Inputs`부터 `Verification and Report`까지 전부 적용한다.
...
Structure Decision의 single-file/multi-file 결과와 content template 선택은 서로 독립이다.
...
spec 작성 직전에 선택한 runtime-local asset만 **Read**한다. 선택한 template 파일의 전체 skeleton을 verbatim 복사해 heading·field order를 보존하고, source evidence로 placeholder를 치환하며 evidence가 없는 optional block은 제거한다. reference 내용을 기억이나 이 SKILL 본문으로 대체하지 않는다.
