...
**언제 ralph가 아닌가**: 세션 하나 안에서 닫히는 반복 작업에 ralph를 쓰면 격리 환경 준비와 승인 우회 비용만 치른다. 단발 디버깅은 `investigate`, 조건 충족까지의 세션 내 반복은 네이티브 `/goal`(조건·하네스 셋업은 `goal-init`)을 쓴다.
...
IMPORTANT: Do NOT invoke any skills, modes, or slash commands. Do NOT delegate to other agents. You are inside a standalone automation loop, not an interactive session. `_sdd/`는 읽기 전용이다 — 수정 금지.
...
아래 템플릿 구조를 기준으로 `ralph/run.sh`를 생성하고 `chmod +x ralph/run.sh`를 실행한다. Step 5에서 중간 phase 이름을 커스터마이즈했으면 (`SETUP`·`DONE`은 유지) `VALID_PHASES`를 PROMPT.md에 정의한 phase 집합과 동일하게 맞추고(불일치 시 `validate_state_file`이 정상 상태를 reject한다), `ADJUST_PHASE`를 그 집합의 중간 진단·수정 phase 이름으로 맞춘다.
...
if ! [[ "${MAX_RUNTIME_MINUTES}" =~ ^[0-9]+$ ]] || [ "${MAX_RUNTIME_MINUTES}" -lt 1 ]; then
  echo "[ralph] Invalid MAX_RUNTIME_MINUTES='${MAX_RUNTIME_MINUTES}'." >&2
  exit 1
fi

if ! [[ "${MAX_ITERATIONS}" =~ ^[0-9]+$ ]] || [ "${MAX_ITERATIONS}" -lt 1 ]; then
  echo "[ralph] Invalid MAX_ITERATIONS='${MAX_ITERATIONS}'." >&2
  exit 1
fi
...
3. `ralph/CHECKS.md`의 각 항목을 검증한다. 실패 항목은 위 재시도 상한 안에서 수정·재검증한다.
