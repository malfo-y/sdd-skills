...
active skill catalog에서 설치된 `goal-init`을 이름으로 선택해 `preset=sdd` 입력으로 수행한다. workspace-relative source-tree 경로를 가정하지 않는다. 목표·권한·조건 self-check를 충족하고 4파일을 생성할 때까지 산출물 정의를 이 스킬에서 재구현하지 않는다.
