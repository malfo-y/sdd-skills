...
- 참조 정보
- Strategic Code Map 또는 manual code-map appendix
...
`Strategic Code Map`은 agentic coding을 위한 optional navigation surface다. 전체 파일 목록이나 component inventory가 아니라, 사람이나 LLM이 코드를 수정하기 전에 먼저 확인해야 하는 핵심 좌표만 담는다.

허용되는 정보는 아래처럼 탐색 판단을 바꾸는 항목이다.
...
- 작은 repo: `_sdd/spec/main.md` 안의 짧은 appendix
- 중간 이상 repo: `_sdd/spec/components.md` 또는 `_sdd/spec/code-map.md` 같은 supporting surface
- 특정 변경의 target file / validation detail: temporary spec
- 특정 기능의 상세 구현/리뷰 설명: guide

primary navigation axis는 하나만 선택한다. app/service/product repo는 feature/domain/change-path 중심, library/framework/compiler repo는 module/layer 중심, workflow/tooling repo는 entrypoint/workflow 중심이 기본값이다. secondary axis는 별도 동등 문서 체계로 만들지 않고 cross-reference로만 둔다.

`Strategic Code Map`은 stale 가능성이 있는 힌트다. feature planning과 implementation은 반드시 현재 코드를 재확인한 뒤 `Target Files`를 확정한다.
...
temporary spec(= feature draft)은 global spec의 축약본이 아니라 변경 실행 청사진이다.

feature-level contract, AC, target files는 여기에 둔다.

canonical 구조 (`feature-draft` 스킬의 Required Output이 단일 소스):
...
3. `Decisions and Assumptions`(조건부) — 결과 방향·Target Files 선택·task boundary를 바꿀 수 있는 중요 결정만 `Decision / Assumption`·`Evidence`·`Rejected alternatives`·`Confidence`·`User confirmation needed` 5필드로 기록한다. 중요 결정이 없으면 생략한다.
4. `Propagation Surfaces`(조건부)
...
5. `Part 2: Tasks` — task마다 의도 1줄, `Contracts`(새 약속이 있을 때만), falsifiable AC, 실측 Target Files. census형 sweep이면 마지막에 read-only 검증 task.
6. `Open Questions`

temporary spec의 목적은 이번 변경이 무엇을 바꾸고, 어디를 건드리며, 무엇으로 검증할지를 직접 다루는 것이다.

검증의 단일 정의 지점은 실행 단위에 병치된다 — **각 task의 AC**가 그 지점이다. 다음 rubric의 닻은 여기이며, planning/implementation/review 스킬이 이를 구현한다.

- **AC falsifiability**: 모든 AC는 충족/미충족이 증거로 닫혀야 한다. "미충족"을 말할 관찰/증거가 없는 AC는 두지 않는다.
- **평가방법 2등급**: 기준은 "측정 가능"이 아니라 "증거 기반 판정 가능"이다. 1등급(정량 측정형: test/check의 재현 가능한 출력)과 2등급(정성 rubric 판정형: 명시 기준+리뷰어 판정, 인용 근거)을 허용하되, 두 등급 모두 (a) 이진 판정으로 닫히고 (b) 외부 증거에 묶이고 (c) 제3자가 반박 가능해야 한다. 품질·가독성은 2등급으로 받는다.
- **증거 기반 결과**: 실행 후 모든 AC의 verdict는 증거(재현 가능한 출력 또는 인용 근거)에 묶여 기록된다 — 구현의 AC→증거 테이블과 리뷰의 verification ledger가 그 기록처다. 증거 없는 "충족"은 인정하지 않는다.

정보 배치는 "상세는 task"를 따른다:

- 계약의 정밀 서술(인터페이스/불변식 실체)은 각 task의 **`Contracts` 필드**가 단일 소유한다 — downstream 구현이 "발명 없이 실행"할 계약의 앵커.
- task-국소 탐색 근거는 해당 task의 Target Files 주석이 소유한다.
- 여러 task에 걸친 **sweep형 검증**(전 task 완료 후 전역 1회 실행 — census류)은 개별 task에 쪼개지 않고 마지막 read-only 검증 task(Target Files `없음 (read-only 검증)`)로 승격한다.
- (legacy) 구형 full draft의 7섹션 구조·coverage index·`V*` 매핑·`Touchpoints` census는 기록물 형식이다 — 새 draft에 요구하지 않으며, 기록물을 읽을 때만 그 형식 기준을 적용한다.
...
| Strategic Code Map | `main.md` appendix 또는 supporting surface | agentic coding의 시작 좌표. exhaustive inventory가 아니라 entrypoint / hotspot / validation surface 중심 |
| code inventory | code 또는 generated tooling | 전수 목록은 drift 비용이 커서 global spec의 지속 본문이 아님 |
...
> 규모 판정: [판정 근거 1줄 — 적격 | 분할 필요 — 분할 계획 포함]
...
중요 결정이 있을 때만 5필드로 작성하고, 없으면 섹션 생략
...
# Part 2: Tasks
### Task N: [title]
Contracts (있을 때만) / Acceptance Criteria / Target Files
...
- `spec-create`: 공통 코어 4축을 만족하는 얇은 global spec을 만든다. 기본값은 `_sdd/spec/main.md` 단일 파일이고, 분할은 navigation + surface fit 근거가 있을 때만 허용한다. 코드베이스가 있으면 primary navigation axis를 하나 선택하고, 유용할 때만 compact Strategic Code Map을 appendix 또는 supporting surface에 둔다.
- `spec-review`: spec type에 맞는 rubric을 구분 적용한다. global spec의 feature-level 오염은 기본적으로 `Quality`이며, 문서 타입 혼동이나 잘못된 repo-wide truth처럼 판단을 직접 오도할 때만 `Critical`로 올린다. 모든 finding은 spec/code/doc evidence를 가져야 하고, 근거가 약하면 `UNTESTED`로 남긴다. Strategic Code Map 부재는 기본 defect가 아니지만, non-trivial repo에서 다음 탐색 surface가 없으면 improvement로 제안할 수 있다.
- `spec-summary`: `_sdd/spec/summary.md`를 reader-facing whitepaper로 작성한다. 문제, 배경/동기, 대안 대비 선택 이유, 핵심 설계, 코드 근거, 사용/기대 결과, 추가 읽을 surface를 함께 설명하며, status memo가 아니라 기술 whitepaper처럼 읽혀야 한다. Strategic Code Map은 code grounding 입력으로 사용할 수 있지만 summary가 map을 대체하지 않는다.
- `spec-rewrite`: 공통 코어 4축을 더 잘 드러내도록 구조를 재정리한다. pruning 과정에서 rationale, citation, code excerpt header를 잃지 않고, migration history나 실행 로그성 설명은 body 대신 `decision_log` 또는 rewrite report로 내린다. navigation-critical hint는 삭제하지 말고 compact Strategic Code Map 또는 supporting surface로 보존한다.
- `spec-upgrade`: legacy spec을 current model로 마이그레이션한다. 다만 구조 재설계, 대규모 재분할, 역할 재배치가 핵심이면 upgrade로 덮지 않고 `spec-rewrite`로 분기한다. legacy inventory는 decision-bearing truth, navigation-critical hint, stale/exhaustive detail로 분류한다.
