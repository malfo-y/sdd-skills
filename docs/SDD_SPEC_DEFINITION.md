...
- **증거 기반 결과**: 실행 후 모든 AC의 verdict는 증거(재현 가능한 출력 또는 인용 근거)에 묶여 기록된다 — 구현의 AC→증거 테이블(implementation ledger 파일 `_sdd/implementation/*_implementation_ledger_*.md`에 완성)과 리뷰의 verification ledger가 그 기록처다. 증거 없는 "충족"은 인정하지 않는다.
