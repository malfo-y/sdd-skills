...
| 중규모 | 4-10개 | 1-3개 | 기존 섹션 패치 | feature-draft → implementation 또는 expanded path 시 feature-draft → implementation-plan → phase-iterative implementation loop |
| 대규모 | 10개+ | 3개+ | 신규 섹션 추가 가능 | feature-draft → (조건부 spec-sync) → implementation-plan → phase-iterative implementation loop → final integration review |
...
- `spec-sync`는 planned persistent global alignment가 실제로 필요할 때만 조건부로 추가합니다(구현 전 planned alignment / 구현 후 evidence sync를 같은 스킬이 evidence에 맞춰 처리).
- medium이라도 multi-phase plan이 생성되면 기본 review-fix scope는 `per-phase`로 승격되고, 마지막에 `final integration review`를 한 번 더 수행합니다.
- review가 포함된 small/medium/large 모든 경로에서 `implementation`과 `implementation-review`는 항상 에이전트 실행 단위로 유지됩니다. 부모 sdd-autopilot이 로컬 구현/로컬 리뷰로 대체하지 않습니다.
...
오케스트레이터 생성 단계에서 실제로 materialize되는 산출물은 `_sdd/pipeline/orchestrators/orchestrator_<topic>.md` 하나뿐입니다. `_sdd/drafts/*`, `_sdd/implementation/*`, 코드/테스트 출력은 해당 step이 실행되기 전까지 planned output으로만 존재합니다. 아래 가이드의 산출물/체크리스트는 이 boundary를 전제로 읽습니다.
...
| 5 | **오케스트레이터 생성** | 규모에 맞는 맞춤형 파이프라인 계획을 `_sdd/pipeline/orchestrators/orchestrator_<topic>.md`에 저장. 이 단계는 오케스트레이터 파일만 생성하고 downstream 산출물은 planned output으로만 선언 |
| 6 | **사용자 확인** | 파이프라인 요약 제시, 수정/승인/취소 선택 |
| 7 | **자율 실행** | 승인된 파이프라인을 에이전트 순차 호출로 실행. implementation-plan 결과를 소비하는 expanded path면 `Execution Mode: phase-iterative`와 `Phase Source`를 따라 per-phase `implementation -> review -> fix -> validation` gate와 final integration review까지 집행 |
...
feature-draft, implementation-plan, spec-sync를 생략합니다. 단일 함수/클래스 수정, 버그 수정, UI 미세 조정 등이 해당됩니다.

기본 small path에는 review가 포함되지 않을 수 있습니다. 다만 review를 포함하는 small path라면 `implementation agent -> implementation-review agent -> implementation agent (fix) -> implementation-review agent` 루프를 사용하며, 부모 autopilot이 직접 구현/리뷰하지 않습니다.
...
→ global review-fix loop (max 3회) → 인라인 테스트 → spec-sync agent
```
...
→ final integration review → 인라인 테스트 → spec-sync agent
```
...
- generated orchestrator는 downstream `implementation` step에 `Execution Mode: phase-iterative`와 `Phase Source`를 기록하고, phase count/boundary는 오케스트레이터 생성 시점이 아니라 runtime에 `implementation-plan` output을 읽어 해석한다
...
```text
feature-draft agent → (조건부) spec-sync agent → implementation-plan agent
→ phase-iterative implementation loop
→ Phase 1: implementation → review-fix → validation
→ Phase 2..N: implementation → review-fix → validation
→ final integration review
→ 테스트 (인라인 or ralph-loop-init) → spec-sync agent
→ spec-review agent (선택)
```

아키텍처 레벨 변경, 시스템 전체에 영향을 미치는 대규모 기능, E2E 테스트가 필요한 작업이 해당됩니다. `spec-sync`는 구현 전 planned persistent global change가 실제로 필요할 때만 추가합니다.

이 expanded path에서도 phase 수와 경계는 Step 5의 오케스트레이터 생성 시점에 미리 materialize하지 않고, `implementation-plan` 실행 결과를 `Phase Source`로 읽어 runtime에 확정합니다.
...
[sdd-autopilot] Step 4/4: spec-sync 완료 -- _sdd/spec/main.md 업데이트
...
[sdd-autopilot] Step 2/5: spec-sync 완료 -- 스펙에 결제 시스템 섹션 추가
...
[sdd-autopilot] Step 5/5: spec-sync 완료
...
sdd-autopilot은 파이프라인 실행 중 다음 파일들을 생성합니다. Step 5 generation boundary는 위에서 설명한 대로 유지되며, 아래 경로들은 각 owning step이 실행될 때 materialize됩니다.
...
Codex에서는 이 오케스트레이터가 `.codex/agents/`의 custom agents를 직접 spawn합니다. 승인 전에는 `_sdd/env.md`와 현재 Codex 런타임의 custom agent 사용 가능성, 테스트/실행 리소스 갭을 확인합니다.
...
| spec-sync | `_sdd/spec/main.md` (업데이트) | 구현 전 계획된 기능 사전 반영 / 구현 후 evidence 동기화를 evidence에 맞춰 처리 |
| implementation-plan | `_sdd/implementation/<YYYY-MM-DD>_implementation_plan_<topic>.md` | 상세 구현 계획 + 나중에 `Phase Source`로 소비될 phase metadata |
...
- **비핵심 단계** (`spec-sync`, `spec-review`, `ralph-loop-init`) 실패: 건너뛰거나 대체 전략을 사용하고, 로그에 실패 사유와 수동 후속 조치를 기록합니다.
...
이 시점에는 downstream 산출물이 아직 없을 수 있습니다. 자세한 materialization boundary는 위 설명을 기준으로 읽으면 됩니다.
...
사용할 수 있습니다. 단, sdd-autopilot은 `_sdd/spec/`의 존재 여부에 따라 파이프라인을 조정합니다. 스펙이 없으면 `spec-sync` 단계가 생략될 수 있습니다. 스펙 기반 워크플로우의 이점을 최대한 활용하려면 먼저 `/spec-create`로 스펙을 생성하는 것을 권장합니다.
...
> 이 섹션은 Codex 런타임에서 `sdd-autopilot` 계약을 확인하는 운영 체크리스트다. wrapper -> custom agent -> inline 2-phase writing -> autopilot dry-run 경로를 점검할 때 사용한다.
...
- 확인:
  - 긴 산출물이 skeleton -> fill 또는 동등한 2-phase 작성 전략으로 정리되는가
  - helper agent nesting이나 특정 전역 agent depth 설정을 전제로 하지 않는가
...
- generated orchestrator 생성 시점에 `_sdd/drafts/*`, `_sdd/implementation/*`, `_sdd/pipeline/report_*`가 미리 materialize되지 않는다
- `_sdd/pipeline/log_*.md`가 생성/갱신된다
- `_sdd/pipeline/report_*.md`가 생성된다
- multi-phase plan이면 downstream `implementation` step에 `Execution Mode: phase-iterative`와 `Phase Source`가 기록되고, carry-over policy와 final integration review가 함께 오케스트레이터에 남는다
- review가 포함된 경로라면 규모와 무관하게 `implementation`/`implementation-review`가 custom agent 이름으로만 매핑된다

### 8.5 Pre-flight Check

- `_sdd/env.md`가 존재하는가
- 필요한 `.codex/agents/*.toml` custom agent가 존재하는가
- 현재 런타임에서 custom agent dispatch와 병렬 fan-out을 수행할 수 있는가
- 사용자 전역 `~/.codex/config.toml` 값을 파이프라인 전제로 삼거나 수정하지 않는가
...
- 확인:
  ...
  - helper agent nesting이나 특정 전역 agent depth 설정 없이 완료되는가
...
- 확인:
  - 승인 전 pre-flight에서 `_sdd/env.md`, custom agent 사용 가능성, 테스트/리소스 갭을 요약하는가
  ...
  - orchestrator generation 단계에서 downstream artifact를 미리 만들지 않는가
  - `_sdd/pipeline/log_<topic>_<timestamp>.md`가 생성/갱신되는가
  - `_sdd/pipeline/report_<topic>_<timestamp>.md`가 생성되는가
  - multi-phase plan이면 `Execution Mode: phase-iterative`, `Phase Source`, phase gate, final integration review가 실제로 연결되는가
...
| `/spec-sync` | `/spec-sync` | 구현 전 계획 반영 / 구현 후 스펙 동기화 (evidence에 맞춰 적응) |
