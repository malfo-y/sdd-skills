...
| 5 | **오케스트레이터 생성** | 규모에 맞는 맞춤형 파이프라인 계획을 활성 스킬 디렉토리(`.claude/skills/orchestrator_<topic>/` 또는 `.codex/skills/orchestrator_<topic>/`)에 저장 |
...
| 파일 | 경로 | 설명 |
|------|------|------|
| 활성 오케스트레이터 | `.claude/skills/orchestrator_<topic>/SKILL.md` 또는 `.codex/skills/orchestrator_<topic>/SKILL.md` | 실행 중인 파이프라인 계획. 단계 순서, handoff 규칙, review-fix 정책 포함 |
| 실행 로그 | `_sdd/pipeline/log_<topic>_<timestamp>.md` | 각 단계의 시작/완료 시간, 출력 경로, 핵심 결정사항, 에러 기록 |
| 완료 아카이브 | `_sdd/pipeline/orchestrators/<topic>_<timestamp>/` | 완료된 오케스트레이터와 관련 메타데이터 보관 |

- `<topic>`: 기능명을 영문 snake_case로 변환 (e.g., "인증 시스템" -> `auth_system`)
- `<timestamp>`: `YYYYMMDD_HHmmss` 형식

Codex에서는 실행 중 오케스트레이터가 `.codex/agents/`의 custom agents를 직접 spawn합니다. 승인 전에는 `_sdd/env.md`와 `.codex/config.toml`을 함께 읽어 `agents.max_depth`, `agents.max_threads`, 테스트/실행 리소스 갭을 확인합니다.
...
│           ├── SKILL.md                           # 실행 중 활성 오케스트레이터
│           └── skill.json
└── _sdd/
    ├── pipeline/
    │   ├── log_jwt_auth_20260316_143000.md        # 실행 로그
    │   └── orchestrators/
    │       └── jwt_auth_20260316_143000/          # 완료된 오케스트레이터 아카이브
    ├── drafts/
...
수정 중인 활성 오케스트레이터는 `.claude/skills/orchestrator_<topic>/` 또는 `.codex/skills/orchestrator_<topic>/`에 유지됩니다. 파이프라인이 완료되면 `_sdd/pipeline/orchestrators/<topic>_<timestamp>/`로 아카이브되어 나중에 참조할 수 있습니다.
...
> 이 섹션은 Codex 전환기의 운영 체크리스트입니다. wrapper -> custom agent -> nested `write_phased` -> autopilot dry-run 검증이 충분히 끝나고 팀의 신뢰 기준이 안정화되면, 더 짧은 유지보수 노트로 축약하거나 제거할 수 있습니다.

### 8.1 Wrapper -> Agent Smoke Test

- 대상: `/feature-draft`, `/implementation-plan`, `/implementation`, `/implementation-review`, `/spec-review`
- 확인:
  - wrapper skill이 대응 custom agent 이름과 기본 산출물 계약만 얇게 설명하는가
...
- active orchestrator가 `.codex/skills/orchestrator_<topic>/`에 유지된다
- 완료 후 `_sdd/pipeline/orchestrators/<topic>_<timestamp>/`로 아카이브된다
...
- 확인:
  ...
  - `.codex/skills/orchestrator_<topic>/`가 active 상태로 유지되는가
  - `_sdd/pipeline/log_<topic>_<timestamp>.md`가 생성/갱신되는가
  - `_sdd/pipeline/report_<topic>_<timestamp>.md`가 생성되는가
  - 완료 후 `_sdd/pipeline/orchestrators/<topic>_<timestamp>/`로 아카이브되는가
