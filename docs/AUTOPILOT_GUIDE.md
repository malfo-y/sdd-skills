...
SDD 파이프라인을 자동으로 실행하는 autopilot 메타스킬 가이드
...
**autopilot**은 이 전체 파이프라인을 하나의 명령으로 자동 실행하는 **적응형 오케스트레이터 메타스킬**입니다. 사용자의 기능 요청을 받으면 요구사항 토론, 코드베이스 탐색, 규모 판단, 파이프라인 구성, 에이전트 순차 실행, 리뷰-수정 루프, 테스트, 스펙 동기화까지 end-to-end로 처리합니다. 사용자는 초반 요구사항 확인과 파이프라인 승인만 참여하면 되고, 이후는 autopilot이 자율적으로 완료합니다.

---

## 2. 핵심 개념

autopilot을 이해하기 위한 3가지 핵심 개념입니다.

### 2.1 2-Phase Orchestration (Interactive → Autonomous)

autopilot은 실행을 두 단계로 나눕니다.
...
autopilot은 요청 분석과 코드베이스 탐색 결과를 기반으로 규모를 자동 판단하고, 그에 맞는 파이프라인을 선택합니다. 사용자가 규모를 직접 지정할 필요가 없습니다.
...
정량적 기준과 정성적 기준(복잡도, 의존성, 테스트 범위)이 다른 규모를 가리키면, autopilot은 **더 큰 규모를 선택**합니다.

### 2.3 Agent Wrapper 패턴 (스킬 → 에이전트 위임)

autopilot은 각 SDD 스킬을 **서브에이전트**로 호출합니다. 에이전트 간 상태는 파일 경로를 통해 전달되며, 에이전트의 전체 출력을 부모 컨텍스트에 누적하지 않습니다.
...
각 에이전트에는 이전 에이전트의 **출력 파일 경로**와 **사용자의 원래 요청**이 함께 전달됩니다. autopilot은 에이전트 결과에서 핵심 정보(출력 파일 경로, 주요 결정사항)만 추출하여 파이프라인 로그에 기록합니다.
...
```bash
/autopilot JWT 기반 인증 시스템을 구현해줘. 로그인, 로그아웃, 토큰 갱신 포함.

/autopilot 결제 시스템 전체 구현. Stripe API 연동, 웹훅 처리, 환불 로직 포함.

/autopilot 로그인 버튼 색상을 파란색에서 초록색으로 변경해줘.
```
...
### 3.3 사용자 역할 vs autopilot 역할

| Step | 사용자가 해야 할 것 | autopilot이 자동으로 하는 것 |
|------|---------------------|---------------------------|
...
> **핵심**: Step 6에서 승인한 이후(Phase 2)에는 autopilot이 사용자에게 질문하지 않습니다. 완료까지 자율 실행합니다.

---

## 4. 규모별 파이프라인

autopilot은 Step 4에서 판단한 규모에 따라 서로 다른 에이전트 조합을 사용합니다.
...
```
사용자: /autopilot 로그인 버튼 색상을 파란색에서 초록색으로 변경해줘

[autopilot] 요청 분석: 소규모 예측 (UI 컴포넌트 1개 수정)
[autopilot] 질문: "로그인 버튼이 있는 파일이 어디인가요?"
...
[autopilot] 코드베이스 탐색 완료: src/components/LoginButton.tsx 식별
[autopilot] 규모 판단: 소규모 (파일 1개, 스펙 변경 없음)
[autopilot] 파이프라인: implementation → 인라인 테스트
...
[autopilot] Step 1/1: implementation 시작...
[autopilot] Step 1/1: implementation 완료 -- src/components/LoginButton.tsx 수정
[autopilot] 테스트 통과 (3/3)
[autopilot] 완료 -- 총 소요 시간: 2분
```

### 5.2 중규모 -- "JWT 인증 시스템 추가"

```
사용자: /autopilot JWT 기반 인증 시스템 구현해줘. 로그인, 로그아웃, 토큰 갱신 포함.

[autopilot] 요청 분석: 중규모 예측 (여러 컴포넌트 언급)
[autopilot] 질문: "비밀번호 해싱 알고리즘과 토큰 만료 시간 정책은?"
...
[autopilot] 질문: "기존 User 모델을 확장할까요, 새 모델을 만들까요?"
  1. "기존 User 모델 확장"
  2. "새 Auth 모델 생성"
  3. "충분합니다 -- 진행해주세요"
...
[autopilot] 코드베이스 탐색 완료: Express.js 프로젝트, 관련 파일 7개 식별
[autopilot] 규모 판단: 중규모 (파일 7개, 신규 컴포넌트 3개, 스펙 패치 필요)
...
[autopilot] Step 1/5: feature-draft 완료 -- _sdd/drafts/feature_draft_jwt_auth.md
[autopilot] Step 2/5: implementation-plan 완료 -- _sdd/implementation/IMPLEMENTATION_PLAN.md
[autopilot] Step 3/5: implementation 완료 -- 7개 파일 생성/수정
[autopilot] Review-Fix Round 1/3: critical 1건, high 1건 -- 수정 중...
[autopilot] Review-Fix Round 2/3: critical 0건, high 0건 -- 리뷰 통과
[autopilot] 테스트 통과 (18/18)
[autopilot] Step 5/5: spec-update-done 완료 -- _sdd/spec/main.md 업데이트
[autopilot] 완료 -- 총 소요 시간: 20분
```

### 5.3 대규모 -- "결제 시스템 전체 구현"

```
사용자: /autopilot 결제 시스템 전체 구현. Stripe API 연동, 웹훅 처리, 환불 로직 포함.

[autopilot] 요청 분석: 대규모 예측 (시스템 레벨 변경)
[autopilot] 질문 1: "결제 수단은? (카드만 / 카드+가상계좌 / 전체)"
[autopilot] 질문 2: "결제 상태 관리 정책은?"
[autopilot] 질문 3: "테스트 환경은? (Stripe test mode / mock)"
[autopilot] 질문 4: "웹훅 보안 검증 방식은?"
... (3-5회 토론)

[autopilot] 코드베이스 탐색 완료: 15개 파일 영향, 신규 모듈 5개 필요
[autopilot] 규모 판단: 대규모 (파일 15개+, 신규 컴포넌트 5개, 스펙 신규 섹션)
...
[autopilot] Step 1/7: feature-draft 완료
[autopilot] Step 2/7: spec-update-todo 완료 -- 스펙에 결제 시스템 섹션 추가
[autopilot] Step 3/7: implementation-plan 완료 -- 3 phase로 분할
[autopilot] Step 4/7: implementation 완료 -- 18개 파일 생성/수정
[autopilot] Review-Fix Round 1/3: critical 2건 -- 수정 중...
[autopilot] Review-Fix Round 2/3: critical 0건, high 0건 -- 리뷰 통과
[autopilot] Step 6/7: 테스트 통과 (32/32)
[autopilot] Step 7/7: spec-update-done 완료
[autopilot] 완료 -- 총 소요 시간: 45분
```
...
autopilot은 파이프라인 실행 중 다음 파일들을 생성합니다.
...
### Q1. autopilot vs 개별 스킬 직접 호출 -- 언제 뭘 쓰나?

| 상황 | 추천 |
|------|------|
| "이 기능 처음부터 끝까지 구현해줘" | `/autopilot` |
| "스펙 리뷰만 해줘" | `/spec-review` 직접 호출 |
| "이미 구현 계획이 있어, 구현만 해줘" | `/implementation` 직접 호출 |
| "토론만 하고 싶어" | `/discussion` 직접 호출 |
| 여러 스킬을 자동으로 연결해야 할 때 | `/autopilot` |
| 단일 스킬로 완료 가능한 작업 | 개별 스킬 직접 호출 |

**판단 기준**: 2개 이상의 스킬을 연속으로 호출해야 한다면 autopilot을 사용하세요. 단일 스킬로 충분하다면 직접 호출이 빠릅니다.

### Q2. 파이프라인 중간에 실패하면?

autopilot은 에이전트 실패 시 **최대 3회 재시도**합니다. 재시도에도 실패하면:
...
- **"파이프라인을 수정하고 싶습니다"**: 수정 사항을 텍스트로 입력하면 autopilot이 오케스트레이터를 수정합니다.
- **"오케스트레이터 전체를 보고 싶습니다"**: 전체 내용을 출력한 뒤 재확인합니다.

수정 후에도 오케스트레이터는 `_sdd/pipeline/`에 저장되므로 나중에 참조할 수 있습니다.

### Q4. autopilot이 규모를 잘못 판단하면?

Step 6에서 파이프라인 요약을 확인할 때, 규모가 맞지 않다고 느끼면 "파이프라인을 수정하고 싶습니다"를 선택하여 규모와 에이전트 조합을 변경할 수 있습니다.

### Q5. 스펙이 없는 프로젝트에서도 autopilot을 쓸 수 있나?

사용할 수 있습니다. 단, autopilot은 `_sdd/spec/`의 존재 여부에 따라 파이프라인을 조정합니다. 스펙이 없으면 `spec-update-todo`와 `spec-update-done` 단계가 생략될 수 있습니다. 스펙 기반 워크플로우의 이점을 최대한 활용하려면 먼저 `/spec-create`로 스펙을 생성하는 것을 권장합니다.

---

## 8. 관련 스킬

autopilot이 내부적으로 호출하는 에이전트들을 개별 스킬로도 직접 사용할 수 있습니다.
