...
| Layer | 역할 | 담는 것 |
|------|------|---------|
| Harness (AGENTS.md + 훅 자산) | repo 작업 규약(how) / 작업 진입 | 작업 원칙, 읽는 순서, 검증 표준, 워크플로우 단계 순서, 판단 기준 포인터, 그리고 그 규약을 강제하거나 되돌려 놓는 실행 자산 |
...
하네스는 별도 레이어이며 global spec 본문을 키우지 않는다. 이해(what/why)는 global spec, 작업 규약(how)은 하네스가 담는다. 하네스는 **문서 규약만이 아니다** — 규약 중 모델 재량으로 흘릴 수 있는 것은 실행 자산이 맡는다. 실행 자산은 두 방향으로 일한다: 규약을 **강제**하거나(work log 커밋 게이트), 규약이 사라진 자리에 **되돌려 놓는다**(compact/clear 후 하네스 재주입). 산문으로만 적힌 규약은 지켜지지 않고 컨텍스트에서 사라진 규약은 존재하지 않는 것과 같다는 관찰이, 이 레이어에 실행 자산을 둔 이유다. 하네스는 스킬 카탈로그·라우팅 표를 담지 않고(설치된 SDD 스킬을 가리킨다), repo-specific 행동 트리거도 담지 않는다(이는 global spec Guardrails가 단일 소스다).
