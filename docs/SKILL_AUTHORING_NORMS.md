...
- [ ] **재구성 금지, 복사 지시.** reference를 "생성/사용하라"고 하면 모델이 재구성하며 내용이 유실된다. 전파가 필요한 템플릿은 Read 후 필수 구조를 복사하도록 지시한다. 슬롯 치환·반복 행·조건부 생략·표시 문구 번역은 reference가 허용한 범위만 적용하며, 그 밖의 순서·의미는 보존한다.
