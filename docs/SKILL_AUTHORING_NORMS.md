...
- [ ] **재구성 금지, 복사 지시.** reference를 "생성/사용하라"고 하면 모델이 재구성하며 내용이 유실된다. 전파가 필요한 템플릿은 "Read 후 verbatim 복사"로 기계적으로 지시하고, 슬롯 치환·반복 행·조건부 생략·표시 문구 번역은 reference가 명시한 예외 목록으로만 허용한다. 그 밖의 줄은 추가·삭제·재배열·요약하지 않는다.
