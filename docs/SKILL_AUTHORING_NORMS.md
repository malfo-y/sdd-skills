...
Anthropic은 Claude 5 세대 모델에서 Claude Code 시스템 프롬프트의 80% 이상을 걷어냈고 성능 저하가 없었다. 이전 세대에서 필요했던 촘촘한 규칙·예시·반복 지시가 이제는 오히려 비용이다: 모델은 규칙 나열보다 **판단 기준과 맥락**을 줄 때 더 잘 일반화한다.
...
| 이전 (Claude 4 이전) | 지금 (Claude 5 세대) | 이 repo에서의 의미 |
|---|---|---|
...
| 지시를 반복 | 한 곳에만 배치 | 규범의 단일 홈 원칙 — canonical 목록 복제 금지 |
